...
bool RAWParams::operator ==(const RAWParams& other) const
{
    return
        ...
        && rawCrop == other.rawCrop
        && rcX == other.rcX
        && rcY == other.rcY
        && rcWidth == other.rcWidth
        && rcHeight == other.rcHeight;
}
...
// Raw
        ...
        saveToKeyfile(!pedited || pedited->raw.rawCrop, "RAW", "RawCropEnabled", raw.rawCrop, keyFile);
        saveToKeyfile(!pedited || pedited->raw.rcX, "RAW", "RawCropX", raw.rcX, keyFile);
        saveToKeyfile(!pedited || pedited->raw.rcY, "RAW", "RawCropY", raw.rcY, keyFile);
        saveToKeyfile(!pedited || pedited->raw.rcWidth, "RAW", "RawCropWidth", raw.rcWidth, keyFile);
        saveToKeyfile(!pedited || pedited->raw.rcHeight, "RAW", "RawCropHeight", raw.rcHeight, keyFile);
...
int ProcParams::load(const Glib::ustring& fname, ParamsEdited* pedited)
{
    ...
    try {
        ...
        if (keyFile.has_group ("RAW")) {
            ...
            assignFromKeyfile(keyFile, "RAW", "RawCropEnabled", pedited, raw.rawCrop, pedited->raw.rawCrop);
            assignFromKeyfile(keyFile, "RAW", "RawCropX", pedited, raw.rcX, pedited->raw.rcX);
            assignFromKeyfile(keyFile, "RAW", "RawCropY", pedited, raw.rcY, pedited->raw.rcY);
            assignFromKeyfile(keyFile, "RAW", "RawCropWidth", pedited, raw.rcWidth, pedited->raw.rcWidth);
            assignFromKeyfile(keyFile, "RAW", "RawCropHeight", pedited, raw.rcHeight, pedited->raw.rcHeight);
