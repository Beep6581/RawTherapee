...
//    lp.strucc = local_struc;
    lp.war = local_warm;
    lp.hsena = locallab.spots.at(sp).expshadhigh;
    lp.highlihs = highhs;
    lp.shadowhs = shadhs;
    lp.radiushs = radhs;
...
//shadow highlight
        ...
        if ((lp.highlihs > 0.f || lp.shadowhs > 0.f) && call < 3  && lp.hsena) {
            LabImage *bufexporig = nullptr;
            LabImage *bufexpfin = nullptr;
            ...
            if (call <= 3) { //simpleprocess, dcrop, improccoordinator
                ...
                transit_shapedetect(9, bufexpfin, nullptr, buflight, bufl_ab, nullptr, nullptr, nullptr, false, hueref, chromaref, lumaref, sobelref, 0.f,  nullptr, lp, original, transformed, cx, cy, sk);
...
//                        int ncstdv = 0.f;
                ...
                if ((lp.showmaskcolmet == 2  || lp.enaColorMask || lp.showmaskcolmet == 3)  && lp.radmacol > 0.f) { 
                    ...
                    for (int y = 0; y < transformed->H ; y++) //{
                        for (int x = 0; x < transformed->W; x++) {
                            ...
                            if (lox >= begx && lox < xEn && loy >= begy && loy < yEn) {
                                bufmaskblurcol->L[loy - begy][lox - begx] = ble[loy - begy][lox - begx] * 32768.f;
                            }
                        }
                }
                ...
                if (lp.showmaskcolmet == 2  || lp.enaColorMask || lp.showmaskcolmet == 3) {
...
#endif
                    {
                        ...
                        gaussianBlur(bufmaskblurcol->a, bufmaskorigcol->a, bfw, bfh, 1.f + (0.2f * lp.radmacol) / sk);
                        gaussianBlur(bufmaskblurcol->b, bufmaskorigcol->b, bfw, bfh, 1.f + (0.2f * lp.radmacol) / sk);
                    }
