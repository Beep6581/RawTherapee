...
void ColorCurveFilter::process (MultiImage* sourceImage, MultiImage* targetImage, Buffer<int>* buffer) {

    int* myCurve;

    // curve is only generated once: in the root filter chain
    if (procParams->colorBoost.enable_saturationlimiter && c>1) {
    ...
    for (int i=0; i<sourceImage->height; i++) {
		for (int j=0; j<sourceImage->width; j++) {
            ...
            if (allowed_cmul >= 1.0 && procParams->colorBoost.avoidclip) {
                double cclip = 100000;
                double cr = tightestroot ((double)lnew->L[i][j]/655.35, (double)(oa + shift_a)*500/16384*amul, (double)(ob + shift_b)*200/16384*bmul, 3.079935, -1.5371515, -0.54278342);
                double cg = tightestroot ((double)lnew->L[i][j]/655.35, (double)(oa + shift_a)*500/16384*amul, (double)(ob + shift_b)*200/16384*bmul, -0.92123418, 1.87599, 0.04524418);
                double cb = tightestroot ((double)lnew->L[i][j]/655.35, (double)(oa + shift_a)*500/16384*amul, (double)(ob + shift_b)*200/16384*bmul, 0.052889682, -0.20404134, 1.15115166);
