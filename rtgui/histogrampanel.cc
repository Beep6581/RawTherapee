    ...
    pointer_moved_delayed_call(
        [this](bool validPos, const rtengine::procparams::ColorManagementParams *cmp,
            ...
            if (!validPos) {
                ...
                if (histogramRGBArea) {
                    update_hist_rgb_area =
                        histogramRGBArea->updatePointer(r, g, b, cmp);
                }
    ...
    panel_listener(nullptr)
{
    ...
    setExpandAlignProperties(
        histogramArea, true, true, Gtk::ALIGN_FILL, Gtk::ALIGN_FILL);

    histogramRGBAreaHori.reset(new HistogramRGBAreaHori());
    setExpandAlignProperties(
        histogramRGBAreaHori.get(), true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_END);

    histogramRGBAreaVert.reset(new HistogramRGBAreaVert());
    setExpandAlignProperties(
        histogramRGBAreaVert.get(), false, true, Gtk::ALIGN_END, Gtk::ALIGN_FILL);
    ...
    histogramArea->signal_factor_changed().connect(
        sigc::mem_fun(*histogramRGBAreaHori, &HistogramRGBArea::factorChanged));
    histogramArea->signal_factor_changed().connect(
        sigc::mem_fun(*histogramRGBAreaVert, &HistogramRGBArea::factorChanged));

    gfxGrid = Gtk::manage(new Gtk::Grid());
    gfxGrid->set_row_spacing(1);
    gfxGrid->set_column_spacing(1);
    gfxGrid->add(*histogramArea);
    gfxGrid->attach_next_to(*histogramRGBAreaVert, *histogramArea,
        options.histogramPosition == 1 ? Gtk::POS_RIGHT : Gtk::POS_LEFT, 1, 1);
    gfxGrid->attach_next_to(
        *histogramRGBAreaHori, *histogramArea, Gtk::POS_BOTTOM, 1, 1);
    ...
    Gtk::Image *histImage = Gtk::manage(
        new RTImage("histogram-type-histogram-small", Gtk::ICON_SIZE_BUTTON));
    Gtk::Image *histRawImage = Gtk::manage(
        new RTImage("histogram-type-histogram-raw-small", Gtk::ICON_SIZE_BUTTON));
    Gtk::Image *paradeImage =
        Gtk::manage(new RTImage("histogram-type-parade-small", Gtk::ICON_SIZE_BUTTON));
    Gtk::Image *waveImage = Gtk::manage(
        new RTImage("histogram-type-waveform-small", Gtk::ICON_SIZE_BUTTON));
    Gtk::Image *vectHcImage = Gtk::manage(
        new RTImage("histogram-type-vectorscope-hc-small", Gtk::ICON_SIZE_BUTTON));
    Gtk::Image *vectHsImage = Gtk::manage(
        new RTImage("histogram-type-vectorscope-hs-small", Gtk::ICON_SIZE_BUTTON));
    ...
    if (options.histogramDrawMode == 0) showMode->set_image(*mode0Image);
    ...
    setExpandAlignProperties(
        showRed, false, false, Gtk::ALIGN_CENTER, Gtk::ALIGN_CENTER);
    setExpandAlignProperties(
        showGreen, false, false, Gtk::ALIGN_CENTER, Gtk::ALIGN_CENTER);
    setExpandAlignProperties(
        showBlue, false, false, Gtk::ALIGN_CENTER, Gtk::ALIGN_CENTER);
    setExpandAlignProperties(
        showValue, false, false, Gtk::ALIGN_CENTER, Gtk::ALIGN_CENTER);
    setExpandAlignProperties(
        showChro, false, false, Gtk::ALIGN_CENTER, Gtk::ALIGN_CENTER);
    setExpandAlignProperties(
        showMode, false, false, Gtk::ALIGN_CENTER, Gtk::ALIGN_CENTER);
    setExpandAlignProperties(
        scopeOptions, false, false, Gtk::ALIGN_START, Gtk::ALIGN_CENTER);
    setExpandAlignProperties(
        showBAR, false, false, Gtk::ALIGN_CENTER, Gtk::ALIGN_CENTER);
    setExpandAlignProperties(
        scopeOptions, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_FILL);
    setExpandAlignProperties(
        scopeHistBtn, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_FILL);
    setExpandAlignProperties(
        scopeHistRawBtn, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_FILL);
    setExpandAlignProperties(
        scopeParadeBtn, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_FILL);
    setExpandAlignProperties(
        scopeWaveBtn, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_FILL);
    setExpandAlignProperties(
        scopeVectHcBtn, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_FILL);
    setExpandAlignProperties(
        scopeVectHsBtn, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_FILL);
    setExpandAlignProperties(
        persistentButtons, false, true, Gtk::ALIGN_START, Gtk::ALIGN_FILL);
    setExpandAlignProperties(
        optionButtons, false, true, Gtk::ALIGN_START, Gtk::ALIGN_FILL);

    showRed->signal_toggled().connect(
        sigc::mem_fun(*this, &HistogramPanel::red_toggled), showRed);
    showGreen->signal_toggled().connect(
        sigc::mem_fun(*this, &HistogramPanel::green_toggled), showGreen);
    showBlue->signal_toggled().connect(
        sigc::mem_fun(*this, &HistogramPanel::blue_toggled), showBlue);
    showValue->signal_toggled().connect(
        sigc::mem_fun(*this, &HistogramPanel::value_toggled), showValue);
    showChro->signal_toggled().connect(
        sigc::mem_fun(*this, &HistogramPanel::chro_toggled), showChro);
    showMode->signal_released().connect(
        sigc::mem_fun(*this, &HistogramPanel::mode_released), showMode);
    scopeOptions->signal_toggled().connect(
        sigc::mem_fun(*this, &HistogramPanel::scopeOptionsToggled));
    showBAR->signal_toggled().connect(
        sigc::mem_fun(*this, &HistogramPanel::bar_toggled), showBAR);
    scopeHistBtn->signal_toggled().connect(
        sigc::bind(sigc::mem_fun(*this, &HistogramPanel::type_selected), scopeHistBtn));
    scopeHistRawBtn->signal_toggled().connect(sigc::bind(
        sigc::mem_fun(*this, &HistogramPanel::type_selected), scopeHistRawBtn));
    scopeParadeBtn->signal_toggled().connect(sigc::bind(
        sigc::mem_fun(*this, &HistogramPanel::type_selected), scopeParadeBtn));
    scopeWaveBtn->signal_toggled().connect(
        sigc::bind(sigc::mem_fun(*this, &HistogramPanel::type_selected), scopeWaveBtn));
    scopeVectHcBtn->signal_toggled().connect(sigc::bind(
        sigc::mem_fun(*this, &HistogramPanel::type_selected), scopeVectHcBtn));
    scopeVectHsBtn->signal_toggled().connect(sigc::bind(
        sigc::mem_fun(*this, &HistogramPanel::type_selected), scopeVectHsBtn));

    brightnessWidget = Gtk::manage(new Gtk::Scale(Gtk::ORIENTATION_VERTICAL));
    brightnessWidget->set_inverted();
    brightnessWidget->set_range(
        log(HistogramArea::MIN_BRIGHT), log(HistogramArea::MAX_BRIGHT));
    brightnessWidget->set_draw_value(false);
    brightnessWidget->signal_value_changed().connect(
        sigc::mem_fun(*this, &HistogramPanel::brightnessWidgetValueChanged));
    brightnessWidget->set_name("histScale");
    brightnessWidget->set_tooltip_text(M("HISTOGRAM_TOOLTIP_TRACE_BRIGHTNESS"));
    setExpandAlignProperties(
        brightnessWidget, true, false, Gtk::ALIGN_CENTER, Gtk::ALIGN_START);
    ...
    setExpandAlignProperties(
        separator, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_CENTER);
    ...
    brightness_changed_connection = histogramArea->getBrighnessChangedSignal().connect(
        sigc::mem_fun(*this, &HistogramPanel::brightnessUpdated));
    rconn =
...
void HistogramPanel::resized(Gtk::Allocation &req)
{
    ...
    bool size_changed = old_height != req.get_height() || old_width != req.get_width();
...
void HistogramPanel::pointerMoved(bool validPos,
...
void HistogramPanel::reorder(Gtk::PositionType align)
{
    if (align == Gtk::POS_LEFT) {
        ...
        gfxGrid->remove(*histogramRGBAreaVert);
        gfxGrid->attach_next_to(
            *histogramRGBAreaVert, *histogramArea, Gtk::POS_LEFT, 1, 1);
...
double HistogramScaling::log(double vsize, double val)
{
    ...
    return vsize * std::log(factor / (factor + val)) /
           std::log(factor / (factor + vsize));
}
...
void HistogramRGBArea::getPreferredThickness(
    int &min_thickness, int &natural_thickness) const
{
    int minimumLength = 0;
    int naturalLength = 0;
...
void HistogramRGBArea::getPreferredThicknessForLength(
    int length, int &min_thickness, int &natural_thickness) const
{
...
// unused?
void HistogramRGBArea::getPreferredLengthForThickness(
    int thickness, int &min_length, int &natural_length) const
{
...
void HistogramRGBArea::updateDrawingArea(const ::Cairo::RefPtr<Cairo::Context> &cc)
{
    ...
    // Note: updateDrawingArea is called by the on_draw function so its call does not
    // need to be protected GThreadLock lock; // All GUI access from idle_add callbacks
    // or separate thread HAVE to be protected
    ...
    if ((needLuma || needChroma) && (scopeType == Options::ScopeType::HISTOGRAM ||
...
bool HistogramRGBArea::updatePointer(const int new_r, const int new_g, const int new_b,
    const rtengine::procparams::ColorManagementParams *cmp)
{
    ...
    if (new_r == -1 || new_g == -1 || new_b == -1) {
        ...
        ImProcFunctions::rgb2lab(static_cast<std::uint8_t>(r),
            static_cast<std::uint8_t>(g), static_cast<std::uint8_t>(b), lab_L, lab_a,
            lab_b, cmp != nullptr ? *cmp : DEFAULT_CMP, true);
        pointerValid = true;
    }
...
bool HistogramRGBArea::on_draw(const ::Cairo::RefPtr<Cairo::Context> &cr)
{
    ...
    style->render_background(cr, 0., 0., static_cast<double>(get_width()),
    ...
    style->render_frame(cr, 0., 0., static_cast<double>(get_width()),
        static_cast<double>(get_height()));
...
void HistogramRGBAreaHori::drawBar(const Cairo::RefPtr<Cairo::Context> &cc,
    const double value, const double max_value, const int winw, const int winh)
{
    double pos;
    if (options.histogramDrawMode < 2) {
        pos = padding + value * (winw - padding * 2.0) / max_value;
    } else {
        pos = padding + HistogramScaling::log(max_value, value) *
                            (winw - padding * 2.0) / max_value;
    }
    cc->move_to(pos, 0.0);
    ...
    const double value, const double max_value, const int winw, const int winh)
{
    ...
    if (options.histogramDrawMode < 2 ||
        ...
        pos = padding +
              HistogramScaling::log(max_value, value) * (winh - padding * 2.0) /
                  max_value +
              0.5;
    }
...
void HistogramRGBAreaVert::get_preferred_height_vfunc(
    int &minimum_height, int &natural_height) const
{
    getPreferredLength(minimum_height, natural_height);
}

void HistogramRGBAreaVert::get_preferred_width_vfunc(
    int &minimum_width, int &natural_width) const
{
...
void HistogramRGBAreaVert::get_preferred_height_for_width_vfunc(
    int width, int &minimum_height, int &natural_height) const
{
    getPreferredLengthForThickness(width, minimum_height, natural_height);
}

void HistogramRGBAreaVert::get_preferred_width_for_height_vfunc(
    int height, int &minimum_width, int &natural_width) const
{
...
HistogramArea::HistogramArea(DrawModeListener *fml) :
    ...
    vectorscope_scale(0), vect_hc(0, 0), vect_hs(0, 0), vect_hc_buffer_dirty(true),
    vect_hs_buffer_dirty(true), waveform_scale(0), rwave(0, 0), gwave(0, 0),
    bwave(0, 0), lwave(0, 0), parade_buffer_r_dirty(true), parade_buffer_g_dirty(true),
    parade_buffer_b_dirty(true), wave_buffer_dirty(true), wave_buffer_luma_dirty(true),
    LUT_valid(false),
    // Intensity of waveform and vectorscope trace
    trace_brightness(1.f),
    // Saved pointer parameters
    pointer_red(-1), pointer_green(-1), pointer_blue(-1), pointer_a(0.f),
    pointer_b(0.f), pointer_valid(false),
    // Drawing options
    needRed(options.histogramRed), needGreen(options.histogramGreen),
    needBlue(options.histogramBlue), needLuma(options.histogramLuma),
    needChroma(options.histogramChroma), needPointer(options.histogramBar),
    scopeType(options.histogramScopeType), drawMode(options.histogramDrawMode),
    myDrawModeListener(fml),
...
HistogramArea::~HistogramArea() { idle_register.destroy(); }
...
void HistogramArea::get_preferred_height_vfunc(
    int &minimum_height, int &natural_height) const
{
...
void HistogramArea::get_preferred_width_vfunc(
    int &minimum_width, int &natural_width) const
{
...
void HistogramArea::get_preferred_height_for_width_vfunc(
    int width, int &minimum_height, int &natural_height) const
{
    minimum_height = 0;
    natural_height = 0;
...
void HistogramArea::get_preferred_width_for_height_vfunc(
    int height, int &minimum_width, int &natural_width) const
{
    get_preferred_width_vfunc(minimum_width, natural_width);
}
...
void HistogramArea::update(const LUTu &histRed, const LUTu &histGreen,
    const LUTu &histBlue, const LUTu &histLuma, const LUTu &histChroma,
    ...
    const array2D<int> &waveformBlue, const array2D<int> &waveformLuma)
{
    ...
    idle_register.add([this, &histRed, &histGreen, &histBlue, &histLuma, &histChroma,
                          &histRedRaw, &histGreenRaw, &histBlueRaw, vectorscopeScale,
                          &vectorscopeHC, &vectorscopeHS, waveformScale, &waveformRed,
                          &waveformGreen, &waveformBlue, &waveformLuma]() -> bool {
        ...
        if (histRed) {
            switch (scopeType) {
                ...
                case ScopeType::WAVEFORM: {
                    ...
                    bwave = waveformBlue;
                    lwave = waveformLuma;
                    parade_buffer_r_dirty = parade_buffer_g_dirty =
                        parade_buffer_b_dirty = wave_buffer_dirty =
                            wave_buffer_luma_dirty = true;
                    break;
                }
                case ScopeType::VECTORSCOPE_HS:
...
void HistogramArea::updateDrawingArea(const ::Cairo::RefPtr<Cairo::Context> &cr)
{
    ...
    if (scopeType == Options::ScopeType::PARADE ||
        scopeType == Options::ScopeType::WAVEFORM) {
        for (int i = 0; i <= nrOfVGridPartitions; i++) {
            const double ypos =
                winh - padding - (pow(2.0, i) - 1) * (winh - 2 * padding - 1) / 255.0;
            cr->move_to(0, ypos);
            cr->line_to(winw, ypos);
            cr->stroke();
        }
    } else if (scopeType == Options::ScopeType::VECTORSCOPE_HC ||
               scopeType == Options::ScopeType::VECTORSCOPE_HS) {
        // Vectorscope has no gridlines.
    } else if (drawMode == 0) {
        for (int i = 1; i < nrOfHGridPartitions; i++) {
            cr->move_to(
                padding, i * static_cast<double>(winh) / nrOfHGridPartitions + 0.5);
        ...
        for (int i = 1; i < nrOfHGridPartitions; i++) {
            cr->move_to(
                padding, winh -
                             HistogramScaling::log(winh,
                                 i * static_cast<double>(winh) / nrOfHGridPartitions) +
                             0.5);
            cr->line_to(winw - padding,
                winh -
                    HistogramScaling::log(
                        winh, i * static_cast<double>(winh) / nrOfHGridPartitions) +
                    0.5);
            cr->stroke();
        }
    }
    ...
    if (LUT_valid && (scopeType == Options::ScopeType::HISTOGRAM ||
                         scopeType == Options::ScopeType::HISTOGRAM_RAW)) {
        ...
        // make double copies of LUT, one for faster access, another one to scale down
        // the raw histos
        LUTu rhchanged(256), ghchanged(256), bhchanged(256);
        unsigned int lhisttemp[256] ALIGNED16{0}, chisttemp[256] ALIGNED16{0},
            rhtemp[256] ALIGNED16{0}, ghtemp[256] ALIGNED16{0},
            bhtemp[256] ALIGNED16{0};
...
bool HistogramArea::updatePointer(const int r, const int g, const int b,
    const rtengine::procparams::ColorManagementParams *cmp)
{
    // Do not update pointer values if pointer is not visible
    // (by user choice or according to scope type)
    if (!needPointer || !(scopeType == Options::ScopeType::VECTORSCOPE_HC ||
                            scopeType == Options::ScopeType::VECTORSCOPE_HS)) {
        return false;
    }
    ...
    if (r == -1 || g == -1 || b == -1) {
        ...
        pointer_blue = b;
        ImProcFunctions::rgb2lab(static_cast<std::uint8_t>(r),
            static_cast<std::uint8_t>(g), static_cast<std::uint8_t>(b), L, pointer_a,
            pointer_b, cmp != nullptr ? *cmp : DEFAULT_CMP, true);
        L /= 327.68f;
        pointer_a /= 327.68f;
        pointer_b /= 327.68f;
        pointer_valid = true;
...
void HistogramArea::drawCurve(const Cairo::RefPtr<Cairo::Context> &cr, const LUTu &data,
    const double scale, const int hsize, const int vsize)
{
    cr->set_line_width(1.);
    cr->move_to(padding, vsize - 1);
    const double new_scale =
        scale <= 0.0 ? 0.001 : scale; // avoid division by zero and negative values
...
void HistogramArea::drawMarks(const Cairo::RefPtr<Cairo::Context> &cr, const LUTu &data,
    const double scale, const int wsize, int &ui, int &oi)
{
    if (data[0] > scale) {
...
void HistogramArea::drawParade(
    const Cairo::RefPtr<Cairo::Context> &cr, const int w, const int h)
{
    ...
    const int cairo_stride = Cairo::ImageSurface::format_stride_for_width(
        Cairo::FORMAT_ARGB32, rwave.getWidth());
    const auto buffer_size =
        static_cast<std::vector<unsigned char>::size_type>(wave_height) * cairo_stride;
    ...
    if (parade_buffer_r_dirty && needRed) {
        ...
        for (int val = 0; val < wave_height; val++) {
            ...
            std::uint32_t *const buffer_r_row = reinterpret_cast<uint32_t *>(
                parade_buffer_r.data() + (255 - val) * cairo_stride);
    ...
    if (parade_buffer_g_dirty && needGreen) {
        ...
        for (int val = 0; val < wave_height; val++) {
            ...
            std::uint32_t *const buffer_g_row = reinterpret_cast<uint32_t *>(
                parade_buffer_g.data() + (255 - val) * cairo_stride);
            for (int col = 0; col < wave_width; col++) {
    ...
    if (parade_buffer_b_dirty && needBlue) {
        ...
        for (int val = 0; val < wave_height; val++) {
            ...
            std::uint32_t *const buffer_b_row = reinterpret_cast<uint32_t *>(
                parade_buffer_b.data() + (255 - val) * cairo_stride);
            for (int col = 0; col < wave_width; col++) {
    ...
    if (wave_buffer_luma_dirty && needLuma) {
        ...
        for (int val = 0; val < wave_height; val++) {
            ...
            std::uint32_t *const buffer_row = reinterpret_cast<uint32_t *>(
                wave_buffer_luma.data() + (255 - val) * cairo_stride);
            for (int col = 0; col < wave_width; col++) {
...
void HistogramArea::drawVectorscope(
    const Cairo::RefPtr<Cairo::Context> &cr, const int w, const int h)
{
    if (scopeType != Options::ScopeType::VECTORSCOPE_HC &&
        scopeType != Options::ScopeType::VECTORSCOPE_HS) {
        return;
    }

    const auto &vect =
        (scopeType == Options::ScopeType::VECTORSCOPE_HC) ? vect_hc : vect_hs;
    auto &vect_buffer = (scopeType == Options::ScopeType::VECTORSCOPE_HC)
                            ? vect_hc_buffer
                            : vect_hs_buffer;
    auto &vect_buffer_dirty = (scopeType == Options::ScopeType::VECTORSCOPE_HC)
                                  ? vect_hc_buffer_dirty
                                  : vect_hs_buffer_dirty;

    const int vect_width = vect.getWidth();
    const int vect_height = vect.getHeight();

    // See Cairo documentation on stride.
    const int cairo_stride =
        Cairo::ImageSurface::format_stride_for_width(Cairo::FORMAT_ARGB32, vect_width);

    if (vect_buffer_dirty && vectorscope_scale > 0) {
        // Arbitrary scale factor multiplied by vectorscope area and divided by
        // current scale.
        const float scale =
            trace_brightness * 8.f * vect_width * vect_height / vectorscope_scale;

        if (vect_buffer.size() !=
            static_cast<std::size_t>(cairo_stride) * vect_height) {
        ...
        for (int y = 0; y < vect_height; y++) {
            ...
            std::uint32_t *const buffer_row = reinterpret_cast<uint32_t *>(
                vect_buffer.data() + (vect_height - 1 - y) * cairo_stride);
    ...
    const float scope_scale =
        fit_width ? (w - 2 * padding) / vect_width : (h - 2 * padding) / vect_height;
    const float scope_size =
        (vectorscope_scale > 0)
            ? scope_scale * std::max<double>(vect_width, vect_height)
    ...
    if (scopeType == ScopeType::VECTORSCOPE_HS) { // Hue-Saturation.
        ...
        for (int i = 0; i < 3; i++) {
            auto gradient =
                Cairo::LinearGradient::create(-line_length, 0, line_length, 0);
    ...
    if (vectorscope_scale > 0) {
        Cairo::RefPtr<Cairo::ImageSurface> surface =
            Cairo::ImageSurface::create(vect_buffer.data(), Cairo::FORMAT_ARGB32,
                vect_width, vect_height, cairo_stride);
        ...
        if (needPointer && pointer_valid) {
            ...
            if (scopeType == ScopeType::VECTORSCOPE_HS) {
                float H, S, L;
                Color::rgb2hslfloat(pointer_red * 257.f, pointer_green * 257.f,
                    pointer_blue * 257.f, H, S, L);
...
void HistogramArea::drawWaveform(
    const Cairo::RefPtr<Cairo::Context> &cr, const int w, const int h)
{
    // Arbitrary scale factor divided by current scale.
    const float scale = trace_brightness * 32.f * 255.f / waveform_scale;
    ...
    if (wave_buffer_dirty && (needRed || needGreen || needBlue)) {
        ...
        for (int val = 0; val < wave_height; val++) {
            ...
            const int *const b_row = bwave[val];
            std::uint32_t *const buffer_row = reinterpret_cast<uint32_t *>(
                wave_buffer.data() + (255 - val) * cairo_stride);
            for (int col = 0; col < wave_width; col++) {
                const unsigned char r =
                    needRed ? std::min<float>(scale * r_row[col], 0xff) : 0;
                const unsigned char g =
                    needGreen ? std::min<float>(scale * g_row[col], 0xff) : 0;
                const unsigned char b =
                    needBlue ? std::min<float>(scale * b_row[col], 0xff) : 0;
    ...
    if (needLuma) {
        surface = Cairo::ImageSurface::create(wave_buffer_luma.data(),
            Cairo::FORMAT_ARGB32, wave_width, wave_height, cairo_stride);
        cr->set_source(surface, 0, 0);
        cr->set_operator(Cairo::OPERATOR_OVER);
        cr->paint();
        surface->finish();
    ...
    if (needRed || needGreen || needBlue) {
        surface = Cairo::ImageSurface::create(wave_buffer.data(), Cairo::FORMAT_ARGB32,
            wave_width, wave_height, cairo_stride);
        cr->set_source(surface, 0, 0);
        cr->set_operator(Cairo::OPERATOR_OVER);
        cr->paint();
        surface->finish();
...
bool HistogramArea::on_motion_notify_event(GdkEventMotion *event)
{
    ...
    if (scopeType == ScopeType::HISTOGRAM ||
        scopeType == ScopeType::HISTOGRAM_RAW) { // Adjust log scale.
        ...
        queue_draw();
    } else if (scopeType == ScopeType::PARADE || scopeType == ScopeType::WAVEFORM ||
               scopeType == ScopeType::VECTORSCOPE_HC ||
               scopeType == ScopeType::VECTORSCOPE_HS) { // Adjust brightness.
        constexpr float RANGE = MAX_BRIGHT / MIN_BRIGHT;
        double dx = (event->x - movingPosition) / get_width();
        float new_brightness =
            LIM<float>(trace_brightness * pow(RANGE, dx), MIN_BRIGHT, MAX_BRIGHT);
...
float HistogramArea::getBrightness(void) { return trace_brightness; }
...
void HistogramArea::setBrightness(float brightness)
{
    ...
    if (brightness != trace_brightness) {
        parade_buffer_r_dirty = parade_buffer_g_dirty = parade_buffer_b_dirty =
            wave_buffer_dirty = wave_buffer_luma_dirty = vect_hc_buffer_dirty =
                vect_hs_buffer_dirty = true;
