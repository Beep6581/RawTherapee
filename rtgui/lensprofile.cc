...
LensProfilePanel::LensProfilePanel() :
    FoldableToolPanel(this, TOOL_NAME, M("TP_LENSPROFILE_LABEL")), lcModeChanged(false),
    lcpFileChanged(false), useDistChanged(false), useVignChanged(false),
    useCAChanged(false), useLensfunChanged(false), lensfunAutoChanged(false),
    lensfunCameraChanged(false), lensfunLensChanged(false), allowFocusDep(true),
    isRaw(true), metadata(nullptr), modesGrid(Gtk::manage(new Gtk::Grid())),
    ...
    ckbUseCA(Gtk::manage((new Gtk::CheckButton(M("TP_LENSPROFILE_USE_CA")))))
{
    ...
    Gtk::Frame *nodesFrame =
        Gtk::manage(new Gtk::Frame(M("TP_LENSPROFILE_MODE_HEADER")));
    nodesFrame->set_label_align(0.025, 0.5);

    modesGrid->get_style_context()->add_class("grid-spacing");
    setExpandAlignProperties(
        modesGrid, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_CENTER);
    ...
    setExpandAlignProperties(
        corrLcpFileChooser, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_CENTER);

    // Manually-selected profile widgets:

    setExpandAlignProperties(
        lensfunCamerasLbl, false, false, Gtk::ALIGN_END, Gtk::ALIGN_CENTER);
    ...
    setExpandAlignProperties(
        lensfunCameras, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_CENTER);
    ...
    setExpandAlignProperties(
        lensfunLensesLbl, false, false, Gtk::ALIGN_END, Gtk::ALIGN_CENTER);
    ...
    setExpandAlignProperties(
        lensfunLenses, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_CENTER);
    ...
    corrLensfunManualRB->signal_toggled().connect(
        sigc::bind(sigc::mem_fun(*this, &LensProfilePanel::onCorrModeChanged),
            corrLensfunManualRB));
    corrLcpFileRB->signal_toggled().connect(sigc::bind(
        sigc::mem_fun(*this, &LensProfilePanel::onCorrModeChanged), corrLcpFileRB));
}

void LensProfilePanel::read(
    const rtengine::procparams::ProcParams *pp, const ParamsEdited *pedited)
{
   ...
   if (!batchMode && !checkLensfunCanCorrect(true)) {
    ...
    if (!batchMode && corrLensfunManualRB->get_active() &&
   !checkLensfunCanCorrect(false)) { corrOffRB->set_active(true);
    }
    */
    ...
    rtengine::procparams::ProcParams *pp, ParamsEdited *pedited)
{
    ...
    if (LCPStore::getInstance()->isValidLCPFileName(
            corrLcpFileChooser->get_filename())) {
...
void LensProfilePanel::onUseDistChanged()
{
    ...
    if (listener) {
        listener->panelChanged(EvLCPUseDist,
            ckbUseDist->get_active() ? M("GENERAL_ENABLED") : M("GENERAL_DISABLED"));
    }
}
...
void LensProfilePanel::onUseVignChanged()
{
    ...
    if (listener) {
        listener->panelChanged(EvLCPUseVign,
            ckbUseVign->get_active() ? M("GENERAL_ENABLED") : M("GENERAL_DISABLED"));
    }
}
...
void LensProfilePanel::onUseCAChanged()
{
    ...
    if (listener) {
        listener->panelChanged(EvLCPUseCA,
            ckbUseCA->get_active() ? M("GENERAL_ENABLED") : M("GENERAL_DISABLED"));
    }
}
...
void LensProfilePanel::onCorrModeChanged(const Gtk::RadioButton *rbChanged)
{
    if (rbChanged->get_active()) {
        // because the method gets called for the enabled AND the disabled RadioButton,
        // we do the processing only for the enabled one
        Glib::ustring mode;
...
bool LensProfilePanel::setLensfunCamera(
    const Glib::ustring &make, const Glib::ustring &model)
{
...
void LensProfilePanel::updateLensfunWarning()
{
    ...
    if (corrLensfunManualRB->get_active() || corrLensfunAutoRB->get_active()) {
        ...
        const LFCamera c = db->findCamera(
            (*itc)[lf->lensfunModelCam.make], (*itc)[lf->lensfunModelCam.model], false);
        const auto itl = lensfunLenses->get_active();
