...
MyDiagonalCurve::MyDiagonalCurve() :
    MyCurve(), closest_point(0), clampedX(0.0), clampedY(0.0), deltaX(0.0), deltaY(0.0),
    distanceX(0.0), distanceY(0.0), ugpX(0.0), ugpY(0.0), activeParam(-1),
    bghistvalid(false), locallabRef(0.0)
...
void MyDiagonalCurve::updateDrawingArea(
    const int handle, const ::Cairo::RefPtr<Cairo::Context> &cr)
{
    if (!get_realized()) {
        return;
    }
    ...
    if (leftBar) {
        ...
        cr->rectangle(
            0.5, graphY - graphH - 0.5 - 0.5, CBAR_WIDTH + 1, (double)graphH + 1. + 1.);
        cr->stroke();
    }
                        ...
                        if (edited_point > -1 && grab_point == -1) {
                            ...
                            if (cursorX >= 0 && cursorX <= graphW && cursorY >= 0 &&
                                ...
                                if (distanceX <= minDistanceX) {
                                    ...
                                    // the cursor is inside the graph but away from
                                    // existing points
                                    new_type = CSPlus;
                                    curveIsDirty = true;
        ...
        case GDK_MOTION_NOTIFY:
            ...
            if (curve.type == DCT_Linear || curve.type == DCT_Spline ||
                ...
                if (grab_point == -1) {
                    if (edited_point == -1) {
...

                        {
                            ...
                            if (cursorX < -RADIUS || cursorX > extendedGraphW ||
                                ...
                                // the cursor is close to an existing point
                                new_type =
                                    CSPlus; // Shown when hovering over node snapping
                        ...
                        if (distanceX <= minDistanceX) {
                            ...
                            // the cursor is outside the graph or inside the graph but
                    ...
                    // the unclamped grabbed point is brought back in the range when
                    // snapTo is active
                    if (snapTo) {
...
CursorShape MyDiagonalCurve::motionNotify(
...
void MyDiagonalCurve::pipetteMouseOver(
...
bool MyDiagonalCurve::pipetteButton1Pressed(EditDataProvider *provider, int modifierKey)
{
    ...
    if (curve.type != DCT_Parametric) {
        ...
        if (distanceX > minDistanceX) {
            ...
            // the graph is refreshed only if a new point is created (snapped to a
            // pixel)
            if (lit_point >= 0) {
...
void MyDiagonalCurve::pipetteDrag(EditDataProvider *provider, int modifierKey)
{
    ...
    getCursorPosition(Gdk::MOTION_NOTIFY, false, cursorX + graphX,
        graphY - cursorY + provider->deltaPrevScreen.y, Gdk::ModifierType(modifierKey));
    ...
    if (curve.type != DCT_Parametric) {
        for (int i = 0; i < (int)curve.x.size(); i++) {
            ...
            if (currDistX < distanceX) {
                ...
                // there is more than 1 point for that X coordinate, we select the
...
void MyDiagonalCurve::reset(const std::vector<double> &resetCurve, double identityValue)
{
    ...
    switch (curve.type) {
        ...
        case DCT_Parametric:
            curve.x.resize(7);
            curve.y.clear();
            // the SHCSelector values doesn't really matter for the identity curve
            // display
            curve.x.at(0) = 0.25;
            curve.x.at(1) = 0.50;
            curve.x.at(2) = 0.75;
