...
public:
    ...
    /// @brief Used to notify all listeners that a parameters has been effectively
    /// changed
    virtual void panelChanged(
        const rtengine::ProcEvent &event, const Glib::ustring &descr) = 0;
...
/// @brief This class control the space around the group of tools inside a tab, as well
/// as the space separating each tool. */
...
public:
    ToolPanel(Glib::ustring toolName = "", bool need11 = false) :
        toolName(toolName), listener(nullptr), tmp(nullptr), batchMode(false),
        multiImage(false), need100Percent(need11)
    {
    }
    virtual ~ToolPanel() {}
    ...
    virtual void write(
        rtengine::procparams::ProcParams *pp, ParamsEdited *pedited = nullptr)
    {
    }
...
public:
    FoldableToolPanel(Gtk::Box *content, Glib::ustring toolName, Glib::ustring UILabel,
        bool need11 = false, bool useEnabled = false);
