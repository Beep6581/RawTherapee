...
// This recursive mutex will be used by gdk_threads_enter/leave instead of a simple
// mutex
    ...
    for (int iArg = 1; iArg < argc; iArg++) {
        ...
        if (currParam.at(0) == '-' && currParam.size() > 1) {
            switch (currParam.at(1)) {
...
#ifndef __APPLE__ // TODO agriggio - there seems to be already some "single instance
                  // app" support for OSX in rtwindow. Disabling it here until I
                  // understand how to merge the two
                ...
                case '?':
                default: {
                    printf("  An advanced, cross-platform program for developing raw "
                           "photos.\n\n");
                    ...
                    printf("Usage:\n");
                    printf(
                        "  %s <folder>           Start File Browser inside folder.\n",
...
#ifndef __APPLE__
                    printf("  -R Raise an already running RawTherapee instance (if "
                           "available)\n");
#endif
...
bool init_rt()
{
    ...
    if (Glib::file_test(
        ...
        g_rename(Glib::build_filename(options.rtdir, "cache").c_str(),
...
RTWindow *create_rt_window()
{
    ...
    rtWindow->setWindowSize(); // Need to be called after RTWindow creation to work with
                               // all OS Windows Manager
    return rtWindow;
}
    ...
    bool create_window()
    {
        ...
        if (!init_rt()) {
            Gtk::MessageDialog msgd(
                "Fatal error!\nThe RT_SETTINGS and/or RT_PATH environment variables "
                "are set, but use a relative path. The path must be absolute!",
...
int main(int argc, char **argv)
{
    ...
    Glib::init(); // called by Gtk::Main, but this may be important for thread handling,
                  // so we call it ourselves now
    Gio::init();
    ...
    if (argc > 1) {
        if (!remote && !Glib::file_test(argv1, Glib::FILE_TEST_EXISTS) &&
            ...
            const bool stdoutRedirecttoConsole =
            ...
            if (stdoutRedirecttoConsole) {
                ...
                // We have to do that in this step, because it controls whether to open
                // a console to show the output of following steps
                bool Console = true;
                ...
                if (Console && AllocConsole()) {
                    ...
                    snprintf(consoletitle, sizeof(consoletitle),
                        "RawTherapee %s Console", RTVERSION);
    ...
    gdk_threads_set_lock_functions(
        G_CALLBACK(myGdkLockEnter), (G_CALLBACK(myGdkLockLeave)));
    gdk_threads_init();
    gtk_init(&argc,
        &argv); // use the "--g-fatal-warnings" command line flag to make warnings fatal
