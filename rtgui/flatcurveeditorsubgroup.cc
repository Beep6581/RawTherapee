    ...
    CurveEditorGroup *prt, Glib::ustring &curveDir) : CurveEditorSubGroup(curveDir)
{
    ...
    if (options.curvebboxpos == 0) {
        ...
        setExpandAlignProperties(
            CPointsbbox, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_CENTER);
        ...
        setExpandAlignProperties(
            CPointsbbox, false, true, Gtk::ALIGN_CENTER, Gtk::ALIGN_FILL);
    ...
    if (options.curvebboxpos == 0) {
        CPointsCurveGrid->attach_next_to(
            *CPointsbbox, *CPointsCurveBox, Gtk::POS_TOP, 1, 1);
        CPointsCurveGrid->attach_next_to(
            *CPointsCoordAdjuster, *CPointsCurveBox, Gtk::POS_BOTTOM, 1, 1);
    } else if (options.curvebboxpos == 1) {
        CPointsCurveGrid->attach_next_to(
            *CPointsbbox, *CPointsCurveBox, Gtk::POS_RIGHT, 1, 1);
        CPointsCurveGrid->attach_next_to(
            *CPointsCoordAdjuster, *CPointsCurveBox, Gtk::POS_BOTTOM, 2, 1);
    } else if (options.curvebboxpos == 2) {
        CPointsCurveGrid->attach_next_to(
            *CPointsCoordAdjuster, *CPointsCurveBox, Gtk::POS_BOTTOM, 1, 1);
        CPointsCurveGrid->attach_next_to(
            *CPointsbbox, *CPointsCoordAdjuster, Gtk::POS_BOTTOM, 1, 1);
    } else if (options.curvebboxpos == 3) {
        CPointsCurveGrid->attach_next_to(
            *CPointsbbox, *CPointsCurveBox, Gtk::POS_LEFT, 1, 1);
        CPointsCurveGrid->attach_next_to(
            *CPointsCoordAdjuster, *CPointsbbox, Gtk::POS_BOTTOM, 2, 1);
    }
...
FlatCurveEditorSubGroup::~FlatCurveEditorSubGroup() { delete CPointsCurveGrid; }

/*
 * Add a new curve to the curves list
 */
FlatCurveEditor *FlatCurveEditorSubGroup::addCurve(
    Glib::ustring curveLabel, bool isPeriodic)
{
...
void FlatCurveEditorSubGroup::pipetteMouseOver(
...
bool FlatCurveEditorSubGroup::pipetteButton1Pressed(
    EditDataProvider *provider, int modifierKey)
{
    CurveEditor *curveEditor = static_cast<FlatCurveEditor *>(parent->displayedCurve);
    ...
    if (parent->displayedCurve) {
        switch (parent->displayedCurve->selected) {
            /*case (FCT_Parametric):
               ...
               getCurveFromGUI(FCT_Parametric)); break;*/
            case (FCT_MinMaxCPoints):
                storeCurveValues(
                    parent->displayedCurve, getCurveFromGUI(FCT_MinMaxCPoints));
                break;
...
void FlatCurveEditorSubGroup::storeCurveValues(
...
/*void FlatCurveEditorSubGroup::updateBackgroundHistogram (CurveEditor* ce) {
    ...
    if (fce==displayedCurve) {
        paramCurve->updateBackgroundHistogram (fce->bgHistValid ? fce->histogram :
NULL); customCurve->updateBackgroundHistogram (fce->bgHistValid ? fce->histogram :
NULL); NURBSCurve->updateBackgroundHistogram (fce->bgHistValid ? fce->histogram : NULL);
    }
}*/
