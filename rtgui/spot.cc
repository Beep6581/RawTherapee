...
Spot::Spot() :
    ...
    EditSubscriber(ET_OBJECTS), draggedSide(DraggedSide::NONE), lastObject(-1),
    activeSpot(-1), sourceIcon("spot-normal", "spot-active", "spot-prelight", "", "",
                        Geometry::DP_CENTERCENTER),
    editedCheckBox(nullptr)
{
    countLabel =
        Gtk::manage(new Gtk::Label(Glib::ustring::compose(M("TP_SPOT_COUNTLABEL"), 0)));
    ...
    spotSize = Gtk::manage(new Adjuster(M("TP_SPOT_DEFAULT_SIZE"),
        SpotParams::minRadius, SpotParams::maxRadius, 1, 25));
...
Spot::~Spot()
{
    ...
    if (EditSubscriber::visibleGeometry.size()) {
        for (size_t i = 0;
            i < EditSubscriber::visibleGeometry.size() - VISIBLE_OBJECT_COUNT;
            ++i) { // static visible geometry at the end of the list
    ...
    if (batchMode) {
        ...
        if (listener) {
            listener->panelChanged(
                EvSpotEntry, Glib::ustring::compose(M("TP_SPOT_COUNTLABEL"), 0));
        }
    } else {
        if (!spots.empty()) {
...
void Spot::setBatchMode(bool batchMode)
{
    ...
    if (batchMode) {
        ...
        if (!editedCheckBox) {
            ...
            editedConn = editedCheckBox->signal_toggled().connect(
                sigc::mem_fun(*this, &Spot::editedToggled));
            editedCheckBox->show();
        }
    }
}
...
{
    if (listener) {
        listener->panelChanged(EvSpotEntry,
            !editedCheckBox->get_active()
                ? M("GENERAL_UNCHANGED")
                : Glib::ustring::compose(M("TP_SPOT_COUNTLABEL"), spots.size()));
    }
}
...
void Spot::enabledChanged()
{
    if (listener) {
        if (get_inconsistent()) {
            listener->panelChanged(
                edit->get_active() ? EvSpotEnabledOPA : EvSpotEnabled,
                M("GENERAL_UNCHANGED"));
        } else if (getEnabled()) {
            listener->panelChanged(
                edit->get_active() ? EvSpotEnabledOPA : EvSpotEnabled,
    ...
    if (listener) {
        if (edit->get_active()) {
            ...
            listener->refreshPreview(
                EvSpotEnabledOPA); // reprocess the preview w/o creating History entry
            subscribe();
        } else {
            releaseEdit();
            unsubscribe();
...
void Spot::createGeometry()
{
    ...
    EditSubscriber::mouseOverGeometry.at(i++) =
        &targetFeatherCircle; // MO_OBJECT_COUNT + 4
    assert(i == MO_SOURCE_FEATHER_CIRCLE);
    EditSubscriber::mouseOverGeometry.at(i++) =
        &sourceFeatherCircle; // MO_OBJECT_COUNT + 5
    ...
    for (; j < EditSubscriber::visibleGeometry.size() - VISIBLE_OBJECT_COUNT;
        ++i, ++j) {
        EditSubscriber::mouseOverGeometry.at(i) =
            EditSubscriber::visibleGeometry.at(j) = new OPIcon(normalImg, activeImg,
                prelightImg, nullptr, nullptr, Geometry::DP_CENTERCENTER);
    ...
    EditSubscriber::visibleGeometry.at(j++) =
        &sourceFeatherCircle; // VISIBLE_OBJECT_COUNT + 1
    ...
    EditSubscriber::visibleGeometry.at(j++) =
        &targetFeatherCircle; // VISIBLE_OBJECT_COUNT + 4
...
void Spot::addNewEntry()
{
    ...
    // TODO: find a way to disable the active spot's Mouse Over geometry but still
...
CursorShape Spot::getCursor(int objectID, int xPos, int yPos) const
{
    ...
    if (editProvider && activeSpot > -1) {
        ...
        if (objectID >= MO_TARGET_CIRCLE && objectID <= MO_SOURCE_FEATHER_CIRCLE) {
            Coord delta(Coord(xPos, yPos) - ((objectID == MO_SOURCE_CIRCLE ||
                                                 objectID == MO_SOURCE_FEATHER_CIRCLE)
...
bool Spot::mouseOver(int modifierKey)
{
    ...
    if (editProvider && editProvider->getObject() != lastObject) {
        ...
        if (editProvider->getObject() > -1) {
            ...
            if (editProvider->getObject() >= MO_OBJECT_COUNT) {
                ...
                if (activeSpot != oldActiveSpot) {
                    if (oldActiveSpot > -1) {
                        EditSubscriber::visibleGeometry.at(oldActiveSpot)->state =
                            Geometry::NORMAL;
                        EditSubscriber::mouseOverGeometry
                            .at(oldActiveSpot + MO_OBJECT_COUNT)
                            ->state = Geometry::NORMAL;
                    }

                    EditSubscriber::visibleGeometry.at(activeSpot)->state =
                        Geometry::PRELIGHT;
                    EditSubscriber::mouseOverGeometry.at(activeSpot + MO_OBJECT_COUNT)
        ...
        if (lastObject > -1 &&
            EditSubscriber::mouseOverGeometry.at(lastObject) == getActiveSpotIcon()) {
...
// Create a new Target and Source point or start the drag of a Target point under the
// cursor
bool Spot::button1Pressed(int modifierKey)
{
    ...
    if (editProvider) {
        if (lastObject == -1 && (modifierKey & GDK_CONTROL_MASK)) {
            ...
            if (startPos.x < 0 || startPos.y < 0 || startPos.x > imW ||
                startPos.y > imH) {
            ...
            draggedSide = lastObject == MO_TARGET_DISK   ? DraggedSide::TARGET
                          : lastObject == MO_SOURCE_DISC ? DraggedSide::SOURCE
                                                         : DraggedSide::NONE;
...
bool Spot::drag1(int modifierKey)
{
    ...
    if (loGeom == &sourceMODisc) {
        ...
        EditSubscriber::mouseOverGeometry.at(activeSpot + MO_OBJECT_COUNT)->state =
            Geometry::DRAGGED;
        ...
        spots.at(activeSpot).radius = LIM<int>(
            int(currPolar.radius), SpotParams::minRadius, SpotParams::maxRadius);
        ...
        rtengine::Coord currPos = editProvider->posImage + editProvider->deltaImage;
        rtengine::PolarCoord currPolar(currPos - spots.at(activeSpot).sourcePos);
        spots.at(activeSpot).feather =
            LIM01<float>((currPolar.radius - double(spots.at(activeSpot).radius)) /
...
void Spot::tweakParams(procparams::ProcParams &pparams)
{
    ...
    // pparams.toneCurve.hrenabled = false;  // not sure for this one, it could be
    // useful for ExpComp w/o performance penalty
