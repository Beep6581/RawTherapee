...
WhiteBalance::WhiteBalance () : FoldableToolPanel(this, TOOL_NAME, M("TP_WBALANCE_LABEL"), true, true), wbp(nullptr), wblistener(nullptr)
{
    ...
    itcwb_sorted = Gtk::manage (new Gtk::CheckButton (M("TP_WBALANCE_ITCWB_SORTEDANC")));
    itcwb_sorted->set_tooltip_markup (M("TP_WBALANCE_ITCWBSORTEDANC_TOOLTIP"));
...
void WhiteBalance::WBChanged(double temperature, double greenVal, double rw, double gw, double bw, int dread, float studgood, float minchrom, int kmin, float histmin, float histmax)
{
    idle_register.add(
        [this, temperature, greenVal, rw, gw, bw, dread, studgood, minchrom, kmin, histmin, histmax]() -> bool
        {
            disableListener();
