    ...
    wbp(nullptr), wblistener(nullptr)
{
    ...
    setExpandAlignProperties(
        resetButton, false, false, Gtk::ALIGN_START, Gtk::ALIGN_CENTER);
    ...
    setExpandAlignProperties(
        wbsizehelper, false, false, Gtk::ALIGN_START, Gtk::ALIGN_CENTER);
    ...
    temp = Gtk::manage(new Adjuster(M("TP_WBALANCE_TEMPERATURE"), MINTEMP, MAXTEMP, 5,
        CENTERTEMP, itempL, itempR, &wbSlider2Temp, &wbTemp2Slider));
    green = Gtk::manage(new Adjuster(
        M("TP_WBALANCE_GREEN"), MINGREEN, MAXGREEN, 0.001, 1.0, igreenL, igreenR));
    equal = Gtk::manage(new Adjuster(M("TP_WBALANCE_EQBLUERED"), MINEQUAL, MAXEQUAL,
        0.001, 1.0, iblueredL, iblueredR));
    tempBias = Gtk::manage(new Adjuster(
        M("TP_WBALANCE_TEMPBIAS"), -1.1, 1.1, 0.005, 0.0, itempbiasL, itempbiasR));
    ...
    methconn = method->signal_changed().connect(
        sigc::mem_fun(*this, &WhiteBalance::optChanged));
...
void WhiteBalance::adjusterChanged(Adjuster *a, double newval)
{
    ...
    if (!ppMethod.first || (ppMethod.second.ppLabel != wbCustom.second.ppLabel &&
    ...
    if (listener && getEnabled()) {
        if (a == temp) {
            ...
            listener->panelChanged(
                EvWBitcwgreen, Glib::ustring::format(std::setw(4), std::fixed,
                                   std::setprecision(2), a->getValue()));
        }
    }
}
...
void WhiteBalance::checkBoxToggled(CheckBox *c, CheckValue newval)
{
    ...
    if (c == observer10) {
        ...
        listener->panelChanged(EvWBObserver10,
    ...
    if (opt != row[methodColumns.colId]) {
        ...
        if (row[methodColumns.colLabel] == M("GENERAL_UNCHANGED")) {
            ...
            unsigned int methodId =
                findWBEntryId(row[methodColumns.colLabel], WBLT_GUI);
            const WBEntry &currMethod = WBParams::getWbEntries()[methodId];
            ...
            switch (currMethod.type) {
                ...
                default:
                    ...
                    temp->setValue(
                        temp->getAddMode() ? 0.0 : (double)(currMethod.temperature));
                    green->setValue(
                        green->getAddMode() ? 0.0 : (double)(currMethod.green));
                    equal->setValue(
                        equal->getAddMode() ? 0.0 : (double)(currMethod.equal));
...
void WhiteBalance::read(const ProcParams *pp, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        // By default, temperature and green are said "UnEdited", but it may change
        // later
        temp->setEditedState(UnEdited);
        green->setEditedState(UnEdited);
        ...
        compatVersionAdjuster->setEditedState(
            pedited->wb.compat_version ? Edited : UnEdited);
    }
    ...
    if (pedited && !pedited->wb.method) {
        ...
        switch (wbValues.type) {
            ...
            case WBEntry::Type::AUTO:
                ...
                // set default values first if in ADD mode, otherwise keep the current
                // ones
                if (temp->getAddMode()) {
                    temp->setValue(0.0);
                }
            ...
            */
            default:
                ...
                // The user may have changed the green value even for predefined WB
                // values
                if (pedited) {
                    green->setEditedState(pedited->wb.green ? Edited : UnEdited);
                    equal->setEditedState(pedited->wb.equal ? Edited : UnEdited);
...
void WhiteBalance::write(ProcParams *pp, ParamsEdited *pedited)
{
    ...
    if (pedited) {
        ...
        pedited->wb.itcwb_prim =
            itcwb_prim->get_active_text() != M("GENERAL_UNCHANGED");
...
int WhiteBalance::getSize() { return atoi(spotsize->get_active_text().c_str()); }
...
void WhiteBalance::setWB(int vtemp, double vgreen)
{
    ...
    if (listener) {
        listener->panelChanged(
            EvWBTemp, Glib::ustring::compose("%1, %2", (int)temp->getValue(),
                          Glib::ustring::format(std::setw(4), std::fixed,
                              std::setprecision(3), green->getValue())));
    }
}

void WhiteBalance::resetWB() { setActiveMethod(M("TP_WBALANCE_CAMERA")); }

void WhiteBalance::setAdjusterBehavior(
    bool tempadd, bool greenadd, bool equaladd, bool tempbiasadd)
{
...
inline void WhiteBalance::cache_customTemp(int temp) { custom_temp = temp; }

void WhiteBalance::cache_customGreen(double green) { custom_green = green; }
void WhiteBalance::cache_customEqual(double equal) { custom_equal = equal; }
...
unsigned int WhiteBalance::findWBEntryId(
    const Glib::ustring &label, enum WB_LabelType lblType)
{
    for (unsigned int i = 0; i < WBParams::getWbEntries().size(); i++) {
        if (label == (lblType == WBLT_GUI ? WBParams::getWbEntries()[i].GUILabel
...
std::pair<bool, const WBEntry &> WhiteBalance::findWBEntry(
    const Glib::ustring &label, enum WB_LabelType lblType)
{
    for (unsigned int i = 0; i < WBParams::getWbEntries().size(); ++i) {
        if (label == (lblType == WBLT_GUI ? WBParams::getWbEntries()[i].GUILabel
...
int WhiteBalance::_setActiveMethod(
    Glib::ustring &label, Gtk::TreeModel::Children &children)
{
    int found = -1;
...
inline Gtk::TreeRow WhiteBalance::getActiveMethod() { return *(method->get_active()); }

void WhiteBalance::WBChanged(int met, double temperature, double greenVal, double rw,
    double gw, double bw, float temp0, float delta, int bia, int dread, float studgood,
    float minchrom, int kmin, float histmin, float histmax, AWBMode aWBMode)
{
    idle_register.add(
        [this, met, temperature, greenVal, rw, gw, bw, temp0, delta, bia, dread,
            studgood, minchrom, kmin, histmin, histmax, aWBMode]() -> bool {
            disableListener();
            ...
            int bia2 = bia;
            mulLabel->set_text(Glib::ustring::compose(M("TP_WBALANCE_MULLABEL"),
            ...
            if (bia == 3) {
                bia2 = bia - 1;
                StudLabel->set_text(Glib::ustring::compose(M("TP_WBALANCE_STUDLABEL"),
                ...
                StudLabel->set_text(Glib::ustring::compose(M("TP_WBALANCE_STUDLABEL1"),
                    Glib::ustring::format(std::fixed, std::setprecision(4), stud),
                    Glib::ustring::format(std::fixed, std::setprecision(0), bia),
                    Glib::ustring::format(std::fixed, std::setprecision(0), temp0)));
            } else {
                StudLabel->set_text(Glib::ustring::compose(M("TP_WBALANCE_STUDLABEL0"),
                    Glib::ustring::format(std::fixed, std::setprecision(4), stud),
                    Glib::ustring::format(std::fixed, std::setprecision(0), bia),
                    Glib::ustring::format(std::fixed, std::setprecision(0), temp0)));
            }
            PatchLabel->set_text(Glib::ustring::compose(M("TP_WBALANCE_PATCHLABEL"),
