...
LensGeometry::LensGeometry() :
    FoldableToolPanel(this, TOOL_NAME, M("TP_LENSGEOM_LABEL")), rlistener(nullptr),
    lastFill(false)
{
    ...
    hb1->pack_start(
        *Gtk::manage(new Gtk::Label(M("TP_RAW_DMETHOD") + ": ")), Gtk::PACK_SHRINK, 4);
    ...
    autoCrop->set_image(
        *Gtk::manage(new RTImage("crop-auto-small", Gtk::ICON_SIZE_BUTTON)));
    autoCrop->get_style_context()->add_class("independent");
    pack_start(*autoCrop, Gtk::PACK_SHRINK, 2);

    method->connect(method->signal_changed().connect(
        sigc::mem_fun(*this, &LensGeometry::methodChanged)));
    autoCrop->signal_pressed().connect(
        sigc::mem_fun(*this, &LensGeometry::autoCropPressed));
    fillConn = fill->signal_toggled().connect(
        sigc::mem_fun(*this, &LensGeometry::fillPressed));
...
LensGeometry::~LensGeometry() { idle_register.destroy(); }
    ...
    if (listener) {
        if (a == scale) {
            listener->panelChanged(
                EvTransScale, Glib::ustring::format(scale->getValue()));
        } else {
            if (settings->verbose) {
                std::cout
                    << "Unknown adjuster given in LensGeometry::adjusterChanged, file "
