...
RenameDialog::RenameDialog(Gtk::Window *parent) :
    Gtk::Dialog(M("FILEBROWSER_RENAMEDLGLABEL"), *parent, true), p(parent),
    imageData(nullptr)
{
    ...
    //    templates->set_row_separator_func (sigc::mem_fun(*this,
    //    &RenameDialog::rowSeparatorFunc));
    //    templates->signal_changed().connect(sigc::mem_fun(*this,
    //    &RenameDialog::tmplSelectionChanged)); useTmpl->signal_toggled().connect(
    //    sigc::mem_fun(*this, &RenameDialog::useTemplToggled) );
...
Glib::ustring RenameDialog::getNewName() { return newName->get_text(); }
