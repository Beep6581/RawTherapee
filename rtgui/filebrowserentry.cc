...
FileBrowserEntry::FileBrowserEntry(Thumbnail *thm, const Glib::ustring &fname) :
    ThumbBrowserEntryBase(fname, thm), wasInside(false), iatlistener(nullptr),
    press_x(0), press_y(0), action_x(0), action_y(0), rot_deg(0.0), landscape(true),
    cropParams(new rtengine::procparams::CropParams), cropgl(nullptr), state(SNormal),
...
{
    editedIcon = std::shared_ptr<RTSurface>(
        new RTSurface("tick-small", Gtk::ICON_SIZE_SMALL_TOOLBAR));
    recentlySavedIcon = std::shared_ptr<RTSurface>(
        new RTSurface("save-small", Gtk::ICON_SIZE_SMALL_TOOLBAR));
    enqueuedIcon = std::shared_ptr<RTSurface>(
        new RTSurface("gears-small", Gtk::ICON_SIZE_SMALL_TOOLBAR));
    hdr = std::shared_ptr<RTSurface>(
        new RTSurface("filetype-hdr", Gtk::ICON_SIZE_SMALL_TOOLBAR));
    ps = std::shared_ptr<RTSurface>(
        new RTSurface("filetype-ps", Gtk::ICON_SIZE_SMALL_TOOLBAR));
}
...
void FileBrowserEntry::refreshThumbnailImage() { refreshThumbnailImage(false); }
...
void FileBrowserEntry::refreshQuickThumbnailImage()
{
    ...
    bool upgrade_to_processed =
        (!options.internalThumbIfUntouched || thumbnail->isPParamsValid());
...
void FileBrowserEntry::calcThumbnailSize()
{
    if (thumbnail) {
        ...
        if (ow != prew || oh != preh ||
            preview.size() != static_cast<std::size_t>(prew * preh * 3)) {
            preview.clear();
        }
    }
}
...
{
    if (scale != 1.0 &&
        cropParams->enabled) { // somewhere in pipeline customBackBufferUpdate is called
                               // when scale == 1.0, which is nonsense for a thumb
        if (state == SCropSelecting || state == SResizeH1 || state == SResizeH2 ||
...
void FileBrowserEntry::procParamsChanged(
...
void FileBrowserEntry::updateImage(rtengine::IImage8 *img, double scale,
...
void FileBrowserEntry::_updateImage(rtengine::IImage8 *img, double s,
    const rtengine::procparams::CropParams &cropParams)
{
...
bool FileBrowserEntry::motionNotify(int x, int y)
{
    ...
    if (inspector && inspector->isActive() &&
        (!parent->isInTabMode() || options.inspectorWindow)) {
    ...
    if (state == SRotateSelecting) {
        ...
        cropgl->cropHeight1Resized(cropParams->x, cropParams->y, cropParams->w,
            cropParams->h, crop_custom_ratio);
        {
        ...
        cropgl->cropHeight2Resized(cropParams->x, cropParams->y, cropParams->w,
            cropParams->h, crop_custom_ratio);
        {
            MYREADERLOCK(l, lockRW);
            updateBackBuffer();
        ...
        cropgl->cropWidth1Resized(cropParams->x, cropParams->y, cropParams->w,
            cropParams->h, crop_custom_ratio);
        {
            MYREADERLOCK(l, lockRW);
            updateBackBuffer();
        ...
        cropgl->cropWidth2Resized(cropParams->x, cropParams->y, cropParams->w,
            cropParams->h, crop_custom_ratio);
        {
            MYREADERLOCK(l, lockRW);
            updateBackBuffer();
        ...
        cropParams->y = action_y + (y - press_y) / scale;
        cropParams->h += oy - cropParams->y;
        cropgl->cropTopLeftResized(cropParams->x, cropParams->y, cropParams->w,
            cropParams->h, crop_custom_ratio);
        {
            MYREADERLOCK(l, lockRW);
            updateBackBuffer();
        ...
        cropParams->y = action_y + (y - press_y) / scale;
        cropParams->h += oy - cropParams->y;
        cropgl->cropTopRightResized(cropParams->x, cropParams->y, cropParams->w,
            cropParams->h, crop_custom_ratio);
        {
            MYREADERLOCK(l, lockRW);
            updateBackBuffer();
        ...
        cropParams->w += ox - cropParams->x;
        cropParams->h = action_y + (y - press_y) / scale;
        cropgl->cropBottomLeftResized(cropParams->x, cropParams->y, cropParams->w,
            cropParams->h, crop_custom_ratio);
        {
            MYREADERLOCK(l, lockRW);
            updateBackBuffer();
        ...
        cropParams->w = action_x + (x - press_x) / scale;
        cropParams->h = action_y + (y - press_y) / scale;
        cropgl->cropBottomRightResized(cropParams->x, cropParams->y, cropParams->w,
            cropParams->h, crop_custom_ratio);
        {
            MYREADERLOCK(l, lockRW);
            updateBackBuffer();
            ...
            if (onArea(CropTopLeft, ix, iy)) {
                ...
                if (tm == TMStraighten) {
                    ...
                    iatlistener->spotWBselected(
                        (ix - prex) / scale, (iy - prey) / scale, thumbnail);
                    b = true;
                    ...
                    if (cropgl) {
                        ...
                        cropgl->cropInit(
                            cropParams->x, cropParams->y, cropParams->w, cropParams->h);
                        b = true;
                    }
                }
            }
        ...
        if (state == SRotateSelecting) {
            ...
            if (iatlistener->getToolBar()) {
                iatlistener->getToolBar()->setTool(TMHand);
                   ...
                   (state == SCropSelecting || state == SResizeH1 ||
...
bool FileBrowserEntry::onArea(CursorArea a, int x, int y)
{
    ...
    switch (a) {
        ...
        case CropTopLeft:
            return cropParams->enabled && y1 >= cropParams->y - cropResizeBorder &&
        ...
        case CropTop:
            return cropParams->enabled && x1 > cropParams->x + cropResizeBorder &&
                   x1 < cropParams->x + cropParams->w - 1 - cropResizeBorder &&
                   y1 > cropParams->y - cropResizeBorder &&
                   y1 < cropParams->y + cropResizeBorder;
        ...
        case CropInside:
            return cropParams->enabled && y1 > cropParams->y &&
                   y1 < cropParams->y + cropParams->h - 1 && x1 > cropParams->x &&
...
void FileBrowserEntry::drawStraightenGuide(Cairo::RefPtr<Cairo::Context> cr)
{
    ...
    Glib::RefPtr<Pango::Context> context =
        parent->getDrawingArea()->get_pango_context();
    Pango::FontDescription fontd =
        parent->getDrawingArea()->get_style_context()->get_font();
    ...
    Glib::RefPtr<Pango::Layout> deglayout =
        parent->getDrawingArea()->create_pango_layout(Glib::ustring::compose(
...

    {
        ...
        if (x2 < prex + ofsX + startx) {
            ...
            y2 = y1 - (double)(y1 - y2) * (x1 - (prew + prex + ofsX + startx - 1)) /
                          (x1 - x2);
        ...
        if (y2 < prey + ofsY + starty) {
            ...
            x2 = x1 - (double)(x1 - x2) * (y1 - (preh + prey + ofsY + starty - 1)) /
                          (y1 - y2);
