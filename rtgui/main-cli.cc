...
// Set this to 1 to make RT work when started with Eclipse and arguments, at least on
// Windows platform
...
int main(int argc, char **argv)
{
    ...
    if (options.is_defProfRawMissing()) {
        ...
        std::cerr
            ...
            << std::endl
            << "\"" << DEFPROFILE_RAW << "\" will be used instead." << std::endl
            << std::endl;
    }
    if (options.is_bundledDefProfRawMissing()) {
        std::cerr << std::endl
                  << "The bundled profile \"" << options.defProfRaw
                  << "\" could not be found!" << std::endl
                  << "Your installation could be damaged." << std::endl
                  << "Default internal values will be used instead." << std::endl
                  << std::endl;
    ...
    if (options.is_defProfImgMissing()) {
        ...
        std::cerr
            << std::endl
            << "The default profile for non-raw photos could not be found or is not "
               "set."
            << std::endl
            << "Please check your profiles' directory, it may be missing or damaged."
            << std::endl
            << "\"" << DEFPROFILE_IMG << "\" will be used instead." << std::endl
            << std::endl;
    }
    if (options.is_bundledDefProfImgMissing()) {
        std::cerr << std::endl
                  << "The bundled profile " << options.defProfImg
                  << " could not be found!" << std::endl
                  << "Your installation could be damaged." << std::endl
                  << "Default internal values will be used instead." << std::endl
                  << std::endl;
    ...
    if (Glib::file_test(
            Glib::build_filename(options.rtdir, "cache"), Glib::FILE_TEST_IS_DIR) &&
        !Glib::file_test(options.cacheBaseDir, Glib::FILE_TEST_IS_DIR)) {
        if (g_rename(Glib::build_filename(options.rtdir, "cache").c_str(),
                options.cacheBaseDir.c_str()) == -1) {
            std::cout << "g_rename "
                      << Glib::build_filename(options.rtdir, "cache").c_str() << " => "
                      << options.cacheBaseDir.c_str() << " failed." << std::endl;
        }
    }
    ...
    // printing RT's version in all case, particularly useful for the 'verbose' mode,
    // but also for the batch processing
...
#endif
        if (currParam.length() > 1 && currParam.at(0) == '-' &&
            currParam.at(1) == 'q') {
            return true;
        }
    }
...
int processLineParams(int argc, char **argv)
{
    ...
    for (int iArg = 1; iArg < argc; iArg++) {
        ...
        if (currParam.at(0) == '-' && currParam.size() > 1) {
            switch (currParam.at(1)) {
                ...
                case 'o': // outputfile or dir
                    if (iArg + 1 < argc) {
                        ...
                        if (outputPath.substr(0, 9) == "/dev/null") {
                            outputPath.assign(
                                "/dev/null"); // removing any useless chars or filename
                            outputDirectory = false;
                            leaveUntouched = true;
                        } else if (Glib::file_test(
                                       outputPath, Glib::FILE_TEST_IS_DIR)) {
                ...
                case 'p': // processing parameters for all inputs; all set procparams
                          // are required, so
                    ...
                    if (iArg + 1 < argc) {
                        ...
                        if (fname.at(0) == '-') {
                            std::cerr
                                << "Error: filename missing next to the -p switch."
                                << std::endl;
                            deleteProcParams(processingParams);
                            return -3;
                        }

                        rtengine::procparams::PartialProfile *currentParams =
                            new rtengine::procparams::PartialProfile(true);

                        if (!(currentParams->load(fname))) {
                            processingParams.push_back(currentParams);
                        } else {
                            std::cerr << "Error: \"" << fname << "\" not found."
                                      << std::endl;
                            deleteProcParams(processingParams);
                            return -3;
                        }
                ...
                case 'j':
                    if (currParam.length() > 2 && currParam.at(2) == 's') {
                        if (currParam.length() == 3) {
                            std::cerr
                                << "Error: the -js switch requires a mandatory value!"
                                << std::endl;
                            deleteProcParams(processingParams);
                            return -3;
                        }
                        ...
                        if (subsampling < 1 || subsampling > 3) {
                            std::cerr << "Error: the value accompanying the -js switch "
                                         "has to be in the [1-3] range!"
                                      << std::endl;
                            deleteProcParams(processingParams);
                            return -3;
                        }
                        ...
                        if (currParam.size() < 3) {
                            ...
                            if (compression < 0 || compression > 100) {
                                std::cerr << "Error: the value accompanying the -j "
                                             "switch has to be in the [0-100] range!"
                                          << std::endl;
                                deleteProcParams(processingParams);
                                return -3;
                            }
                ...
                case 'b':
                    ...
                    if (currParam.length() >= 3 && currParam.at(2) == '8') { // -b8
                        bits = 8;
                    } else if (currParam.length() >= 4 && currParam.length() <= 5 &&
                               currParam.at(2) == '1' &&
                               currParam.at(3) == '6') { // -b16, -b16f
                        bits = 16;
                        if (currParam.length() == 5 && currParam.at(4) == 'f') {
                            isFloat = true;
                               ...
                               currParam.at(2) == '3' &&
                               currParam.at(3) == '2') { // -b32 == -b32f
                        bits = 32;
                        isFloat = true;
                    }

                    if (bits != 8 && bits != 16 && bits != 32) {
                        std::cerr << "Error: specify output bit depth per channel as "
                                     "-b8 for 8-bit integer, -b16 for 16-bit integer, "
                                     "-b16f for 16-bit float or -b32 for 32-bit float."
                                  << std::endl;
                        deleteProcParams(processingParams);
                        return -3;
                    }
                ...
                case 't':
                    outputType = "tif";
                    compression =
                        ((currParam.size() < 3 || currParam.at(2) != 'z') ? 0 : 1);
                    break;
                ...
                case 'c': // MUST be last option
                    while (iArg + 1 < argc) {
                        ...
                        if (Glib::file_test(argument, Glib::FILE_TEST_IS_REGULAR)) {
                            bool notAll =
                                allExtensions && !options.is_parse_extention(argument);
                            bool notRetained =
                                !allExtensions &&
                            ...
                            try {
                                ...
                                while (auto file = enumerator->next_file()) {
                                    ...
                                    const auto fileName = Glib::build_filename(
                                        argument, file->get_name());
                                    bool isDir = file->get_file_type() ==
                                                 Gio::FILE_TYPE_DIRECTORY;
                                    bool notAll = allExtensions &&
                                                  !options.is_parse_extention(fileName);
                                    bool notRetained =
                                        !allExtensions &&
                                        !options.has_retained_extention(fileName);

                                    if (isDir || notAll || notRetained) {
                                        if (isDir) {
                                            std::cout
                                                << "\"" << fileName
                                                << "\"  is a folder. Folder skipped"
                                                << std::endl;
                                        } else if (notAll) {
                                            std::cout << "\"" << fileName
                                                      << "\"  is not one of the parsed "
                                    ...
                                    if (sideProcParams && skipIfNoSidecar) {
                                        ...
                                        if (!Glib::file_test(
                                                fileName + paramFileExtension,
                                                Glib::FILE_TEST_EXISTS)) {
                                            std::cout << "\"" << fileName
                                                      << "\"  has no side-car file. "
                                                         "Image skipped."
                                                      << std::endl;
                                            continue;
                                        }
                ...
                default: {
                    ...
                    std::cout << "  An advanced, cross-platform program for developing "
                                 "raw photos."
                              << std::endl;
                    std::cout << std::endl;
                    ...
                    std::cout << "  <Chevrons> indicate parameters you can change."
                              << std::endl;
                    std::cout << "  [Square brackets] mean the parameter is optional."
                              << std::endl;
                    std::cout
                        << "  The pipe symbol | indicates a choice of one or the other."
                        << std::endl;
                    std::cout << "  The dash symbol - denotes a range of possible "
                                 "values from one to the other."
                              << std::endl;
                    std::cout << std::endl;
                    std::cout << "Usage:" << std::endl;
                    std::cout << "  " << Glib::path_get_basename(argv[0])
                              << " -c <dir>|<files>   Convert files in batch with "
                                 "default parameters."
                              << std::endl;
                    std::cout << "  " << Glib::path_get_basename(argv[0])
                              << " <other options> -c <dir>|<files>   Convert files in "
                                 "batch with your own settings."
                              << std::endl;
                    std::cout << std::endl;
                    std::cout << "Options:" << std::endl;
                    std::cout
                        << "  " << Glib::path_get_basename(argv[0])
                        << "[-o <output>|-O <output>] [-q] [-a] [-s|-S] [-p <one.pp3> "
                           "[-p <two.pp3> ...] ] [-d] [ -j[1-100] -js<1-3> | -t[z] "
                           "-b<8|16|16f|32> | -n -b<8|16> ] [-Y] [-f] -c <input>"
                        << std::endl;
                    std::cout << std::endl;
                    std::cout << "  -c <files>       Specify one or more input files "
                                 "or folders."
                              << std::endl;
                    std::cout
                        << "                   When specifying folders, Rawtherapee "
                           "will look for image file types which comply"
                        << std::endl;
                    std::cout << "                   with the selected extensions (see "
                                 "also '-a')."
                              << std::endl;
                    std::cout << "                   -c must be the last option."
                              << std::endl;
                    std::cout << "  -o <file>|<dir>  Set output file or folder."
                              << std::endl;
                    std::cout << "                   Saves output file alongside input "
                                 "file if -o is not specified."
                              << std::endl;
                    std::cout
                        << "  -O <file>|<dir>  Set output file or folder and copy "
                        << pparamsExt << " file into it." << std::endl;
                    std::cout << "                   Saves output file alongside input "
                                 "file if -O is not specified."
                              << std::endl;
                    std::cout << "  -q               Quick-start mode. Does not load "
                                 "cached files to speedup start time."
                              << std::endl;
                    std::cout << "  -a               Process all supported image file "
                                 "types when specifying a folder, even those"
                              << std::endl;
                    std::cout << "                   not currently selected in "
                                 "Preferences > File Browser > Parsed Extensions."
                              << std::endl;
                    std::cout << "  -s               Use the existing sidecar file to "
                                 "build the processing parameters,"
                              << std::endl;
                    std::cout << "                   e.g. for photo.raw there should "
                                 "be a photo.raw."
                              << pparamsExt << " file in the same folder." << std::endl;
                    std::cout << "                   If the sidecar file does not "
                                 "exist, neutral values will be used."
                              << std::endl;
                    std::cout << "  -S               Like -s but skip if the sidecar "
                                 "file does not exist."
                              << std::endl;
                    std::cout << "  -p <file.pp3>    Specify processing profile to be "
                                 "used for all conversions."
                              << std::endl;
                    std::cout << "                   You can specify as many sets of "
                                 "\"-p <file.pp3>\" options as you like,"
                              << std::endl;
                    std::cout << "                   each will be built on top of the "
                                 "previous one, as explained below."
                              << std::endl;
                    std::cout << "  -d               Use the default raw or non-raw "
                                 "processing profile as set in"
                              << std::endl;
                    std::cout << "                   Preferences > Image Processing > "
                                 "Default Processing Profile"
                              << std::endl;
                    std::cout << "  -j[1-100]        Specify output to be JPEG "
                                 "(default, if -t and -n are not set)."
                              << std::endl;
                    std::cout << "                   Optionally, specify compression "
                                 "1-100 (default value: 92)."
                              << std::endl;
                    std::cout << "  -js<1-3>         Specify the JPEG chroma "
                                 "subsampling parameter, where:"
                              << std::endl;
                    std::cout << "                   1 = Best compression:   2x2, 1x1, "
                                 "1x1 (4:2:0)"
                              << std::endl;
                    std::cout << "                       Chroma halved vertically and "
                                 "horizontally."
                              << std::endl;
                    std::cout << "                   2 = Balanced (default): 2x1, 1x1, "
                                 "1x1 (4:2:2)"
                              << std::endl;
                    std::cout << "                       Chroma halved horizontally."
                              << std::endl;
                    std::cout << "                   3 = Best quality:       1x1, 1x1, "
                                 "1x1 (4:4:4)"
                              << std::endl;
                    std::cout << "                       No chroma subsampling."
                              << std::endl;
                    std::cout << "  -b<8|16|16f|32>  Specify bit depth per channel."
                              << std::endl;
                    std::cout << "                   8   = 8-bit integer.  Applies to "
                                 "JPEG, PNG and TIFF. Default for JPEG and PNG."
                              << std::endl;
                    std::cout << "                   16  = 16-bit integer. Applies to "
                                 "TIFF and PNG. Default for TIFF."
                              << std::endl;
                    std::cout
                    ...
                    std::cout
                        ...
                        << std::endl;
                    std::cout << "  -t[z]            Specify output to be TIFF."
                              << std::endl;
                    std::cout << "                   Uncompressed by default, or "
                                 "deflate compression with 'z'."
                              << std::endl;
                    std::cout
                        << "  -n               Specify output to be compressed PNG."
                        << std::endl;
                    std::cout << "                   Compression is hard-coded to "
                                 "PNG_FILTER_PAETH, Z_RLE."
                              << std::endl;
                    std::cout << "  -Y               Overwrite output if present."
                              << std::endl;
                    std::cout << "  -f               Use the custom fast-export "
                                 "processing pipeline."
                              << std::endl;
                    std::cout << std::endl;
                    std::cout << "Your " << pparamsExt
                              << " files can be incomplete, RawTherapee will build the "
                                 "final values as follows:"
                              << std::endl;
                    std::cout << "  1- A new processing profile is created using "
                                 "neutral values,"
                              << std::endl;
                    std::cout << "  2- If the \"-d\" option is set, the values are "
                                 "overridden by those found in"
                              << std::endl;
                    std::cout << "     the default raw or non-raw processing profile."
                              << std::endl;
                    std::cout << "  3- If one or more \"-p\" options are set, the "
                                 "values are overridden by those"
                              << std::endl;
                    std::cout << "     found in these processing profiles."
                              << std::endl;
                    std::cout << "  4- If the \"-s\" or \"-S\" options are set, the "
                                 "values are finally overridden by those"
                              << std::endl;
                    std::cout << "     found in the sidecar files." << std::endl;
                    std::cout << "  The processing profiles are processed in the order "
    ...
    if (useDefault) {
        ...
        if (options.is_defProfRawMissing() || profPath.empty() ||
            (profPath != DEFPROFILE_DYNAMIC &&
                rawParams->load(profPath == DEFPROFILE_INTERNAL
                                    ? DEFPROFILE_INTERNAL
                                    : Glib::build_filename(profPath,
                                          Glib::path_get_basename(options.defProfRaw) +
                                              paramFileExtension)))) {
            std::cerr << "Error: default raw processing profile not found."
                      << std::endl;
        ...
        if (options.is_defProfImgMissing() || profPath.empty() ||
            (profPath != DEFPROFILE_DYNAMIC &&
                imgParams->load(profPath == DEFPROFILE_INTERNAL
                                    ? DEFPROFILE_INTERNAL
                                    : Glib::build_filename(profPath,
                                          Glib::path_get_basename(options.defProfImg) +
                                              paramFileExtension)))) {
            std::cerr << "Error: default non-raw processing profile not found."
                      << std::endl;
    ...
    for (size_t iFile = 0; iFile < inputFiles.size(); iFile++) {
        ...
        // Has to be reinstanciated at each profile to have a ProcParams object with
        // default values
        rtengine::procparams::ProcParams currentParams;

        Glib::ustring inputFile = inputFiles[iFile];
        std::cout << "Output is " << bits << "-bit "
                  << (isFloat ? "floating-point" : "integer") << "." << std::endl;
        ...
        if (outputPath.empty()) {
            ...
            Glib::ustring::size_type ext = s.find_last_of('.');
            outputFile =
                Glib::build_filename(outputPath, s.substr(0, ext) + "." + outputType);
        } else {
            if (leaveUntouched) {
        ...
        if (!overwriteFiles && Glib::file_test(outputFile, Glib::FILE_TEST_EXISTS)) {
            std::cerr << outputFile
                      << " already exists: use -Y option to overwrite. This image has "
                         "been skipped."
                      << std::endl;
            continue;
        }
        ...
        if (ext.lowercase() == "jpg" || ext.lowercase() == "jpeg" ||
            ext.lowercase() == "tif" || ext.lowercase() == "tiff" ||
            ext.lowercase() == "png") {
            isRaw = false;
        }
        ...
        if (useDefault) {
            if (isRaw) {
                ...
                std::cout << "  Merging default non-raw processing profile."
                          << std::endl;
        ...
        do {
            if (sideProcParams && i == sideCarFilePos) {
                ...
                // the "load" method don't reset the procparams values anymore, so
                // values found in the procparam file override the one of currentParams
                if (!Glib::file_test(sideProcessingParams, Glib::FILE_TEST_EXISTS) ||
                    currentParams.load(sideProcessingParams)) {
                    std::cerr << "Warning: sidecar file requested but not found for: "
                              << sideProcessingParams << std::endl;
                } else {
                    sideCarFound = true;
        ...
        if (sideProcParams && !sideCarFound && skipIfNoSidecar) {
            delete ii;
            errors++;
            std::cerr << "Error: no sidecar procparams found for: " << inputFile
                      << std::endl;
            continue;
        }
        ...
        rtengine::IImagefloat *resultImage =
            rtengine::processImage(job, errorCode, nullptr);
        ...
        if (outputType == "jpg") {
            ...
            errorCode =
                resultImage->saveAsTIFF(outputFile, bits, isFloat, compression == 0);
        } else if (outputType == "png") {
