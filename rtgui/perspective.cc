    ...
    FoldableToolPanel(this, TOOL_NAME, M("TP_PERSPECTIVE_LABEL"))
{
    ...
    RTImage *const ipers_apply =
        Gtk::manage(new RTImage("tick", Gtk::ICON_SIZE_BUTTON));
    ...
    vert = Gtk::manage(new Adjuster(
        M("TP_PERSPECTIVE_VERTICAL"), -100, 100, 0.1, 0, ipersVL, ipersVR));
    vert->setAdjusterListener(this);

    horiz = Gtk::manage(new Adjuster(
        M("TP_PERSPECTIVE_HORIZONTAL"), -100, 100, 0.1, 0, ipersHL, ipersHR));
    ...
    camera_focal_length =
        Gtk::manage(new Adjuster(M("TP_PERSPECTIVE_CAMERA_FOCAL_LENGTH"), 0.5, 2000,
            0.01, PerspectiveParams::DEFAULT_CAMERA_FOCAL_LENGTH));
    ...
    camera_pitch = Gtk::manage(new Adjuster(M("TP_PERSPECTIVE_CAMERA_PITCH"), -60, 60,
        0.1, 0, ipers_cam_pitch_left, ipers_cam_pitch_right));
    camera_pitch->setAdjusterListener(this);

    camera_yaw = Gtk::manage(new Adjuster(M("TP_PERSPECTIVE_CAMERA_YAW"), -60, 60, 0.1,
        0, ipers_cam_yaw_left, ipers_cam_yaw_right));
    ...
    Gtk::Frame *pca_frame = Gtk::manage(
        new Gtk::Frame(M("TP_PERSPECTIVE_POST_CORRECTION_ADJUSTMENT_FRAME")));
    ...
    projection_shift_vert = Gtk::manage(new Adjuster(
        M("TP_PERSPECTIVE_PROJECTION_SHIFT_VERTICAL"), -100, 100, 0.01, 0));
    projection_shift_vert->setAdjusterListener(this);

    projection_rotate = Gtk::manage(new Adjuster(M("TP_PERSPECTIVE_PROJECTION_ROTATE"),
        -45, 45, 0.01, 0, ipers_rotate_left, ipers_rotate_right));
    ...
    projection_pitch = Gtk::manage(new Adjuster(M("TP_PERSPECTIVE_PROJECTION_PITCH"),
        -60, 60, 0.1, 0, ipers_proj_pitch_left, ipers_proj_pitch_right));
    projection_pitch->setAdjusterListener(this);

    projection_yaw = Gtk::manage(new Adjuster(M("TP_PERSPECTIVE_PROJECTION_YAW"), -60,
        60, 0.1, 0, ipers_proj_yaw_left, ipers_proj_yaw_right));
...
void PerspCorrection::read(const ProcParams *pp, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        ...
        camera_pitch->setEditedState(
            pedited->perspective.camera_pitch ? Edited : UnEdited);
        camera_roll->setEditedState(
            pedited->perspective.camera_roll ? Edited : UnEdited);
    ...
    lines->setLines(valuesToControlLines(
        pp->perspective.control_line_values, pp->perspective.control_line_types));
...
void PerspCorrection::write(ProcParams *pp, ParamsEdited *pedited)
{
    ...
    const bool update_fl =
        camera_pitch->getValue() != 0 || camera_yaw->getValue() != 0 ||
        projection_pitch->getValue() != 0 || projection_yaw->getValue() != 0;
    ...
    if (update_fl || pp->perspective.camera_crop_factor > 0 ||
        std::abs(camera_crop_factor->getValue() -
                 PerspectiveParams::DEFAULT_CAMERA_CROP_FACTOR) > 1e-5) {
    ...
    if (update_fl || pp->perspective.camera_focal_length > 0 ||
        std::abs(camera_focal_length->getValue() -
                 PerspectiveParams::DEFAULT_CAMERA_FOCAL_LENGTH) > 1e-4) {
    ...
    if (pedited) {
        ...
        pedited->perspective.camera_focal_length =
            camera_focal_length->getEditedState();
...
void PerspCorrection::setDefaults(
    const ProcParams *defParams, const ParamsEdited *pedited)
{
    ...
    camera_crop_factor->setDefault(defParams->perspective.camera_crop_factor > 0
                                       ...
                                       : PerspectiveParams::DEFAULT_CAMERA_CROP_FACTOR);
    camera_focal_length->setDefault(
        defParams->perspective.camera_focal_length > 0
    ...
    if (pedited) {
        horiz->setDefaultEditedState(
            pedited->perspective.horizontal ? Edited : UnEdited);
        ...
        camera_focal_length->setDefaultEditedState(
            pedited->perspective.camera_focal_length ? Edited : UnEdited);
        camera_pitch->setDefaultEditedState(
            pedited->perspective.camera_pitch ? Edited : UnEdited);
        camera_roll->setDefaultEditedState(
            pedited->perspective.camera_roll ? Edited : UnEdited);
        camera_shift_horiz->setDefaultEditedState(
            pedited->perspective.camera_shift_horiz ? Edited : UnEdited);
        camera_shift_vert->setDefaultEditedState(
            pedited->perspective.camera_shift_vert ? Edited : UnEdited);
        camera_yaw->setDefaultEditedState(
            pedited->perspective.camera_yaw ? Edited : UnEdited);
        ...
        if (a == horiz || a == vert) {
            listener->panelChanged(EvPerspCorr,
                Glib::ustring::compose("%1=%2\n%3=%4", M("TP_PERSPECTIVE_HORIZONTAL"),
                    horiz->getValue(), M("TP_PERSPECTIVE_VERTICAL"), vert->getValue()));
            ...
            listener->panelChanged(
                *event_persp_cam_shift, Glib::ustring::compose("%1=%2\n%3=%4",
            ...
            listener->panelChanged(*event_persp_cam_angle,
                Glib::ustring::compose("%1=%2\n%3=%4\n%5=%6",
                    M("TP_PERSPECTIVE_CAMERA_ROLL"), camera_roll->getValue(),
                    M("TP_PERSPECTIVE_CAMERA_YAW"), camera_yaw->getValue(),
                    M("TP_PERSPECTIVE_CAMERA_PITCH"), camera_pitch->getValue()));
            ...
            listener->panelChanged(*event_persp_proj_angle,
                Glib::ustring::compose("%1=%2\n%3=%4",
                    M("TP_PERSPECTIVE_PROJECTION_PITCH"), projection_pitch->getValue(),
                    M("TP_PERSPECTIVE_PROJECTION_YAW"), projection_yaw->getValue()));
        }
    }
}
...
void PerspCorrection::applyControlLines(void)
{
    ...
    lens_geom_listener->autoPerspRequested(lines->getVerticalCount() >= MIN_VERT_LINES,
        lines->getHorizontalCount() >= MIN_HORIZ_LINES, rot, pitch, yaw,
        &control_lines);
...
void PerspCorrection::setAdjusterBehavior(bool badd, bool camera_focal_length_add,
...
void PerspCorrection::setFocalLengthValue(
    const ProcParams *pparams, const FramesMetaData *metadata)
{
...
void PerspCorrection::switchOffEditMode(void) { lines_button_edit->set_active(false); }
...
void PerspCorrection::linesEraseButtonPressed(void) { lines->removeAll(); }
