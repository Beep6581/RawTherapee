...
bool ExtProgAction::execute(const std::vector<Glib::ustring> &fileNames) const
{
    ...
    for (const auto &fileName : fileNames) {
        ...
        Gtk::MessageDialog(M("MAIN_MSG_IMAGEUNPROCESSED"), true, Gtk::MESSAGE_ERROR,
            Gtk::BUTTONS_OK, true)
            .run();
        return false;
    }
...
void ExtProgStore::init()
{
    ...
    searchProgram("Photoshop", "Adobe\\Adobe Photoshop CS%1 (64 Bit)\\Photoshop.exe",
        "Adobe\\Adobe Photoshop CS%1\\Photoshop.exe", 9, false, true);
    searchProgram(
        "Photomatix Pro", "PhotomatixPro%1\\PhotomatixPro.exe", "", 9, true, true);
    searchProgram("Paint.NET", "Paint.NET\\PaintDotNet.exe", "", 0, false, true);
    searchProgram("MS Image Composition Editor",
        "Microsoft Research\\Image Composite Editor\\ICE.exe", "", 0, false, true);
    searchProgram("PTGui", "PTGui\\PTGui.exe", "", 0, false, true);
    searchProgram("GeoSetter", "GeoSetter\\GeoSetter.exe", "", 0, true, true);
    searchProgram("FastStone Image Viewer", "FastStone Image Viewer\\FSViewer.exe", "",
        0, true, true);
    searchProgram("FastPictureViewer", "FastPictureViewer\\FastPictureViewer.exe", "",
        0, true, true);

    if (!searchProgram("Autopano Giga 3",
            "Kolor\\Autopano Giga 3.%1\\AutopanoGiga_x64.exe",
            "Kolor\\Autopano Giga 3.%1\\AutopanoGiga.exe", 15, true, true)) {
        if (!searchProgram("Autopano Pro 3",
                "Kolor\\Autopano Pro 3.%1\\AutopanoPro_x64.exe",
                "Kolor\\Autopano Pro 3.%1\\AutopanoPro.exe", 15, true, true)) {
            if (!searchProgram("Autopano Giga 2",
                    "Kolor\\Autopano Giga 2.%1\\AutopanoGiga_x64.exe",
                    "Kolor\\Autopano Giga 2.%1\\AutopanoGiga.exe", 6, true, true)) {
                searchProgram("Autopano Pro 2",
                    "Kolor\\Autopano Pro 2.%1\\AutopanoPro_x64.exe",
                    "Kolor\\Autopano Pro 2.%1\\AutopanoPro.exe", 6, true, true);
            }
        }
    }
...
bool ExtProgStore::searchProgram(const Glib::ustring &name,
    const Glib::ustring &exePath, const Glib::ustring &exePath86, int maxVer,
    bool allowRaw, bool allowQueueProcess)
{
    ...
    if (maxVer > 0) {
        ...
        for (auto ver = maxVer; ver >= 0; ver--) {
            ...
            if (!exePath86.empty()) {
                ...
                filePath =
                    progFilesDirx86 + "\\" + Glib::ustring::compose(exePath86, ver);
...
bool ExtProgStore::spawnCommandAsync(const Glib::ustring &cmd)
{
    try {
        ...
        if (rtengine::settings->verbose) {
            std::cerr << "Failed to execute \"" << cmd << "\": " << exception.what()
                      << std::endl;
        }
...
bool ExtProgStore::openInGimp(const Glib::ustring &fileName)
{
    ...
    auto success = ShellExecute(
        NULL, "open", executable.c_str(), fileName.c_str(), NULL, SW_SHOWNORMAL);

#elif defined __APPLE__

    // Apps should be opened using the simplest, case-insensitive form, "open -a
    // NameOfProgram" Calling the executable directly is said to often cause trouble,
    ...
    for (auto ver = 12; ver >= 0; --ver) {
        ...
        executable = Glib::build_filename(options.gimpDir, "bin",
            Glib::ustring::compose(Glib::ustring("gimp-2.%1.exe"), ver));
        Glib::ustring escapedFileName = Glib::ustring::compose("\"%1\"", fileName);
        auto lsuccess = ShellExecute(NULL, "open", executable.c_str(),
            escapedFileName.c_str(), NULL, SW_SHOWNORMAL);
...
bool ExtProgStore::openInPhotoshop(const Glib::ustring &fileName)
{
    ...
    const auto cmdLine = Glib::ustring("\"") + executable + Glib::ustring("\" \"") +
                         fileName + Glib::ustring("\"");

#elif defined __APPLE__

    const auto cmdLine =
        Glib::ustring("open -a Photoshop \'") + fileName + Glib::ustring("\'");
...
bool ExtProgStore::openInCustomEditor(
    const Glib::ustring &fileName, const Glib::ustring *command)
{
    if (!command) {
    ...
    auto success = ShellExecute(NULL, "open", cmdLine.c_str(),
        ('"' + fileName + '"').c_str(), NULL, SW_SHOWNORMAL);
    return (uintptr_t)success > 32;

#elif defined __APPLE__

    const auto cmdLine =
        *command + Glib::ustring(" \"") + fileName + Glib::ustring("\"");
    return spawnCommandAsync(cmdLine);
...
bool ExtProgStore::openInExternalEditor(
    const Glib::ustring &fileName, const EditorInfo &editorInfo)
{
    ...
    try {
        Glib::RefPtr<Gio::AppInfo> appInfo = Gio::AppInfo::create_from_commandline(
        ...
        std::cerr << "Error launching external editor.\n"
    ...
    if (rtengine::settings->verbose) {
        std::cout
            << "Unable to launch external editor with Gio. Trying custom launcher."
            << std::endl;
    }
