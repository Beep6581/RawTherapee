...
ShadowsHighlights::ShadowsHighlights() :
    FoldableToolPanel(this, TOOL_NAME, M("TP_SHADOWSHLIGHTS_LABEL"), false, true)
{
    ...
    hb->pack_start(
        *Gtk::manage(new Gtk::Label(M("TP_DIRPYRDENOISE_MAIN_COLORSPACE") + ": ")),
        Gtk::PACK_SHRINK);
    ...
    highlights =
        Gtk::manage(new Adjuster(M("TP_SHADOWSHLIGHTS_HIGHLIGHTS"), 0, 100, 1, 0));
    h_tonalwidth =
        Gtk::manage(new Adjuster(M("TP_SHADOWSHLIGHTS_HLTONALW"), 10, 100, 1, 70));
    ...
    s_tonalwidth =
        Gtk::manage(new Adjuster(M("TP_SHADOWSHLIGHTS_SHTONALW"), 10, 100, 1, 30));
    pack_start(*shadows);
    ...
    colorspace->signal_changed().connect(
        sigc::mem_fun(*this, &ShadowsHighlights::colorspaceChanged));
...
void ShadowsHighlights::setDefaults(
    const ProcParams *defParams, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        ...
        h_tonalwidth->setDefaultEditedState(
            pedited->sh.htonalwidth ? Edited : UnEdited);
        shadows->setDefaultEditedState(pedited->sh.shadows ? Edited : UnEdited);
        s_tonalwidth->setDefaultEditedState(
            pedited->sh.stonalwidth ? Edited : UnEdited);
    } else {
