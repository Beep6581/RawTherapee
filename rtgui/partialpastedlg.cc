    ...
    selListener(nullptr)
{
    ...
    if (col) {
        col->set_cell_data_func(
            *cell1, sigc::mem_fun(*this, &PartialSpotWidget::render_keep));
    }
...
void PartialSpotWidget::updateSpotWidget(
    const rtengine::procparams::ProcParams *pp, const bool defValue)
{
...
void PartialSpotWidget::render_keep(
...
void PartialSpotWidget::render_spotname(
    Gtk::CellRenderer *cell, const Gtk::TreeModel::iterator &iter)
{
    const auto spot = *iter;
    ...
    Gtk::Dialog(title, *parent, true)
{
    ...
    raw_pixelshift =
        Gtk::manage(new Gtk::CheckButton(M("PARTIALPASTE_RAW_PIXELSHIFT")));
    ...
    raw_dcb_enhance =
        Gtk::manage(new Gtk::CheckButton(M("PARTIALPASTE_RAW_DCBENHANCE")));
    ...
    ff_BlurType =
        Gtk::manage(new Gtk::CheckButton(M("PARTIALPASTE_FLATFIELDBLURTYPE")));
    ff_BlurRadius =
    ...
    Gtk::Dialog::get_content_area()->pack_start(
        *scrolledwindow, Gtk::PACK_EXPAND_WIDGET, 2);
    ...
    rawConn = raw->signal_toggled().connect(
        sigc::mem_fun(*this, &PartialPasteDlg::rawToggled));
    ...
    // Advanced Settings:
    retinexConn = retinex->signal_toggled().connect(sigc::bind(
        sigc::mem_fun(*advanced, &Gtk::CheckButton::set_inconsistent), true));
    colorappearanceConn = colorappearance->signal_toggled().connect(sigc::bind(
        sigc::mem_fun(*advanced, &Gtk::CheckButton::set_inconsistent), true));
    waveletConn = wavelet->signal_toggled().connect(sigc::bind(
        sigc::mem_fun(*advanced, &Gtk::CheckButton::set_inconsistent), true));
...
void PartialPasteDlg::applyPaste(rtengine::procparams::ProcParams *dstPP,
    ParamsEdited *dstPE, const rtengine::procparams::ProcParams *srcPP,
    const ParamsEdited *srcPE)
{

    ParamsEdited
        falsePE; // falsePE is a workaround to set a group of ParamsEdited to false
    falsePE.locallab.spots.resize(
        srcPP->locallab.spots.size(), LocallabParamsEdited::LocallabSpotEdited(false));
    ParamsEdited filterPE(
        true); // Contains the initial information about the loaded values
    filterPE.locallab.spots.resize(
        srcPP->locallab.spots.size(), LocallabParamsEdited::LocallabSpotEdited(true));

    if (srcPE) {
        filterPE = *srcPE;
    }

    // the general section is always ignored, whichever operation we use the
    // PartialPaste for
    ...
    if (!raw_pixelshift->get_active()) {
        filterPE.raw.bayersensor.pixelShiftBlur =
    ...
    if (!locallab->get_active() && !locallab->get_inconsistent()) {
        ...
        rtengine::procparams::ProcParams tmpPP =
            rtengine::procparams::ProcParams(*srcPP);
