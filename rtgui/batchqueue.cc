...
// from start". For N, return Nth index from the end, and for -N return the Nth index
// from the start. N is a digit 1 through 9. The returned value is not range-checked, so
// it may be >=numPathElements. or negative. The caller performs any required
// range-checking.
int decodePathIndex(
    unsigned int &ix, Glib::ustring &templateText, size_t numPathElements)
{
    int pathIndex =
    ...
    if (ix < templateText.size()) {
        if (templateText[ix] == '-') {
            fromStart = true; // minus sign means N is from the start rather than the
                              // end of the path
            ix++;
        }
    }
...
void appendAbsolutePathPrefix(Glib::ustring &path, const Glib::ustring &absolutePath)
{
    if (absolutePath[0] == '/') {
        ...
        if (absolutePath.size() > 1 && absolutePath[1] == '\\') {
            ...
            path +=
                '\\'; // Start of a Windows path that does not include a drive letter
        }
    }
}

// Look in templateText at index ix for quoted string containing a time format string,
// and use that string to format dateTime. Append the formatted time to path.
void appendFormattedTime(Glib::ustring &path, unsigned int &ix,
    const Glib::ustring &templateText, const Glib::DateTime &dateTime)
{
    ...
    processing(nullptr), fileCatalog(aFileCatalog), sequence(0), listener(nullptr)
{
    ...
    pmenu.attach(
        *Gtk::manage(open = new Gtk::MenuItem(M("FILEBROWSER_POPUPOPENINEDITOR"))), 0,
        1, p, p + 1);
    p++;
    pmenu.attach(
    ...
    pmenu.attach(*Gtk::manage(head = new MyImageMenuItem(
                                  M("FILEBROWSER_POPUPMOVEHEAD"), "goto-start-small")),
        0, 1, p, p + 1);
    p++;

    pmenu.attach(*Gtk::manage(tail = new MyImageMenuItem(
                                  M("FILEBROWSER_POPUPMOVEEND"), "goto-end-small")),
        0, 1, p, p + 1);
    p++;
    ...
    open->add_accelerator(
        "activate", pmaccelgroup, GDK_KEY_e, Gdk::CONTROL_MASK, Gtk::ACCEL_VISIBLE);
    selall->add_accelerator(
        "activate", pmaccelgroup, GDK_KEY_a, Gdk::CONTROL_MASK, Gtk::ACCEL_VISIBLE);
...
int BatchQueue::getMaxThumbnailHeight() const { return calcMaxThumbnailHeight(); }

void BatchQueue::saveThumbnailHeight(int height) { options.thumbSizeQueue = height; }
...
void BatchQueue::rightClicked() { pmenu.popup(3, this->eventTime); }
...
void BatchQueue::addEntries(
    const std::vector<BatchQueueEntry *> &entries, bool head, bool save)
{
    {
        MYWRITERLOCK(l, entryRW);
...

    {
        ...
        file << "input image full path|param file full path|output image full "
             ...
             << "png bit depth|png compression|tiff bit depth|tiff is "
                "float|uncompressed tiff|save output params|force format options|fast "
                "export|<end of line>"
             << std::endl;
        ...
        for (const auto fdEntry : fd) {
            ...
            file << entry->filename << '|' << entry->savedParamsFile << '|'
                 ...
                 << entry->fast_pipeline << '|' << saveFormat.bigTiff << '|'
                 << std::endl;
        }
    }
...
bool BatchQueue::loadBatchQueue()
{
    ...
    if (file.is_open()) {
        ...
        while (std::getline(file, row)) {
            ...
            auto entry = new BatchQueueEntry(
                job, pparams, source, prevw, prevh, thumb, options.overwriteOutputFile);
            thumb
                ->decreaseRef(); // Removing the refCount acquired by cacheMgr->getEntry
            entry->setParent(this);
...
void BatchQueue::cancelItems(const std::vector<ThumbBrowserEntryBase *> &items)
{
    ...
    if (!removable_bqes.empty()) {
        mutex_removable_batch_queue_entries.lock();
        removable_batch_queue_entries.insert(
            removable_bqes.begin(), removable_bqes.end());
        mutex_removable_batch_queue_entries.unlock();
...
void BatchQueue::error(const Glib::ustring &descr)
{
    if (processing && processing->processing) {
        ...
        processing->job = rtengine::ProcessingJob::create(processing->filename,
            processing->thumbnail->getType() == FT_Raw, *processing->params);
        processing = nullptr;
        redraw();
    }
...
rtengine::ProcessingJob *BatchQueue::imageReady(rtengine::IImagefloat *img)
{
    ...
    if (processing->outFileName.empty()) { // auto file name
        Glib::ustring s =
            calcAutoFileNameBase(processing->filename, processing->sequence);
        ...
        // format, despite what the user have set in the filename's field of the "Save
        // as" dialog box
        fname = autoCompleteFileName(
            removeExtension(processing->outFileName), saveFormat.format);
    ...
    if (img && !fname.empty()) {
        ...
        if (saveFormat.format == "tif") {
            ...
            err =
                img->saveAsJPEG(fname, saveFormat.jpegQuality, saveFormat.jpegSubSamp);
        }
        ...
        if (err) {
            throw Glib::FileError(
                Glib::FileError::FAILED, M("MAIN_MSG_CANNOTSAVE") + "\n" + fname);
        }
        ...
        if (isEmpty) {
            ...
            try {
                ...
                while (auto file = enumerator->next_file()) {
                    ::g_remove(
                        Glib::build_filename(batchdir, file->get_name()).c_str());
                }
...
Glib::ustring BatchQueue::calcAutoFileNameBase(
    const Glib::ustring &origFileName, int sequence)
{
            ...
            if (options.savePathTemplate[ix] == '%') {
                ...
                if (options.savePathTemplate[ix] == 'P') {
                    ...
                    if (n < static_cast<int>(da.size())) {
                        ...
                        for (unsigned int i = static_cast<unsigned int>(n);
                            i < da.size(); i++) {
                    ...
                    // Insert formatted date/time value. Character after 't' defines
                    // time source
                    if (++ix < options.savePathTemplate.size()) {
                        ...
                        if (dateTime) {
                            appendFormattedTime(
                                path, ix, options.savePathTemplate, dateTime);
                        }
                    }
                }
            }
...
Glib::ustring BatchQueue::autoCompleteFileName(
    const Glib::ustring &fileName, const Glib::ustring &format)
{
...
void BatchQueue::selectionChanged() { updateDestinationPathPreview(); }
