...
enum MouseOverAreas {
    ...
    FCT_Area_H =
        1
        << 3, // cursor is on the horizontal line going through the edited control point
    FCT_Area_V =
        1
        << 4, // cursor is on the vertical   line going through the edited control point
...
public:
    ...
    std::vector<double> x, // Range: [0.0 - 1.0]
        ...
        leftTangent,  // Range: [0.0 - 1.0], where 1.0 = distance from previous to this
                      // point
        rightTangent; // Range: [0.0 - 1.0], where 1.0 = distance from this to next
                      // point
};
...
protected:
    ...
    double
        deltaX; // signed X distance of the cursor between two consecutive MOTION_NOTIFY
    double
        deltaY; // signed Y distance of the cursor between two consecutive MOTION_NOTIFY
    ...
    double deletedPointX;  // Backup of the X value of the edited point, when deleted
                           // while being dragged
    HandlePosition leftTanHandle;  // XY coordinate if the upper left and bottom right
                                   // corner of the left tangent handle
    HandlePosition rightTanHandle; // XY coordinate if the upper left and bottom right
                                   // corner of the right tangent handle
    ...
    CursorShape motionNotify(
        CursorShape type, double minDistanceX, double minDistanceY, int num);
...
public:
    ...
    void reset(
        const std::vector<double> &resetCurve, double identityValue = 0.5) override;
    // void updateBackgroundHistogram (unsigned int* hist);

    void pipetteMouseOver(
        CurveEditor *ce, EditDataProvider *provider, int modifierKey) override;
