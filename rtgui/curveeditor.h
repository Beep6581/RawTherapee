 ...
 * It handles the methods related to a specific curve. It is created by
 * CurveEditorGroup::addCurve
 */
...
protected:
    /*
     * The curve editor contains only one widget (the curve type button) to receive the
     * signals but it's co-handled by the CurveEditorGroup too
     */
...
public:
    CurveEditor(
        Glib::ustring text, CurveEditorGroup *ceGroup, CurveEditorSubGroup *ceSubGroup);
    ...
    void refresh(); // refresh the display of the CurveEditor (e.g. when a ColoredBar
                    // has been changed from the outside)
    bool openIfNonlinear(); // Open up the curve if it has modifications and it's not
                            // already opened
...
protected:
    // reflects the buttonType active selection ; used as a pre-'selectionChange'
    // reminder value
...
public:
    ...
    void setRangeLabels(
        Glib::ustring r1, Glib::ustring r2, Glib::ustring r3, Glib::ustring r4);
    void getRangeLabels(
        Glib::ustring &r1, Glib::ustring &r2, Glib::ustring &r3, Glib::ustring &r4);
    void setRangeDefaultMilestones(double m1, double m2, double m3);
    void getRangeDefaultMilestones(double &m1, double &m2, double &m3);

    // set the reset curve for a given curve type. This is optional; all curve type have
    // a default reset curve
...
public:
    FlatCurveEditor(Glib::ustring text, CurveEditorGroup *ceGroup,
        CurveEditorSubGroup *ceSubGroup, bool isPeriodic = true);
    void setIdentityValue(const double iValue = 0.5) override
    {
        identityValue = iValue;
    }
    double getIdentityValue() override { return identityValue; };
    std::vector<double> getCurve() override;
