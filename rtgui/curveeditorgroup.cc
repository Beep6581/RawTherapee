...
CurveEditorGroup::CurveEditorGroup(
    Glib::ustring &curveDir, Glib::ustring groupLabel, int blank) :
    curveDir(curveDir), line(0), curve_reset(nullptr), displayedCurve(nullptr),
    ...
    numberOfPackedCurve(0)
{
    ...
    if (blank == 0) {
        curveGroupLabel =
            Gtk::manage(new Gtk::Label(groupLabel + ":", Gtk::ALIGN_START));
    } else if (blank == 1) {
        curveGroupLabel = Gtk::manage(new Gtk::Label(groupLabel, Gtk::ALIGN_START));
    }

    setExpandAlignProperties(
        curveGroupLabel, false, false, Gtk::ALIGN_START, Gtk::ALIGN_CENTER);
 ...
 *     curveLabel:    Name of the curve that will be inserted in the toggle button,
 * before the image. If empty, no text will prepend the image relatedWidget: pointer to
 * a widget (or NULL) that will be inserted next to the curve's toggle button. if a
 * smart pointer created by Gtk::manage is passed in, the widget will be deleted by the
 * destructor, otherwise it'll have to be delete it manually periodic:      for
 * FlatCurve only, ask the curve to be periodic (default: True)
 *
 */
CurveEditor *CurveEditorGroup::addCurve(CurveType cType, Glib::ustring curveLabel,
    Gtk::Widget *relatedWidget, bool expandRelatedWidget, bool periodic)
{
    switch (cType) {
        case (CT_Diagonal): {
            if (!diagonalSubGroup) {
...
void CurveEditorGroup::newLine()
{
    ...
    if (curveEditors.size() > numberOfPackedCurve) {
        ...
        setExpandAlignProperties(
            currLine, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_START);
        ...
        for (int i = numberOfPackedCurve; i < (int)(curveEditors.size()); ++i) {
            setExpandAlignProperties(curveEditors[i]->curveType->buttonGroup, !rwe,
                true, Gtk::ALIGN_FILL, Gtk::ALIGN_FILL);
            currLine->attach(*curveEditors[i]->curveType->buttonGroup, x++, 0, 1, 1);

            if (curveEditors[i]->relatedWidget != nullptr) {
                setExpandAlignProperties(curveEditors[i]->relatedWidget,
                    curveEditors[i]->expandRelatedWidget, true, Gtk::ALIGN_FILL,
                    Gtk::ALIGN_FILL);
        ...
        if (isHeader) {
            ...
            setExpandAlignProperties(
                curve_reset, false, false, Gtk::ALIGN_CENTER, Gtk::ALIGN_FILL);
            curve_reset->add(
...
void CurveEditorGroup::attachCurve(Gtk::Grid *curve) { attach(*curve, 0, line, 1, 1); }

/*
 * Create all the widgets now that the curve list is complete
 * This method should handle all curve number correctly, i.e. eventually display the
 * curve type buttons in a grid (or table)
 */
void CurveEditorGroup::curveListComplete()
{
    newLine();

    // We check the length of the label ; if it contains only one char (':'), we set it
    // to the right default string
    if (curveGroupLabel->get_label().size() == 1) {
        curveGroupLabel->set_label(
            M(curveEditors.size() > 1 ? "CURVEEDITOR_CURVES" : "CURVEEDITOR_CURVE") +
            ":");
    }
 ...
 * it will activate the button, and so emit 'signal_toggled' (-> curveTypeToggled here
 * under)
 */
void CurveEditorGroup::typeSelectionChanged(CurveEditor *ce, int n)
{
    ...
    if (!ce->curveType->get_active()) {
        ...
        ce->curveType->set_active(
            n > ce->subGroup->valLinear && n < ce->subGroup->valUnchanged);

        if (n == ce->subGroup->valLinear || n == ce->subGroup->valUnchanged) {
            // Since we do not activate the curve when the user switch the toggled off
            // button to 'Linear', we have to to call the curve listener manually,
            // because 'curveChanged' uses displayedCurve...
            if (cl) {
                if (cl->isMulti()) {
                    cl->curveChanged(ce);
                } else {
...
void CurveEditorGroup::updateGUI(CurveEditor *ce)
{
    ...
    // we update the curve type button to the corresponding curve type, only if it is
    // not currently set to 'Unchanged'
    if (ce->curveType->getSelected() < ce->subGroup->valUnchanged) {
        ce->curveType->setSelected(ce->selected);
    }
...
void CurveEditorGroup::setCurveExternal(CurveEditor *ce, const std::vector<double> &c)
{
    if (!c.empty()) {
        ...
        (ce)->selected = c[0]; // We set the selected curve type in the CurveEditor to
                               // the one of the specified curve
    }
 ...
 * Listener called when the user has modified the curve
 */
float CurveEditorGroup::blendPipetteValues(
...
{
    if (uc) {
        // the user selected several thumbnails, so we hide the editors and set the
        // curveEditor selection to 'Unchanged'
...
CurveEditorSubGroup::CurveEditorSubGroup(Glib::ustring &curveDir) :
    curveDir(curveDir), valLinear(0), valUnchanged(0), parent(nullptr)
{
    leftBar = nullptr;
    bottomBar = nullptr;
}
...
void CurveEditorSubGroup::initButton(Gtk::Button &button, const Glib::ustring &iconName,
    Gtk::Align align, bool separatorButton, const Glib::ustring &tooltip)
{
...
void CurveEditorSubGroup::updateEditButton(
    CurveEditor *curve, Gtk::ToggleButton *button, sigc::connection &connection)
{
...
Glib::ustring CurveEditorSubGroup::outputFile()
{
    ...
    Gtk::FileChooserDialog dialog(getToplevelWindow(parent),
        M("CURVEEDITOR_SAVEDLGLABEL"), Gtk::FILE_CHOOSER_ACTION_SAVE);
...
Glib::ustring CurveEditorSubGroup::inputFile()
{
    ...
    Gtk::FileChooserDialog dialog(getToplevelWindow(parent),
        M("CURVEEDITOR_LOADDLGLABEL"), Gtk::FILE_CHOOSER_ACTION_OPEN);
    bindCurrentFolder(dialog, curveDir);
