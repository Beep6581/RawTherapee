...
BayerPreProcess::BayerPreProcess() :
    FoldableToolPanel(
        this, TOOL_NAME, M("TP_PREPROCESS_LABEL"), options.prevdemo != PD_Sidecar)
{
    auto m = ProcEventMapper::getInstance();
    EvLineDenoiseDirection =
        m->newEvent(DARKFRAME, "HISTORY_MSG_PREPROCESS_LINEDENOISE_DIRECTION");
    EvPDAFLinesFilter =
        m->newEvent(DARKFRAME, "HISTORY_MSG_PREPROCESS_PDAFLINESFILTER");
...
void BayerPreProcess::read(
    const rtengine::procparams::ProcParams *pp, const ParamsEdited *pedited)
{
    disableListener();

    if (pedited) {
        lineDenoise->setEditedState(
            pedited->raw.bayersensor.linenoise ? Edited : UnEdited);
        greenEqThreshold->setEditedState(
    ...
    const rtengine::procparams::ProcParams *defParams, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        lineDenoise->setDefaultEditedState(
            pedited->raw.bayersensor.linenoise ? Edited : UnEdited);
...
void BayerPreProcess::lineDenoiseDirectionChanged()
{
    if (listener) {
        listener->panelChanged(
            EvLineDenoiseDirection, lineDenoiseDirection->get_active_text());
    }
}
