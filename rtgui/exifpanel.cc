...
ExifPanel::ExifPanel() :
    idata(nullptr), changeList(new rtengine::procparams::ExifPairs),
    defChangeList(new rtengine::procparams::ExifPairs),
    // keepicon("tick-small"),
    editicon("edit-small"), open_icon_("expander-open-small"),
    closed_icon_("expander-closed-small"), pl_(nullptr)
{
    ...
    exif_active_renderer_.signal_toggled().connect(
        sigc::mem_fun(this, &ExifPanel::onKeyActiveToggled));
    exif_active_column_.pack_start(exif_active_renderer_);
    exif_active_column_.set_cell_data_func(
        exif_active_renderer_, sigc::mem_fun(this, &ExifPanel::setKeyActive));
    ...
    viewcolv->set_cell_data_func(
        *render_txtv, sigc::mem_fun(this, &ExifPanel::setExifTagValue));
    render_txtv->signal_edited().connect(
        sigc::mem_fun(this, &ExifPanel::onEditExifTagValue));
    ...
    const auto addbtn = [&](const Glib::ustring &tip,
                            const Glib::ustring &icon) -> Gtk::Button * {
    ...
    exifTree->get_selection()->signal_changed().connect(
        sigc::mem_fun(*this, &ExifPanel::exifSelectionChanged));

    reset->signal_clicked().connect(sigc::mem_fun(*this, &ExifPanel::resetPressed));
    resetAll->signal_clicked().connect(
        sigc::mem_fun(*this, &ExifPanel::resetAllPressed));
    add->signal_clicked().connect(sigc::mem_fun(*this, &ExifPanel::addPressed));
    activate_all_->signal_clicked().connect(
        sigc::mem_fun(*this, &ExifPanel::activateAllPressed));
    activate_none_->signal_clicked().connect(
        sigc::mem_fun(*this, &ExifPanel::activateNonePressed));

    exifTree->signal_button_press_event().connect_notify(
        sigc::mem_fun(*this, &ExifPanel::onExifTreeClick));
    exifTree->signal_row_expanded().connect(
        sigc::mem_fun(*this, &ExifPanel::onExifRowExpanded));
    exifTree->signal_row_collapsed().connect(
        sigc::mem_fun(*this, &ExifPanel::onExifRowCollapsed));
...
void ExifPanel::read(const ProcParams *pp, const ParamsEdited *pedited)
{
    ...
    *changeList = pp->metadata.exif;
    ...
    initial_active_keys_.insert(
        pp->metadata.exifKeys.begin(), pp->metadata.exifKeys.end());
...
void ExifPanel::addTag(const std::string &key,
    const std::pair<Glib::ustring, Glib::ustring> &label,
    const Glib::ustring &exifValue, bool editable, bool edited)
{
    ...
    const auto getgroup = [&]() -> Gtk::TreeNodeChildren {
        auto root = exifTreeModel->children();
    ...
    bool active =
        all_keys_active() || cur_active_keys_.find(key) != cur_active_keys_.end();
...
void ExifPanel::refreshTags()
{
    ...
    try {
        ...
        const auto to_value = [&](Exiv2::Exifdatum &tag) -> Glib::ustring {
            try {
        ...
        struct KeyLt {
            KeyLt() : order_{{"Exif.GPSInfo", 0}, {"Exif.Photo", 1}, {"Exif.Image", 2}}
            {
            }
...
void ExifPanel::onExifTreeClick(GdkEventButton *event)
{
    ...
    int cell_x;
    int cell_y;
    if (exifTree->get_path_at_pos(event->x, event->y, pth, col, cell_x, cell_y) &&
        col == exifTree->get_column(1) && cell_x <= 22) {
...
void ExifPanel::onExifRowExpanded(
...
void ExifPanel::onExifRowCollapsed(
    const Gtk::TreeModel::iterator &it, const Gtk::TreeModel::Path &path)
{
    auto row = *it;
    if (row[exifColumns.is_group]) {
...
void ExifPanel::setExifTagValue(
    Gtk::CellRenderer *renderer, const Gtk::TreeModel::iterator &it)
{
    auto row = *it;
...
std::unordered_map<std::string, validator_func> validators = {
    {"Exif.Photo.FNumber", get_fnumber}, {"Exif.Photo.ExposureTime", get_shutterspeed},
...
void ExifPanel::setProgressListener(rtengine::ProgressListener *pl) { pl_ = pl; }
