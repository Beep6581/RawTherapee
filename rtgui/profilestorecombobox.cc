...
void ProfileStoreComboBox::refreshProfileList_(Gtk::TreeModel::Row *parentRow,
    int parentFolderId, bool initial,
        ...
        if (entry->parentFolderId ==
            ...
            if (entry->type == PSET_FOLDER) {
                Glib::ustring folderPath(
                    ProfileStore::getInstance()->getPathFromId(entry->folderId));
                ...
                if (options.useBundledProfiles ||
                    ...
                    Gtk::TreeModel::Row menuHeader =
                        *(refTreeModel->append(newSubMenu->children()));
 ...
 * If you want to update the ProfileStore list itself (rescan the dir tree), use the
 * "ProfileStore::parseProfiles" method instead
 *
...
Gtk::TreeIter ProfileStoreComboBox::findRowFromEntry_(
    Gtk::TreeModel::Children childs, const ProfileStoreEntry *pse) const
{
    ...
    for (const auto &iter : childs) {
        ...
        if (pse_->type == PSET_FOLDER) {
            const Gtk::TreeIter rowInSubLevel =
                findRowFromEntry_(iter->children(), pse);
...
Gtk::TreeIter ProfileStoreComboBox::findRowFromFullPath_(
    Gtk::TreeModel::Children childs, int parentFolderId,
...
Gtk::TreeIter ProfileStoreComboBox::findRowFromFullPath(const Glib::ustring &path) const
{
    ...
    if (!fName.empty()) {
        int parentFolderId = profileStore->findFolderId(
            Glib::path_get_dirname(path.substr(0, path.length() - fName.length())));
        // 1. find the path in the folder list
        if (parentFolderId != -1) {
            return findRowFromFullPath_(
                refTreeModel->children(), parentFolderId, fName);
        }
    }
...
Glib::ustring ProfileStoreComboBox::getFullPathFromActiveRow() const
{
    ...
    if (currRow) {
        ...
        return Glib::build_filename(
            profileStore->getPathFromId(currEntry->parentFolderId), currEntry->label);
    }
...
Gtk::TreeIter ProfileStoreComboBox::getRowFromLabel(const Glib::ustring &name) const
{
    ...
    return childs.end();
    // return refTreeModel->get_iter(""); // is this fast? We want to send back a null,
    // anvalid or end() iterator object here
}
