...
ThumbBrowserEntryBase::ThumbBrowserEntryBase(
    const Glib::ustring &fname, Thumbnail *thm) :
    fnlabw(0), fnlabh(0), dtlabw(0), dtlabh(0), exlabw(0), exlabh(0), prew(0), preh(0),
    prex(0), prey(0), upperMargin(6), borderWidth(1), textGap(6), sideMargin(8),
    lowerMargin(8), dispname(Glib::path_get_basename(fname)), buttonSet(nullptr),
    width(0), height(0), exp_width(0), exp_height(0), startx(0), starty(0), ofsX(0),
    ofsY(0), redrawRequests(0), parent(nullptr), original(nullptr), bbSelected(false),
    bbFramed(false), bbPreview(nullptr), cursor_type(CSUndefined),
    ...
    thumbnail(thm), filename(fname), selected(false), drawable(false), filtered(false),
    framed(false), processing(false), italicstyle(false), edited(false),
    recentlysaved(false), updatepriority(false), withFilename(WFNAME_NONE)
{
}

ThumbBrowserEntryBase::~ThumbBrowserEntryBase() { delete buttonSet; }

void ThumbBrowserEntryBase::addButtonSet(LWButtonSet *bs) { buttonSet = bs; }
...
void ThumbBrowserEntryBase::updateBackBuffer()
{
    ...
    if (backBuffer && (backBuffer->getWidth() != exp_width ||
                          backBuffer->getHeight() != exp_height)) {
    ...
    */
    ...
    // draw preview frame
    // backBuffer->draw_rectangle (cc, false, (exp_width-prew)/2, upperMargin+bsHeight,
    // prew+1, preh+1);
    // draw thumbnail image
    if (!preview.empty()) {
        prex = borderWidth + (exp_width - prew) / 2;
        const int hh =
            exp_height - (upperMargin + bsHeight + borderWidth + infoh + lowerMargin);
        prey = upperMargin + bsHeight + borderWidth + std::max((hh - preh) / 2, 0);
        backBuffer->copyRGBCharData(
            preview.data(), 0, 0, prew, preh, prew * 3, prex, prey);
    }
    ...
    if ((parent->getLocation() != ThumbBrowserBase::THLOC_EDITOR &&
        ...
        (parent->getLocation() == ThumbBrowserBase::THLOC_EDITOR &&
            options.filmStripShowFileNames && options.filmStripOverlayedFileNames)) {
        cc->begin_new_path();
    ...
    if (!bbIcons.empty()) {
        ...
        for (size_t i = 0; i < bbIcons.size(); i++) {
            ...
            cc->rectangle(
                istartx, istarty, bbIcons[i]->getWidth(), bbIcons[i]->getHeight());
            cc->fill();
    ...
    if (!bbSpecificityIcons.empty()) {
        ...
        for (size_t i = 0; i < bbSpecificityIcons.size(); ++i) {
            ...
            cc->set_source(bbSpecificityIcons[i]->get(), istartx2,
                istarty2 - bbSpecificityIcons[i]->getHeight());
            cc->rectangle(istartx2, istarty2 - bbSpecificityIcons[i]->getHeight(),
                bbSpecificityIcons[i]->getWidth(), bbSpecificityIcons[i]->getHeight());
            cc->fill();
        }
    }
...
void ThumbBrowserEntryBase::resize(int h)
{
    ...
    if (preh != old_preh ||
        prew != old_prew) { // if new thumbnail height or new orientation
        preview.clear();
...
void ThumbBrowserEntryBase::drawFrame(
    Cairo::RefPtr<Cairo::Context> cc, const Gdk::RGBA &bg, const Gdk::RGBA &fg)
{
    ...
    if (selected || framed) {
        ...
        cc->set_source_rgb(
            bg.get_red() * 2 / 3, bg.get_green() * 2 / 3, bg.get_blue() * 2 / 3);
    ...
    if (framed) {
        ...
        cc->arc(-2 + 0.5 + exp_width - 1 - radius, +2 + 0.5 + radius, radius,
            -rtengine::RT_PI / 2, 0);
        cc->arc(-2 + 0.5 + exp_width - 1 - radius, -2 + 0.5 + exp_height - 1 - radius,
            radius, 0, rtengine::RT_PI / 2);
        cc->arc(+2 + 0.5 + radius, -2 + exp_height - 1 - radius, radius,
            rtengine::RT_PI / 2, rtengine::RT_PI);
        cc->arc(+2 + 0.5 + radius, +2 + radius, radius, rtengine::RT_PI,
            -rtengine::RT_PI / 2);
        cc->close_path();
...
void ThumbBrowserEntryBase::draw(Cairo::RefPtr<Cairo::Context> cc)
{
    ...
    if (!backBuffer || selected != bbSelected || framed != bbFramed ||
        preview.data() != bbPreview || exp_width != bbWidth || exp_height != bbHeight ||
    ...
    //    drawProgressBar (window, cc, selected ? texts : textn, selected ? bgs : bgn,
    //    ofsX+startx, exp_width, ofsY+starty +
    //    upperMargin+bsHeight+borderWidth+preh+borderWidth+textGap+tpos, fnlabh);

    // redraw button set above the thumbnail
    if (buttonSet) {
        buttonSet->setColors(
            selected ? parent->getSelectedBgColor() : parent->getNormalBgColor(),
            selected ? parent->getNormalBgColor() : parent->getSelectedBgColor());
        buttonSet->redraw(cc);
    }
}
...
void ThumbBrowserEntryBase::setPosition(int x, int y, int w, int h)
{
    ...
    if (buttonSet) {
        buttonSet->arrangeButtons(
            ofsX + x + sideMargin, ofsY + y + upperMargin, w - 2 * sideMargin, -1);
    }
}
...
bool ThumbBrowserEntryBase::inside(int x, int y) const
{
    ...
    return x > ofsX + startx && x < ofsX + startx + exp_width && y > ofsY + starty &&
           y < ofsY + starty + exp_height;
}
...
bool ThumbBrowserEntryBase::releaseNotify(
    int button, int type, int bstate, int x, int y)
{
...
std::tuple<Glib::ustring, bool> ThumbBrowserEntryBase::getToolTip(int x, int y) const
{
    ...
    // Always show the filename in the tooltip since the filename in the thumbnail could
    // be truncated. If "Show Exif info" is disabled, also show Exif info in the
    // tooltip.
