...
void drawCrop(Cairo::RefPtr<Cairo::Context> cr, int imx, int imy, int imw, int imh,
    ...
    const rtengine::procparams::CropParams &cparams, bool drawGuide = true,
    bool useBgColor = true, bool fullImageVisible = true);
...
struct ScopedEnumHash {
    template <typename T,
        typename std::enable_if<
            std::is_enum<T>::value && !std::is_convertible<T, int>::value, int>::type =
            0>
...
// TODO: The documentation says gdk_threads_enter and gdk_threads_leave should be
// replaced by g_main_context_invoke(), g_idle_add() and related functions, but this
// will require more extensive changes. We silence those warnings until then so that we
// notice the others.
...
{
public:
    explicit ConnectionBlocker(
        Gtk::Widget *associatedWidget, sigc::connection &connection) :
...
private:
    ...
    const Glib::ustring
        inconsistentImage; /// "inconsistent" image, displayed when useEnabled is true ;
                           ...
                           /// opened/closed
    const Glib::ustring
        enabledImage; ///      "enabled" image, displayed when useEnabled is true ; in
    ...
    /// Triggered on enabled/disabled change -> will emit a toggle event to the
    /// connected objects
...
protected:
    ...
    Gtk::Widget
        *headerWidget; /// Widget to display in the header, next to the arrow image ;
...
public:
     ...
     * @param titleLabel A string to display in the header. Warning: you won't be able
     * to switch to a widget label.
     */
     ...
     * @param useEnabled Set whether to handle an enabled/disabled toggle button and
     * display the appropriate image
     * @param titleWidget A widget to display in the header. Warning: you won't be able
     * to switch to a string label.
     */
    ...
    /// Get whether the enabled option is set (to true or false) or unset (i.e.
    /// undefined)
    bool get_inconsistent();
    /// Set whether the enabled option is set (to true or false) or unset (i.e.
    /// undefined)
    ...
    bool getEnabled();
    /// If not inconsistent, set the enabled button to true or false and emit the
    /// message if the state is different If inconsistent, set the internal value to
    /// true or false, but do not update the image and do not emit the message
    ...
    /// Get the header widget. It'll send back the Gtk::Label* if it has been
    /// instantiated with a simple text
...
/**
 * @brief Handle the switch between text and image to be displayed in the HBox (to be
 * used in a button/toolpanel)
 */
class TextOrIcon final : public Gtk::Box
{

public:
    TextOrIcon(const Glib::ustring &icon_name, const Glib::ustring &labelTx,
 ...
 */
class MyRadioImageMenuItem final : public Gtk::RadioMenuItem,
...
public:
    ...
    GradientMilestone(
        double _p = 0., double _r = 0., double _g = 0., double _b = 0., double _a = 0.)
    {
        position = _p;
        r = _r;
        g = _g;
...
public:
    ...
    // set the destination drawing rectangle; return true if the dimensions are
    // different Note: newW & newH must be > 0
    bool setDrawRectangle(Glib::RefPtr<Gdk::Window> window, Gdk::Rectangle &rectangle,
        bool updateBackBufferSize = true);
    bool setDrawRectangle(Glib::RefPtr<Gdk::Window> window, int newX, int newY,
        int newW, int newH, bool updateBackBufferSize = true);
    bool setDrawRectangle(Cairo::Format format, Gdk::Rectangle &rectangle,
        bool updateBackBufferSize = true);
    bool setDrawRectangle(Cairo::Format format, int newX, int newY, int newW, int newH,
        bool updateBackBufferSize = true);
    // set the destination drawing location, do not modify other parameters like size
    // and offset. Use setDrawRectangle to set all parameters at the same time
    ...
    void copyRGBCharData(const unsigned char *srcData, int srcX, int srcY, int srcW,
        int srcH, int srcRowStride, int dstX, int dstY);
    void copySurface(
        Glib::RefPtr<Gdk::Window> window, Gdk::Rectangle *rectangle = nullptr);
    void copySurface(BackBuffer *destBackBuffer, Gdk::Rectangle *rectangle = nullptr);
    void copySurface(Cairo::RefPtr<Cairo::ImageSurface> destSurface,
        Gdk::Rectangle *rectangle = nullptr);
    void copySurface(
        Cairo::RefPtr<Cairo::Context> crDest, Gdk::Rectangle *destRectangle = nullptr);
...
public:
    SpotPicker(int const defaultValue, Glib::ustring const &buttonKey,
...
protected:
    ...
    Gtk::ToggleButton spotButtonTemplate(
...
inline void setActiveTextOrIndex(
    Gtk::ComboBoxText &comboBox, const Glib::ustring &text, int index)
{
    bool valueSet = false;
    if (!text.empty()) {
