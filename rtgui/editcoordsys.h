...
public:
    ...
    /// Convert the widget's DrawingArea (i.e. preview area) coords to the edit buffer
    /// coords
    virtual void screenCoordToCropBuffer(
        int phyx, int phyy, int &cropx, int &cropy) = 0;
    /// Convert the widget's DrawingArea (i.e. preview area) coords to the full image
    /// coords
