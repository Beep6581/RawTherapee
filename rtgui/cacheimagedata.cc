...
CacheImageData::CacheImageData() :
    supported(false), format(FT_Invalid), rankOld(-1), inTrashOld(false),
    recentlySaved(false), timeValid(false), year(0), month(0), day(0), hour(0), min(0),
    sec(0), exifValid(false), frameCount(1), fnumber(0.0), shutter(0.0), focalLen(0.0),
    focalLen35mm(0.0), focusDist(0.f), iso(0), rating(0), isHDR(false), isDNG(false),
    isPixelShift(false), sensortype(rtengine::ST_NONE),
    sampleFormat(rtengine::IIOSF_UNKNOWN), redAWBMul(-1.0), greenAWBMul(-1.0),
    blueAWBMul(-1.0), rotate(0), thumbImgType(0), width(-1), height(-1)
{
}

/*
 * Load the General, DateTime, ExifInfo, File info and ExtraRawInfo sections of the
 * image data file
 */
int CacheImageData::load(const Glib::ustring &fname)
{
    ...
    try {
        if (keyFile.load_from_file(fname)) {
            ...
            if (keyFile.has_group(INI_GROUP_XMP_SIDECAR)) {
                if (keyFile.has_key(INI_GROUP_XMP_SIDECAR, INI_XMP_SIDECAR_MD5)) {
                    xmpSidecarMd5 =
                        keyFile.get_string(INI_GROUP_XMP_SIDECAR, INI_XMP_SIDECAR_MD5);
                }
            }
            ...
            if (keyFile.has_group("FileInfo")) {
                ...
                if (keyFile.has_key("FileInfo", "FrameCount")) {
                    frameCount = static_cast<unsigned int>(
                        keyFile.get_integer("FileInfo", "FrameCount"));
                }
                if (keyFile.has_key("FileInfo", "SampleFormat")) {
                    sampleFormat = (rtengine::IIO_Sample_Format)keyFile.get_integer(
                        "FileInfo", "SampleFormat");
                }
            ...
            if (format == FT_Raw && keyFile.has_group("ExtraRawInfo")) {
                if (keyFile.has_key("ExtraRawInfo", "ThumbImageType")) {
                    thumbImgType =
                        keyFile.get_integer("ExtraRawInfo", "ThumbImageType");
                }
        ...
        if (rtengine::settings->verbose) {
            printf("CacheImageData::load / Error code %d while reading values from "
                   "\"%s\":\n%s\n",
                err.code(), fname.c_str(), err.what().c_str());
        }
    } catch (...) {
        if (rtengine::settings->verbose) {
            printf("CacheImageData::load / Unknown exception while trying to load "
                   "\"%s\"!\n",
                fname.c_str());
        }
    }
...
/*
 * Save the General, DateTime, ExifInfo, File info and ExtraRawInfo sections of the
 * image data file
 */
int CacheImageData::save(const Glib::ustring &fname)
{
    ...
    try {
        ...
        if (rtengine::settings->verbose) {
            printf("CacheImageData::save / Error code %d while reading values from "
                   "\"%s\":\n%s\n",
                err.code(), fname.c_str(), err.what().c_str());
        }
    } catch (...) {
        if (rtengine::settings->verbose) {
            printf("CacheImageData::save / Unknown exception while trying to save "
                   "\"%s\"!\n",
                fname.c_str());
        }
    }
    ...
    if (!f) {
        if (rtengine::settings->verbose) {
            printf("CacheImageData::save / Error: unable to open file \"%s\" with "
                   "write access!\n",
                fname.c_str());
        }
...
std::uint32_t CacheImageData::getFixBadPixelsConstant() const { return 0; }

bool CacheImageData::hasFixBadPixelsConstant() const { return false; }
