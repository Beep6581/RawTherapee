...
BlackWhite::BlackWhite() :
    FoldableToolPanel(this, TOOL_NAME, M("TP_BWMIX_LABEL"), false, true)
{
    ...
    methodconn = method->signal_changed().connect(
        sigc::mem_fun(*this, &BlackWhite::methodChanged));
    ...
    luminanceCurve = static_cast<FlatCurveEditor *>(
        luminanceCEG->addCurve(CT_Flat, M("TP_BWMIX_VAL")));
    ...
    autoconn = autoch->signal_toggled().connect(
        sigc::mem_fun(*this, &BlackWhite::autoch_toggled));

    neutral = Gtk::manage(new Gtk::Button(M("TP_BWMIX_NEUTRAL")));
    neutralconn = neutral->signal_pressed().connect(
        sigc::mem_fun(*this, &BlackWhite::neutral_pressed));
    neutral->show();
    ...
    settingconn = setting->signal_changed().connect(
        sigc::mem_fun(*this, &BlackWhite::settingChanged));
    ...
    enabledcc->show();
    enaccconn = enabledcc->signal_toggled().connect(
        sigc::mem_fun(*this, &BlackWhite::enabledcc_toggled));
    ...
    filterconn = filter->signal_changed().connect(
        sigc::mem_fun(*this, &BlackWhite::filterChanged));
    ...
    mixerRed = Gtk::manage(
        new Adjuster(/*M("TP_BWMIX_RED")*/ "", -100, 200, 1, 33, imgIcon[0]));
    ...
    mixerGreen = Gtk::manage(
        new Adjuster(/*M("TP_BWMIX_GREEN")*/ "", -100, 200, 1, 33, imgIcon[3]));
    ...
    mixerBlue = Gtk::manage(
        new Adjuster(/*M("TP_BWMIX_BLUE")*/ "", -100, 200, 1, 33, imgIcon[5]));
    ...
    algoconn =
        algo->signal_changed().connect(sigc::mem_fun(*this, &BlackWhite::algoChanged));

    mixerOrange = Gtk::manage(
        new Adjuster(/*M("TP_BWMIX_ORANGE")*/ "", -100, 200, 1, 33, imgIcon[1]));
    ...
    mixerYellow = Gtk::manage(
        new Adjuster(/*M("TP_BWMIX_YELLOW")*/ "", -100, 200, 1, 33, imgIcon[2]));
    ...
    mixerCyan = Gtk::manage(
        new Adjuster(/*M("TP_BWMIX_CYAN")*/ "", -100, 200, 1, 33, imgIcon[4]));
    ...
    mixerPurple = Gtk::manage(
        new Adjuster(/*M("TP_BWMIX_PURPLE")*/ "", -100, 200, 1, 33, imgIcon[6]));
    ...
    mixerMagenta = Gtk::manage(
        new Adjuster(/*M("TP_BWMIX_MAGENTA")*/ "", -100, 200, 1, 33, imgIcon[7]));
    ...
    gammaRed = Gtk::manage(
        new Adjuster(/*M("TP_BWMIX_GAM_RED")*/ "", -100, 100, 1, 0, imgIcon[8]));
    ...
    gammaGreen = Gtk::manage(
        new Adjuster(/*M("TP_BWMIX_GAM_GREEN")*/ "", -100, 100, 1, 0, imgIcon[9]));
    ...
    gammaBlue = Gtk::manage(
        new Adjuster(/*M("TP_BWMIX_GAM_BLUE")*/ "", -100, 100, 1, 0, imgIcon[10]));
    ...
    beforeCurveCEG =
        new CurveEditorGroup(options.lastBWCurvesDir, M("TP_BWMIX_CURVEEDITOR1"));
    beforeCurveCEG->setCurveListener(this);

    beforeCurve = static_cast<DiagonalCurveEditor *>(
        beforeCurveCEG->addCurve(CT_Diagonal, "", beforeCurveMode));
    ...
    tcmodeconn = beforeCurveMode->signal_changed().connect(
        sigc::mem_fun(*this, &BlackWhite::curveMode1Changed), true);
    ...
    */
    afterCurveCEG =
        new CurveEditorGroup(options.lastBWCurvesDir, M("TP_BWMIX_CURVEEDITOR2"));
    afterCurveCEG->setCurveListener(this);

    //  afterCurve =
    //  static_cast<DiagonalCurveEditor*>(afterCurveCEG->addCurve(CT_Diagonal, "",
    //  afterCurveMode));
    afterCurve =
        static_cast<DiagonalCurveEditor *>(afterCurveCEG->addCurve(CT_Diagonal, ""));
    ...
    //  tcmodeconn2 = afterCurveMode->signal_changed().connect( sigc::mem_fun(*this,
    //  &BlackWhite::curveMode1Changed2), true );
...
bool BlackWhite::BWComputed_()
{
    ...
    if (listener && nextcount <= 1) {
        // activated only 1 time, but perhaps in some cases if we want that all is
        // update pp3, auto, sliders : nextcount <= 2 but it cost time and result is
        // identical
...
void BlackWhite::read(const ProcParams *pp, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        ...
        mixerOrange->setEditedState(
            pedited->blackwhite.mixerOrange ? Edited : UnEdited);
        mixerYellow->setEditedState(
            pedited->blackwhite.mixerYellow ? Edited : UnEdited);
        mixerCyan->setEditedState(pedited->blackwhite.mixerCyan ? Edited : UnEdited);
        mixerMagenta->setEditedState(
            pedited->blackwhite.mixerMagenta ? Edited : UnEdited);
        mixerPurple->setEditedState(
            pedited->blackwhite.mixerPurple ? Edited : UnEdited);
...
void BlackWhite::write(ProcParams *pp, ParamsEdited *pedited)
{
    ...
    //  if      (tcMode == 0) pp->blackwhite.afterCurveMode =
    //  BlackWhiteParams::TCMode::STD_BW; else if (tcMode == 1)
    //  pp->blackwhite.afterCurveMode = BlackWhiteParams::TCMode::WEIGHTEDSTD;
    ...
    if (pedited) {
        ...
        pedited->blackwhite.filter =
            filter->get_active_text() != M("GENERAL_UNCHANGED");
        pedited->blackwhite.setting =
            setting->get_active_text() != M("GENERAL_UNCHANGED");
        pedited->blackwhite.method =
        ...
        pedited->blackwhite.beforeCurveMode =
            beforeCurveMode->get_active_row_number() != 4;
        ...
        //      afterCurveMode->get_active_row_number() != 1;
    }
...
void BlackWhite::curveMode1Changed()
{
    if (listener) {
        Glib::signal_idle().connect(
            sigc::mem_fun(*this, &BlackWhite::curveMode1Changed_));
    }
}
bool BlackWhite::curveMode1Changed_()
{
    if (listener) {
        listener->panelChanged(
            EvBWBeforeCurveMode, escapeHtmlChars(beforeCurveMode->get_active_text()));
    }
...
void BlackWhite::curveMode1Changed2 () {
    if (listener)  Glib::signal_idle().connect (sigc::mem_fun(*this,
&BlackWhite::curveMode1Changed2_));
}
bool BlackWhite::curveMode1Changed2_ () {
    if (listener) listener->panelChanged (EvBWAfterCurveMode,
escapeHtmlChars(afterCurveMode->get_active_text())); return false;
}
*/
void BlackWhite::colorForValue(double valX, double valY,
...
void BlackWhite::settingChanged()
{
    ...
    if (setting->get_active_row_number() == 10 ||
        setting->get_active_row_number() == 11) {
        // RGB Channel Mixer
        showMixer(3);
        hideEnabledCC();
        showFilter();
    } else if (setting->get_active_row_number() == 12 ||
               setting->get_active_row_number() == 13) {
    ...
    // Checking "listener" to avoid "autoch" getting toggled off because it has to
    // change the sliders when toggling on
    if (listener) {
...
void BlackWhite::setDefaults(const ProcParams *defParams, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        mixerRed->setDefaultEditedState(
            pedited->blackwhite.mixerRed ? Edited : UnEdited);
        mixerGreen->setDefaultEditedState(
            pedited->blackwhite.mixerGreen ? Edited : UnEdited);
        mixerBlue->setDefaultEditedState(
            pedited->blackwhite.mixerBlue ? Edited : UnEdited);
        gammaRed->setDefaultEditedState(
            pedited->blackwhite.gammaRed ? Edited : UnEdited);
        gammaGreen->setDefaultEditedState(
            pedited->blackwhite.gammaGreen ? Edited : UnEdited);
        gammaBlue->setDefaultEditedState(
            pedited->blackwhite.gammaBlue ? Edited : UnEdited);
        mixerOrange->setDefaultEditedState(
            pedited->blackwhite.mixerOrange ? Edited : UnEdited);
        mixerYellow->setDefaultEditedState(
            pedited->blackwhite.mixerYellow ? Edited : UnEdited);
        mixerCyan->setDefaultEditedState(
            pedited->blackwhite.mixerCyan ? Edited : UnEdited);
        mixerMagenta->setDefaultEditedState(
            pedited->blackwhite.mixerMagenta ? Edited : UnEdited);
        mixerPurple->setDefaultEditedState(
            pedited->blackwhite.mixerPurple ? Edited : UnEdited);
    } else {
...
void BlackWhite::adjusterChanged(Adjuster *a, double newval)
{
    // Checking "listener" to avoid "autoch" getting toggled off because it has to
    // change the sliders when toggling on
    if (listener && (a == mixerRed || a == mixerGreen || a == mixerBlue ||
                        a == mixerOrange || a == mixerYellow || a == mixerMagenta ||
                        a == mixerPurple || a == mixerCyan)) {
        if (multiImage && autoch->get_inconsistent()) {
            autoch->set_inconsistent(false);
        }
    ...
    if (a == mixerRed || a == mixerGreen || a == mixerBlue || a == mixerOrange ||
        a == mixerYellow || a == mixerCyan || a == mixerMagenta || a == mixerPurple)
...
void BlackWhite::updateRGBLabel()
{
    if (!batchMode) {
        ...
        Color::computeBWMixerConstants(sSetting, getFilterString(), getalgoString(),
            filcor, r, g, b, mixerOrange->getValue(), mixerYellow->getValue(),
            mixerCyan->getValue(), mixerPurple->getValue(), mixerMagenta->getValue(),
        ...
        RGBLabels->set_text(Glib::ustring::compose(M("TP_BWMIX_RGBLABEL"),
...
void BlackWhite::showGamma() { gammaFrame->show(); }
