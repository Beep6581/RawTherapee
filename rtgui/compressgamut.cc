...
Compressgamut::Compressgamut() :
    FoldableToolPanel(this, TOOL_NAME, M("TP_COMPRESSGAMUT_LABEL"), false, true)
{
    auto m = ProcEventMapper::getInstance();
    EvcgColorspace =
        m->newEvent(COMPR, "HISTORY_MSG_CG_COLORSPACE"); // FIRST to reinit histogram
    ...
    Gtk::Frame *iFrame =
        Gtk::manage(new Gtk::Frame(M("TP_COMPRESSGAMUT_MAIN_COLORSPACE")));
    ...
    iFrame->add(*iVBox);
    pack_start(*iFrame);
    colorspaceconn = colorspace->signal_changed().connect(
        sigc::mem_fun(*this, &Compressgamut::colorspaceChanged));
    ...
    th_c = Gtk::manage(new Adjuster(M("TP_COMPRESSGAMUT_CYANTH"), 0., 1., 0.001,
        0.815)); // 0.25 sRGB 0.999 to avoid: 1 - th = 0
    th_m = Gtk::manage(new Adjuster(
        M("TP_COMPRESSGAMUT_MAGENTATH"), 0., 1., 0.001, 0.803)); // 0.925 sRGB
    th_y = Gtk::manage(new Adjuster(
        M("TP_COMPRESSGAMUT_YELLOWTH"), 0., 1., 0.001, 0.880)); // 0.934 sRGB
    ...
    // cameras
    d_c = Gtk::manage(new Adjuster(
        M("TP_COMPRESSGAMUT_CYANLIM"), 1.001, 2.0, 0.001, 1.147)); // 1.05 sRGB
    d_m = Gtk::manage(new Adjuster(
        M("TP_COMPRESSGAMUT_MAGENTALIM"), 1.001, 2.0, 0.001, 1.264)); // 1.08 sRGB
    d_y = Gtk::manage(new Adjuster(
        M("TP_COMPRESSGAMUT_YELLOWLIM"), 1.001, 2.0, 0.001, 1.312)); // 1.10 sRGB
    ...
    rolloffconn = rolloff->signal_toggled().connect(
        sigc::mem_fun(*this, &Compressgamut::rolloff_change));
...
void Compressgamut::setDefaults(
