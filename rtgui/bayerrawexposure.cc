        ...
        this, TOOL_NAME, M("TP_EXPOS_BLACKPOINT_LABEL"), options.prevdemo != PD_Sidecar)
{
    ...
    PexBlack0->show();
    PextwoGreen =
        Gtk::manage(new CheckBox(M("TP_RAWEXPOS_TWOGREEN"), multiImage)); // two green
    PextwoGreen->set_active(true);
    PextwoGreen->setCheckBoxListener(this);

    Dehablack =
        Gtk::manage(new CheckBox(M("TP_RAWEXPOS_DEHA"), multiImage)); // Black dehaze
...
void BayerRAWExposure::read(
    const rtengine::procparams::ProcParams *pp, const ParamsEdited *pedited)
{
    disableListener();
    if (pedited) {
        PexBlack0->setEditedState(
            pedited->raw.bayersensor.exBlack0 ? Edited : UnEdited);
        PexBlack1->setEditedState(
            pedited->raw.bayersensor.exBlack1 ? Edited : UnEdited);
        PexBlack2->setEditedState(
            pedited->raw.bayersensor.exBlack2 ? Edited : UnEdited);
        PexBlack3->setEditedState(
            pedited->raw.bayersensor.exBlack3 ? Edited : UnEdited);
    }
...
void BayerRAWExposure::write(
    rtengine::procparams::ProcParams *pp, ParamsEdited *pedited)
{
    ...
    if (PextwoGreen->getLastActive()) {
        pp->raw.bayersensor.black3 =
            pp->raw.bayersensor.black0; // active or desactive 2 green together
    } else {
...
BayerRAWExposure::~BayerRAWExposure() { idle_register.destroy(); }

void BayerRAWExposure::autoBlackChanged(
    double reddeha, double greendeha, double bluedeha)
{

    idle_register.add([this, reddeha, greendeha, bluedeha]() -> bool {
...
void BayerRAWExposure::checkBoxToggled(CheckBox *c, CheckValue newval)
{
    if (c == PextwoGreen) {
        if (listener) {
            listener->panelChanged(EvPreProcessExptwoGreen,
                PextwoGreen->getLastActive() ? M("GENERAL_ENABLED")
                                             : M("GENERAL_DISABLED"));
            if (PextwoGreen->getLastActive()) {
    ...
    const rtengine::procparams::ProcParams *defParams, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        PexBlack0->setDefaultEditedState(
            pedited->raw.bayersensor.exBlack0 ? Edited : UnEdited);
        PexBlack1->setDefaultEditedState(
            pedited->raw.bayersensor.exBlack1 ? Edited : UnEdited);
        PexBlack2->setDefaultEditedState(
            pedited->raw.bayersensor.exBlack2 ? Edited : UnEdited);
        PexBlack3->setDefaultEditedState(
            pedited->raw.bayersensor.exBlack3 ? Edited : UnEdited);
