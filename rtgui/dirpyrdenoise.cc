    ...
    lastmedian(false)
{
    ...
    NoiscurveEditorG = new CurveEditorGroup(
        options.lastDenoiseCurvesDir, M("TP_DIRPYRDENOISE_LUMINANCE_CURVE"));
    ...
    ctboxC2->set_tooltip_markup(
        M("TP_DIRPYRDENOISE_CHROMINANCE_METHODADVANCED_TOOLTIP"));
    ...
    CCcurveEditorG = new CurveEditorGroup(
        options.lastDenoiseCurvesDir, M("TP_DIRPYRDENOISE_CHROMINANCE_CURVE"));
    ...
    passes = Gtk::manage(
        new Adjuster(M("TP_DIRPYRDENOISE_MEDIAN_PASSES"), 1.0, 3.0, 1., 1.));
...
void DirPyrDenoise::noiseTilePrev(
    int tileX, int tileY, int prevX, int prevY, int sizeT, int sizeP)
{
    nexttileX = tileX;
    nexttileY = tileY;
    nextprevX = prevX;
...
void DirPyrDenoise::setDefaults(
    const ProcParams *defParams, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        ...
        Ldetail->setDefaultEditedState(
            pedited->dirpyrDenoise.Ldetail ? Edited : UnEdited);
        chroma->setDefaultEditedState(
            pedited->dirpyrDenoise.chroma ? Edited : UnEdited);
        redchro->setDefaultEditedState(
            pedited->dirpyrDenoise.redchro ? Edited : UnEdited);
        bluechro->setDefaultEditedState(
            pedited->dirpyrDenoise.bluechro ? Edited : UnEdited);
        gamma->setDefaultEditedState(pedited->dirpyrDenoise.gamma ? Edited : UnEdited);
        passes->setDefaultEditedState(
            pedited->dirpyrDenoise.passes ? Edited : UnEdited);
    } else {
...
void DirPyrDenoise::setAdjusterBehavior(bool lumaadd, bool lumdetadd, bool chromaadd,
