...
Vibrance::Vibrance() :
    FoldableToolPanel(this, TOOL_NAME, M("TP_VIBRANCE_LABEL"), false, true)
{
    ...
    saturated =
        Gtk::manage(new Adjuster(M("TP_VIBRANCE_SATURATED"), -100., 100., 1., 0.));
    ...
    psThreshold = Gtk::manage(new ThresholdAdjuster(M("TP_VIBRANCE_PSTHRESHOLD"), -100.,
        100., 0., M("TP_VIBRANCE_PSTHRESHOLD_WEIGTHING"), 0, 0., 100., 75.,
        M("TP_VIBRANCE_PSTHRESHOLD_SATTHRESH"), 0, this, false));
    ...
    avoidColorShift =
        Gtk::manage(new Gtk::CheckButton(M("TP_VIBRANCE_AVOIDCOLORSHIFT")));
    ...
    curveEditorGG = new CurveEditorGroup(
        options.lastVibranceCurvesDir, M("TP_VIBRANCE_CURVEEDITOR_SKINTONES_LABEL"));
    curveEditorGG->setCurveListener(this);

    skinTonesCurve = static_cast<DiagonalCurveEditor *>(
        curveEditorGG->addCurve(CT_Diagonal, M("TP_VIBRANCE_CURVEEDITOR_SKINTONES")));
    ...
    skinTonesCurve->setRangeLabels(M("TP_VIBRANCE_CURVEEDITOR_SKINTONES_RANGE1"),
        M("TP_VIBRANCE_CURVEEDITOR_SKINTONES_RANGE2"),
        M("TP_VIBRANCE_CURVEEDITOR_SKINTONES_RANGE3"),
    ...
    pskinsconn = protectSkins->signal_toggled().connect(
        sigc::mem_fun(*this, &Vibrance::protectskins_toggled));
    ashiftconn = avoidColorShift->signal_toggled().connect(
        sigc::mem_fun(*this, &Vibrance::avoidcolorshift_toggled));
    pastsattogconn = pastSatTog->signal_toggled().connect(
        sigc::mem_fun(*this, &Vibrance::pastsattog_toggled));
}

Vibrance::~Vibrance() { delete curveEditorGG; }
...
void Vibrance::read(const ProcParams *pp, const ParamsEdited *pedited)
{
    ...
    if (lastPastSatTog) {
        ...
        saturated->setValue(
            pp->vibrance.saturated); // Pastels and Saturated are separate
    }
...
void Vibrance::autoOpenCurve() { skinTonesCurve->openIfNonlinear(); }
...
void Vibrance::write(ProcParams *pp, ParamsEdited *pedited)
{
    ...
    pp->vibrance.saturated =
        pastSatTog->get_active() ? pp->vibrance.pastels : saturated->getIntValue();
...
void Vibrance::adjusterChanged(ThresholdAdjuster *a, double newBottomLeft,
...
void Vibrance::adjusterChanged(ThresholdAdjuster *a, int newBottom, int newTop)
{
    if (listener && getEnabled()) {
        listener->panelChanged(
            EvVibrancePastSatThreshold, psThreshold->getHistoryString());
    }
}

void Vibrance::adjusterChanged(ThresholdAdjuster *a, int newBottomLeft, int newTopLeft,
    int newBottomRight, int newTopRight)
{
}

void Vibrance::adjusterChanged2(
...
void Vibrance::setDefaults(const ProcParams *defParams, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        ...
        saturated->setDefaultEditedState(
            pedited->vibrance.saturated ? Edited : UnEdited);
        psThreshold->setDefaultEditedState(
            pedited->vibrance.psthreshold ? Edited : UnEdited);
...
std::vector<double> Vibrance::getCurvePoints(ThresholdSelector *tAdjuster) const
{
    ...
    tAdjuster->getPositions<double>(
        transitionWeighting, threshold); // ( range -100;+100,   range 0;+100 )
    ...
    // Y value of the chroma mean point, calculated to get a straight line between p2
    // and s0
    ...
    // point at the location of the Top cursor, at the end of the first linear
    // transition and the beginning of the second one
