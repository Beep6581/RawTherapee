...
class Thumbnail
{
    ...
    int enqueueNumber; // the number of instances in the batch queue corresponding to
                       // this thumbnail

    // if the thumbnail is in processed mode, this class holds its data:
    rtengine::Thumbnail *tpp;
    int tw, th;     // dimensions of timgdata (it stores tpp->width and tpp->height in
                    // processed mode for simplicity)
    float imgRatio; // hack to avoid rounding error
    //  double          scale;              // portion of the sizes of the processed
    //  thumbnail image and the full scale image

    const std::unique_ptr<rtengine::procparams::ProcParams> pparams;
    bool pparamsValid;
    bool imageLoading;

    // these are the data of the result image of the last getthumbnailimage  call (for
    // caching purposes)
    ...
    Glib::ustring getCacheFileName(
        const Glib::ustring &subdir, const Glib::ustring &fext) const;
...
public:
    ...
    Thumbnail(CacheManager *cm, const Glib::ustring &fname, const std::string &md5,
        const std::string &xmpSidecarMd5);
    ~Thumbnail();
    ...
    // Use this to create params on demand for update ; if flaggingMode=true, the
    // procparams is created for a file being flagged (inTrash, rank, colorLabel)
    rtengine::procparams::ProcParams *createProcParamsForUpdate(
        bool returnParams, bool force, bool flaggingMode = false);

    void setProcParams(const rtengine::procparams::ProcParams &pp,
        ParamsEdited *pe = nullptr, int whoChangedIt = -1, bool updateCacheNow = true,
    ...
    //        unsigned char*  getThumbnailImage (int &w, int &h, int fixwh=1); // fixwh
    //        = 0: fix w and calculate h, =1: fix h and calculate w
    rtengine::IImage8 *processThumbImage(
        const rtengine::procparams::ProcParams &pparams, int h, double &scale);
    rtengine::IImage8 *upgradeThumbImage(
        const rtengine::procparams::ProcParams &pparams, int h, double &scale,
        bool forceUpgrade);
    void getThumbnailSize(
        int &w, int &h, const rtengine::procparams::ProcParams *pparams = nullptr);
    ...
    void getCamWB(
        double &temp, double &green, rtengine::StandardObserver observer) const;
