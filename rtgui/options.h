...
struct ExternalEditor {
    ExternalEditor();
    ExternalEditor(const Glib::ustring &name, const Glib::ustring &command,
...
struct SaveFormat {
    SaveFormat(const Glib::ustring &_format, int _png_bits, int _jpeg_quality,
        int _jpeg_sub_samp, int _tiff_bits, bool _tiff_float, bool _tiff_uncompressed,
        bool _big_tiff, bool _save_params) :
        format(_format), pngBits(_png_bits), jpegQuality(_jpeg_quality),
        jpegSubSamp(_jpeg_sub_samp), tiffBits(_tiff_bits), tiffFloat(_tiff_float),
        tiffUncompressed(_tiff_uncompressed), bigTiff(_big_tiff),
        saveParams(_save_params)
    {
    }
    SaveFormat(
        const Glib::ustring &_format, int _png_bits, int _tiff_bits, bool _tiff_float) :
        SaveFormat(
            _format, _png_bits, 90, 2, _tiff_bits, _tiff_float, true, false, true)
    {
    }
...
public:
    ...
    std::vector<std::string> knownExtensions = {"3fr", "arw", "arq", "cr2", "cr3",
        "crf", "crw", "dcr", "dng", "fff", "iiq", "jpg", "jpeg", "jxl", "kdc", "mef",
        "mos", "mrw", "nef", "nrw", "orf", "ori", "pef", "png", "raf", "raw", "rw2",
    ...
    int maxInspectorBuffers;   // maximum number of buffers (i.e. images) for the
                               // Inspector feature
    int inspectorDelay;
    int clutCacheSize;
