    ...
    listener(nullptr), ornamentSurface(new RTSurface("ornament1.svg"))
{
    ...
    use_fast_pipeline = Gtk::manage(
        new Gtk::RadioButton(pipeline_group, M("EXPORT_USE_FAST_PIPELINE")));
    ...
    bypass_sharpening =
        Gtk::manage(new Gtk::CheckButton(M("EXPORT_BYPASS_SHARPENING")));
    bypass_sharpenEdge =
    ...
    Gtk::Frame *bayerFrame =
        Gtk::manage(new Gtk::Frame(M("TP_RAW_SENSOR_BAYER_LABEL")));
    ...
    lblbox->pack_start(
        *Gtk::manage(new Gtk::Label(M("EXPORT_PIPELINE"))), Gtk::PACK_SHRINK, 4);
    ...
    bypass_box->pack_start(
        *Gtk::manage(new Gtk::Separator(Gtk::ORIENTATION_HORIZONTAL)), Gtk::PACK_SHRINK,
        4);
    lblbox = Gtk::manage(new Gtk::Box());
    lblbox->pack_start(
        *Gtk::manage(new Gtk::Label(M("EXPORT_BYPASS"))), Gtk::PACK_SHRINK, 4);
    ...
    rmbox->pack_start(
        *Gtk::manage(new Gtk::Label(M("TP_RESIZE_LABEL"))), Gtk::PACK_SHRINK, 4);
    ...
    wbox->pack_start(
        *Gtk::manage(new Gtk::Label(M("EXPORT_MAXWIDTH"))), Gtk::PACK_SHRINK, 4);
    wbox->pack_start(*MaxWidth);
    hbox->pack_start(
        *Gtk::manage(new Gtk::Label(M("EXPORT_MAXHEIGHT"))), Gtk::PACK_SHRINK, 4);
    ...
    bypass_defringeConn = bypass_defringe->signal_toggled().connect(sigc::bind(
        sigc::mem_fun(*bypass_ALL, &Gtk::CheckButton::set_inconsistent), true));
    bypass_dirpyrDenoiseConn =
        bypass_dirpyrDenoise->signal_toggled().connect(sigc::bind(
            sigc::mem_fun(*bypass_ALL, &Gtk::CheckButton::set_inconsistent), true));
...
#define FE_OPT_STORE_(n, v)                                                            \
    ...
    if (currentRow >= 0) {
        FE_OPT_STORE_(options.fastexport_raw_xtrans_method,
    ...
    FE_OPT_STORE_(
        options.fastexport_use_fast_pipeline, use_fast_pipeline->get_active());
#undef FE_OPT_STORE_
...
void ExportPanel::LoadDefaultSettings()
{
    ...
    bypass_raw_bayer_linenoise->set_active(
        options.fastexport_bypass_raw_bayer_linenoise);
    ...
    for (size_t i = 0;
        i < procparams::RAWParams::XTransSensor::getMethodStrings().size(); ++i)
