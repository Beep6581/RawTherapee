...
void PreviewHandler::setImage(
    rtengine::IImage8 *i, double scale, const rtengine::procparams::CropParams &cp)
{
    pih->pending++;
    ...
    int x, int y, int w, int h, double zoom)
{
    ...
    if (previewImg) {
        ...
        previewImg->scale(
            resPixbuf, 0, 0, w, h, -x, -y, totalZoom, totalZoom, Gdk::INTERP_NEAREST);
    }
...
Glib::RefPtr<Gdk::Pixbuf> PreviewHandler::getRoughImage(
    int desiredW, int desiredH, double &zoom_)
{
    MyMutex::MyLock lock(previewImgMutex);
    ...
    if (previewImg) {
        ...
        previewImg->scale(resPixbuf, 0, 0, previewImg->get_width() * zoom,
            previewImg->get_height() * zoom, 0, 0, zoom, zoom, Gdk::INTERP_BILINEAR);
...
rtengine::procparams::CropParams PreviewHandler::getCropParams() { return *cropParams; }
