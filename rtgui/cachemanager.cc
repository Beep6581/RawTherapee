...
void CacheManager::init()
{
    ...
    for (const auto &cacheDir : cacheDirs) {
        error |= g_mkdir_with_parents(
            Glib::build_filename(baseDir, cacheDir).c_str(), cacheDirMode);
    }

    if (error != 0 && rtengine::settings->verbose) {
        std::cerr << "Failed to create all cache directories: " << g_strerror(errno)
                  << std::endl;
    }
}
...
Thumbnail *CacheManager::getEntry(const Glib::ustring &fname)
{
    ...
    const auto xmpSidecarMd5 = rtengine::settings->metadata_xmp_sync !=
...
void CacheManager::renameEntry(const std::string &oldfilename,
    const std::string &oldmd5, const std::string &newfilename)
{
    MyMutex::MyLock lock(mutex);
    ...
    auto error = g_rename(
        getCacheFileName("profiles", oldfilename, paramFileExtension, oldmd5).c_str(),
        getCacheFileName("profiles", newfilename, paramFileExtension, newmd5).c_str());
    error |= g_rename(getCacheFileName("images", oldfilename, ".rtti", oldmd5).c_str(),
        getCacheFileName("images", newfilename, ".rtti", newmd5).c_str());
    error |=
        g_rename(getCacheFileName("embprofiles", oldfilename, ".icc", oldmd5).c_str(),
            getCacheFileName("embprofiles", newfilename, ".icc", newmd5).c_str());
    error |= g_rename(getCacheFileName("data", oldfilename, ".txt", oldmd5).c_str(),
        getCacheFileName("data", newfilename, ".txt", newmd5).c_str());

    if (error != 0 && rtengine::settings->verbose) {
        std::cerr << "Failed to rename all files for cache entry '" << oldfilename
...
void CacheManager::deleteDir(const Glib::ustring &dirName) const
{
    try {
        ...
        if (error != 0 && rtengine::settings->verbose) {
            std::cerr << "Failed to delete all entries in cache directory '" << dirName
                      << "': " << g_strerror(errno) << std::endl;
        }
...
void CacheManager::deleteFiles(const Glib::ustring &fname, const std::string &md5,
    bool purgeData, bool purgeProfile) const
{
    ...
    if (error != 0 && rtengine::settings->verbose) {
        std::cerr << "Failed to delete all files for cache entry '" << fname
                  << "': " << g_strerror(errno) << std::endl;
    }
}
...
std::string CacheManager::getMD5(const Glib::ustring &fname)
{
    ...
    if (GetFileAttributesExW(wfname.get(), GetFileExInfoStandard, &fileAttr)) {
        ...
        const auto identifier = Glib::ustring::compose("%1-%2-%3-%4",
...
void CacheManager::applyCacheSizeLimitation() const
{
    ...
    try {
        const auto dir =
            Gio::File::create_for_path(Glib::build_filename(baseDir, "data"));
    ...
    const std::size_t toDelete =
        files.size() - options.maxCacheEntries +
        options.maxCacheEntries * 5 / 100; // reserve 5% free cache space

    std::nth_element(files.begin(), files.begin() + toDelete, files.end(),
        [](const FNameMTime &lhs, const FNameMTime &rhs) -> bool {
            return lhs.second < rhs.second;
        });

    for (std::vector<FNameMTime>::const_iterator entry = files.begin(),
                                                 end = files.begin() + toDelete;
        entry != end; ++entry) {
...
void CacheManager::updateImageInfo(const Glib::ustring &fname,
    CacheImageData &imageData, const Glib::ustring &xmpSidecarMd5) const
{
