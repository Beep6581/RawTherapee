...
public:
    struct Job {
        Job(ThumbBrowserEntryBase *tbe, bool *priority, bool upgrade, bool forceUpgrade,
            ThumbImageUpdateListener *listener) :
            tbe_(tbe),
            /*pparams_(pparams),
            height_(height), */
            priority_(priority), upgrade_(upgrade), force_upgrade_(forceUpgrade),
            listener_(listener)
        {
        }

        Job() :
            tbe_(nullptr), priority_(nullptr), upgrade_(false), force_upgrade_(false),
            listener_(nullptr)
        {
        }
    ...
    // This is the only exceptions along with GThreadMutex (guiutils.cc), MyMutex is
    ...
    void processNextJob()
    {
...

        {
            ...
            if (i == jobs_.end()) {
                i = jobs_.begin();
                DEBUG(
                    "processing(first) %s", i->tbe_->thumbnail->getFileName().c_str());
            }
        ...
        if (j.upgrade_) {
            if (thm->isQuick() || j.force_upgrade_) {
                img = thm->upgradeThumbImage(thm->getProcParams(),
                    j.tbe_->getPreviewHeight(), scale, j.force_upgrade_);
            }
        } else {
            img = thm->processThumbImage(
                thm->getProcParams(), j.tbe_->getPreviewHeight(), scale);
        }
...
void ThumbImageUpdater::add(ThumbBrowserEntryBase *tbe, bool *priority, bool upgrade,
