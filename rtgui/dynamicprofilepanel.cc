...
DynamicProfilePanel::EditDialog::EditDialog(
    const Glib::ustring &title, Gtk::Window &parent) : Gtk::Dialog(title, parent)
{
    profilepath_ = Gtk::manage(new ProfileStoreComboBox());
    Gtk::Box *hb = Gtk::manage(new Gtk::Box());
    hb->pack_start(
        *Gtk::manage(new Gtk::Label(M("DYNPROFILEEDITOR_PROFILE"))), false, false, 4);
    ...
    hb->pack_start(
        *Gtk::manage(new Gtk::Label(M("EXIFFILTER_IMAGETYPE"))), false, false, 4);
...
void DynamicProfilePanel::EditDialog::add_range(
...
void DynamicProfilePanel::EditDialog::add_optional(
...
DynamicProfilePanel::DynamicProfilePanel() :
    vbox_(Gtk::ORIENTATION_VERTICAL), button_up_(M("DYNPROFILEEDITOR_MOVE_UP")),
    button_down_(M("DYNPROFILEEDITOR_MOVE_DOWN")),
    button_new_(M("DYNPROFILEEDITOR_NEW")), button_edit_(M("DYNPROFILEEDITOR_EDIT")),
    button_delete_(M("DYNPROFILEEDITOR_DELETE"))
{
    ...
    if (col) {
        col->set_cell_data_func(
            *cell, sigc::mem_fun(*this, &DynamicProfilePanel::render_profilepath));
    }

    cell = Gtk::manage(new Gtk::CellRendererText());
    cols_count = treeview_.append_column(M("EXIFFILTER_CAMERA"), *cell);
    col = treeview_.get_column(cols_count - 1);

    if (col) {
        col->set_cell_data_func(
            *cell, sigc::mem_fun(*this, &DynamicProfilePanel::render_camera));
    }
    ...
    if (col) {
        col->set_cell_data_func(
            *cell, sigc::mem_fun(*this, &DynamicProfilePanel::render_lens));
    }
    ...
    if (col) {
        col->set_cell_data_func(
            *cell, sigc::mem_fun(*this, &DynamicProfilePanel::render_path));
    }
    ...
    if (col) {
        col->set_cell_data_func(
            *cell, sigc::mem_fun(*this, &DynamicProfilePanel::render_imagetype));
    }
    ...
    if (col) {
        col->set_cell_data_func(
            *cell, sigc::mem_fun(*this, &DynamicProfilePanel::render_iso));
    }
    ...
    if (col) {
        col->set_cell_data_func(
            *cell, sigc::mem_fun(*this, &DynamicProfilePanel::render_fnumber));
    }
    ...
    if (col) {
        col->set_cell_data_func(
            *cell, sigc::mem_fun(*this, &DynamicProfilePanel::render_focallen));
    }
    ...
    if (col) {
        col->set_cell_data_func(
            *cell, sigc::mem_fun(*this, &DynamicProfilePanel::render_shutterspeed));
    }

    cell = Gtk::manage(new Gtk::CellRendererText());
    cols_count = treeview_.append_column(M("EXIFFILTER_EXPOSURECOMPENSATION"), *cell);
    col = treeview_.get_column(cols_count - 1);

    if (col) {
        col->set_cell_data_func(
            *cell, sigc::mem_fun(*this, &DynamicProfilePanel::render_expcomp));
    }
...
void DynamicProfilePanel::update_rule(
    Gtk::TreeModel::Row row, const DynamicProfileRule &rule)
{
    ...
    Gtk::CellRenderer *cell, const Gtk::TreeModel::iterator &iter)
{
    RENDER_RANGE_(double, fnumber, [](double f) {
        return std::string("f/") + rtengine::FramesMetaData::apertureToString(f);
    });
}
...
void DynamicProfilePanel::render_shutterspeed(
    Gtk::CellRenderer *cell, const Gtk::TreeModel::iterator &iter)
{
    RENDER_RANGE_(double, shutterspeed, rtengine::FramesMetaData::shutterToString);
}
...
{
    EditDialog d(
        M("DYNPROFILEEDITOR_NEW_RULE"), static_cast<Gtk::Window &>(*get_toplevel()));
    int status = d.run();
...
void DynamicProfilePanel::on_button_edit()
{
    ...
    EditDialog d(
        M("DYNPROFILEEDITOR_EDIT_RULE"), static_cast<Gtk::Window &>(*get_toplevel()));
