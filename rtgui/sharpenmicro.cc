    ...
    FoldableToolPanel(this, TOOL_NAME, M("TP_SHARPENMICRO_LABEL"), true, true)
{
    ...
    uniformity =
        Gtk::manage(new Adjuster(M("TP_SHARPENMICRO_UNIFORMITY"), 0, 10, 1, 5));
...
void SharpenMicro::read(const ProcParams *pp, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        ...
        uniformity->setEditedState(
            pedited->sharpenMicro.uniformity ? Edited : UnEdited);
    }
...
void SharpenMicro::setDefaults(const ProcParams *defParams, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        ...
        contrast->setDefaultEditedState(
            pedited->sharpenMicro.contrast ? Edited : UnEdited);
        uniformity->setDefaultEditedState(
            pedited->sharpenMicro.uniformity ? Edited : UnEdited);
    } else {
...
void SharpenMicro::setAdjusterBehavior(
    bool amountadd, bool contrastadd, bool uniformityadd)
{
