...
ImageArea::ImageArea(ImageAreaPanel *p) :
    ...
    set_can_focus(true);
    // grab_focus(); // Try to grab focus immediately, debug
...
// added to below in order to get shortcut d for detail window working.
void ImageArea::on_realize() {
    Gtk::DrawingArea::on_realize();
    // Add all the necessary events
    ...
    set_can_focus(true); // Make sure widget can receive focus
    grab_focus();        // Try to grab focus immediately, needs to be here!


#if defined(__APPLE__)
    // Workaround: disabling POINTER_MOTION_HINT_MASK as for gtk 2.24.22 the
    // get_pointer() function is buggy for quartz and modifier mask is not updated
    // correctly. This workaround should be removed when bug is fixed in GTK2 or when
    // migrating to GTK3
...
void ImageArea::on_resized(Gtk::Allocation &req) {
    if (ipc && get_width() > 1) { // sometimes on_resize is called in some init state,
                                  // causing wrong sizes
        if (!mainCropWindow) {
...
rtengine::StagedImageProcessor *ImageArea::getImProcCoordinator() const { return ipc; }
...
bool ImageArea::on_button_press_event(GdkEventButton *event) {
    if (focusGrabber) {
        focusGrabber->buttonPress(
            event->button, event->type, event->state, event->x, event->y);
    } else {
        CropWindow *cw = getCropWindow(event->x, event->y);
...
void ImageArea::subscribe(EditSubscriber *subscriber) {
    ...
    if (subscriber && subscriber->getEditingType() == ET_OBJECTS) {
        // In this case, no need to reprocess the image, so we redraw the image to
...
void ImageArea::addCropWindow() {
    g_print("addCropWindow called\n");
    if (!mainCropWindow) {
        return; // if called but no image is loaded, it would crash
    }
    g_print("Creating new crop window\n");
    ...
    if (lastWidth <= 0) { // this is only the case for very first start of RT 4.1 or
                          // when options file is deleted
        cropwidth = 200;
        cropheight = 200;
    } else {
        cropwidth = lastWidth;
...
void ImageArea::cropWindowSizeChanged(CropWindow *cw) { syncBeforeAfterViews(); }
