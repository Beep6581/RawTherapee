...
/**
 *  This class handle the displayed part of the image, ask for the initial data and
 * process it so it can display it. Its position on the preview is handled not set by
 * this class but by the CropHandlerListener (i.e. CropWindow) with which it works
 * closely.
 */
...
public:
    ...
    void colorPick(const rtengine::Coord &pickerPos, float &r, float &g, float &b,
        float &rpreview, float &gpreview, float &bpreview,
    ...
    void setDetailedCrop(rtengine::IImage8 *im, rtengine::IImage8 *imworking,
        ...
        const rtengine::procparams::CropParams &cp, int cx, int cy, int cw, int ch,
        int skip) override;
    ...
    Glib::RefPtr<Gdk::Pixbuf>
        cropPixbuf; // image displayed on monitor, using the monitor profile (i.e. lab
                    // to monitor profile)
    Glib::RefPtr<Gdk::Pixbuf>
        cropPixbuftrue; // internal image in output color space for analysis (i.e. lab
                        // to either Working profile or Output profile, depending on
                        // options.rtSettings.HistogramWorking)
...
private:
    ...
    int zoom; // scale factor (e.g. 5 if 1:5 scale) ; if 1:1 scale and bigger, factor is
              // multiplied by 1000  (i.e. 1000 for 1:1 scale, 2000 for 2:1, etc...)
    int ww, wh;   // size of the crop's canvas on the screen ; might be bigger than the
                  // displayed image, but not smaller
    int cax, cay; // clamped crop anchor's coordinate, i.e. point of the image that
                  // coincide to the center of the display area, expressed in image
                  // coordinates; cannot be outside the image's bounds; but if
                  // cax==cay==-1, designate the center of the image
    int cx, cy, cw, ch; // position and size of the requested crop ; position expressed
                        // in image coordinates, so cx and cy might be negative and cw
                        // and ch higher than the image's 1:1 size
    int cropX, cropY, cropW,
        cropH; // cropPixbuf's displayed area (position and size), i.e. coordinates in
               // 1:1 scale, i.e. cx, cy, cw & ch trimmed to the image's bounds
    bool enabled;
