    ...
    currTransform(0), fromRaw(false)
{
    if (!imagePath.empty() && Glib::file_test(imagePath, Glib::FILE_TEST_EXISTS) &&
        ...
        rtengine::PreviewImage pi(
            imagePath, ext, rtengine::PreviewImage::PIM_EmbeddedOrRaw);
...
Inspector::Inspector() :
    currImage(nullptr), scaled(false), scale(1.0), zoomScale(1.0), zoomScaleBegin(1.0),
    active(false), pinned(false), dirty(false), fullscreen(true), keyDown(false),
    windowShowing(false)
{
    set_name("Inspector");
...
bool Inspector::on_key_release(GdkEventKey *event)
{
    ...
    if (!pinned) {
        switch (event->keyval) {
            case GDK_KEY_f: // Shortcut f: Inspector zoom to 100 %
            case GDK_KEY_F: // Shortcut F: Inspector zoom to 100 %
                zoomScale = 1.0;
                window->set_visible(false);
                return true;
...
bool Inspector::on_key_press(GdkEventKey *event)
{
    ...
    switch (event->keyval) {
        case GDK_KEY_z: // Shortcut z: Inspector no-op
        case GDK_KEY_F: // Shortcut F: Inspector zoom to 100 %
        ...
        case GDK_KEY_f: // Shortcut f: Inspector zoom to 100 %
            ...
            queue_draw();
            return true;
        case GDK_KEY_F11: // Shortcut F11: Inspector toggle fullscreen
            if (fullscreen) window->unfullscreen();
            else
                window->fullscreen();
            fullscreen = !fullscreen;
            return true;
        case GDK_KEY_Escape: // Shortcut Escape: Inspector hide window
            // hide window
            zoomScale = 1.0;
            window->set_visible(false);
            return true;
...
void Inspector::on_window_hide() { windowShowing = false; }
...
bool Inspector::on_draw(const ::Cairo::RefPtr<Cairo::Context> &cr)
{
    ...
    if (currImage && currImage->imgBuffer.surfaceCreated()) {
        ...
        // printf("center: %d, %d   (img: %d, %d)  (availableSize: %d, %d)  (topLeft:
        ...
        currImage->imgBuffer.setDrawRectangle(win, dest.x, dest.y,
            rtengine::min<int>(
        ...
        if (!window || (deviceScale == 1 && !scaledImage)) {
            ...
            if (!scaledImage) {
                ...
                crop = crop->scale_simple(
                    round(viewW * scale), round(viewH * scale), Gdk::INTERP_BILINEAR);
...
void Inspector::get_preferred_height_vfunc(
...
void Inspector::get_preferred_height_for_width_vfunc(
    int width, int &minimum_height, int &natural_height) const
{
    get_preferred_height_vfunc(minimum_height, natural_height);
}

void Inspector::get_preferred_width_for_height_vfunc(
