    ...
    FoldableToolPanel(this, TOOL_NAME, M("TP_PRSHARPENING_LABEL"), false, true)
{
    ...
    diter =
        Gtk::manage(new Adjuster(M("TP_SHARPENING_RLD_ITERATIONS"), 5, 100, 1, 100));
    ...
    method->signal_changed().connect(
        sigc::mem_fun(*this, &PrSharpening::method_changed));
}
...
void PrSharpening::read(const ProcParams *pp, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        ...
        ddamping->setEditedState(
            pedited->prsharpening.deconvdamping ? Edited : UnEdited);
...
void PrSharpening::setDefaults(const ProcParams *defParams, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        contrast->setDefaultEditedState(
            pedited->prsharpening.contrast ? Edited : UnEdited);
        ...
        threshold->setDefaultEditedState(
            pedited->prsharpening.threshold ? Edited : UnEdited);
        eradius->setDefaultEditedState(
            pedited->prsharpening.edges_radius ? Edited : UnEdited);
        etolerance->setDefaultEditedState(
            pedited->prsharpening.edges_tolerance ? Edited : UnEdited);
        hcamount->setDefaultEditedState(
            pedited->prsharpening.halocontrol_amount ? Edited : UnEdited);
        damount->setDefaultEditedState(
            pedited->prsharpening.deconvamount ? Edited : UnEdited);
        dradius->setDefaultEditedState(
            pedited->prsharpening.deconvradius ? Edited : UnEdited);
        diter->setDefaultEditedState(
            pedited->prsharpening.deconviter ? Edited : UnEdited);
...
void PrSharpening::adjusterChanged(
    ThresholdAdjuster *a, double newBottom, double newTop)
{
}

void PrSharpening::adjusterChanged(ThresholdAdjuster *a, double newBottomLeft,
...
void PrSharpening::adjusterChanged(ThresholdAdjuster *a, int newBottomLeft,
...
void PrSharpening::adjusterChanged2(
...
void PrSharpening::setAdjusterBehavior(bool contrastadd, bool radiusadd, bool amountadd,
