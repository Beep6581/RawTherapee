...
static double wbSlider2Temp(double sval)
{
    ...
    if (sval <= 5003) {
        ...
        const double slope =
            (double)(CENTERTEMP0 - MINTEMP0) / (MAXTEMP0 - CENTERTEMP0);
...
ColorAppearance::ColorAppearance() :
    FoldableToolPanel(this, TOOL_NAME, M("TP_COLORAPP_LABEL"), false, true)
{
    ...
    complexmethodconn = complexmethod->signal_changed().connect(
        sigc::mem_fun(*this, &ColorAppearance::complexmethodChanged));
    ...
    modelmethodconn = modelmethod->signal_changed().connect(
        sigc::mem_fun(*this, &ColorAppearance::modelmethodChanged));
    modelmethod->set_tooltip_text(M("TP_COLORAPP_MODELCAT_TOOLTIP"));
    Gtk::Box *const modelHBox = Gtk::manage(new Gtk::Box());
    Gtk::Label *const modelLabel =
        Gtk::manage(new Gtk::Label(M("TP_COLORAPP_MODELCAT") + ":"));
    ...
    catmethodconn = catmethod->signal_changed().connect(
        sigc::mem_fun(*this, &ColorAppearance::catmethodChanged));
    catmethod->set_tooltip_text(M("TP_COLORAPP_CATMET_TOOLTIP"));
    Gtk::Box *const catHBox = Gtk::manage(new Gtk::Box());
    Gtk::Label *const catLabel =
        Gtk::manage(new Gtk::Label(M("TP_COLORAPP_CATMOD") + ":"));
    ...
    degree =
        Gtk::manage(new Adjuster(M("TP_COLORAPP_CIECAT_DEGREE"), 0., 100., 1., 90.));
    ...
    Gtk::Label *surrLabel1 =
        Gtk::manage(new Gtk::Label(M("TP_COLORAPP_SURROUNDSRC") + ":"));
    ...
    tempsc = Gtk::manage(new Adjuster(M("TP_WBALANCE_TEMPERATURE"), MINTEMP0, MAXTEMP0,
        5, CENTERTEMP0, itempL, itempR, &wbSlider2Temp, &wbTemp2Slider));
    greensc = Gtk::manage(new Adjuster(
        M("TP_WBALANCE_GREEN"), MINGREEN0, MAXGREEN0, 0.001, 1.0, igreenL, igreenR));
    ...
    //   adapscen = Gtk::manage (new Adjuster (M ("TP_COLORAPP_ABSOLUTELUMINANCE"),
    //   0.01, 16384., 0.001, 2000.)); // EV -7  ==> EV 17 adapscen = Gtk::manage (new
    //   Adjuster (M ("TP_COLORAPP_ABSOLUTELUMINANCE"), MINLA0, MAXLA0, 0.01, 1997.4,
    //   NULL, NULL, &wbSlider2la, &wbla2Slider));
    adapscen = Gtk::manage(new Adjuster(M("TP_COLORAPP_ABSOLUTELUMINANCE"), MINLA0,
        MAXLA0, 0.01, 2000.)); //, NULL, NULL, &wbSlider2la, &wbla2Slider));
    ...
    expadjust->signal_button_release_event().connect_notify(
        sigc::bind(sigc::mem_fun(this, &ColorAppearance::foldAllButMe), expadjust));
    /*
        Gtk::Frame *p2Frame;
        // Vertical box container for the content of the Process 1 frame
        Gtk::Box* p2VBox;
    ...
    */
    ...
    p2VBox->pack_start(*Gtk::manage(new Gtk::Separator(Gtk::ORIENTATION_HORIZONTAL)),
    ...
    qbright =
        Gtk::manage(new Adjuster(M("TP_COLORAPP_BRIGHT"), -100.0, 100.0, 0.1, 0.));

    //   qbright->setDelay(std::max(options.adjusterMinDelay,
    ...
    schroma =
        Gtk::manage(new Adjuster(M("TP_COLORAPP_CHROMA_S"), -100.0, 100.0, 0.1, 0.));
    ...
    mchroma =
        Gtk::manage(new Adjuster(M("TP_COLORAPP_CHROMA_M"), -100.0, 100.0, 0.1, 0.));
    ...
    //  rstprotection->setDelay(std::max(options.adjusterMinDelay,
    //  options.adjusterMaxDelay));
    ...
    contrast =
        Gtk::manage(new Adjuster(M("TP_COLORAPP_CONTRAST"), -100.0, 100.0, 0.1, 0.));

    //  contrast->setDelay(std::max(options.adjusterMinDelay,
    //  options.adjusterMaxDelay));
    ...
    qcontrast =
        Gtk::manage(new Adjuster(M("TP_COLORAPP_CONTRAST_Q"), -100.0, 100.0, 0.1, 0.));

    //   qcontrast->setDelay(std::max(options.adjusterMinDelay,
    //   options.adjusterMaxDelay));
    ...
    tonecieconn = tonecie->signal_toggled().connect(
        sigc::mem_fun(*this, &ColorAppearance::tonecie_toggled));
    ...
    /*
        ...
        sharpcieconn = sharpcie->signal_toggled().connect( sigc::mem_fun(*this,
       &ColorAppearance::sharpcie_toggled) ); p2VBox->pack_start (*sharpcie);
    */
    p2VBox->pack_start(*Gtk::manage(new Gtk::Separator(Gtk::ORIENTATION_HORIZONTAL)),
        Gtk::PACK_EXPAND_WIDGET, 4);
    ...
    tcmodeconn = toneCurveMode->signal_changed().connect(
        sigc::mem_fun(*this, &ColorAppearance::curveMode1Changed), true);
    ...
    curveEditorG2 =
        new CurveEditorGroup(options.lastToneCurvesDir, M("TP_COLORAPP_CURVEEDITOR2"));
    ...
    //    shape2 = static_cast<DiagonalCurveEditor*> (curveEditorG2->addCurve
    //    (CT_Diagonal, "J(J)"));

    tcmode2conn = toneCurveMode2->signal_changed().connect(
        sigc::mem_fun(*this, &ColorAppearance::curveMode2Changed), true);
    ...
    curveEditorG3 =
        new CurveEditorGroup(options.lastToneCurvesDir, M("TP_COLORAPP_CURVEEDITOR3"));
    curveEditorG3->setCurveListener(this);

    shape3 = static_cast<DiagonalCurveEditor *>(
        curveEditorG3->addCurve(CT_Diagonal, "", toneCurveMode3));
    shape3->setRangeLabels(M("TP_LABCURVE_CURVEEDITOR_CC_RANGE1"),
        M("TP_LABCURVE_CURVEEDITOR_CC_RANGE2"), M("TP_LABCURVE_CURVEEDITOR_CC_RANGE3"),
        M("TP_LABCURVE_CURVEEDITOR_CC_RANGE4"));
    ...
    tcmode3conn = toneCurveMode3->signal_changed().connect(
        sigc::mem_fun(*this, &ColorAppearance::curveMode3Changed), true);
    ...
    datacieconn = datacie->signal_toggled().connect(
        sigc::mem_fun(*this, &ColorAppearance::datacie_toggled));
    ...
    p3Frame = Gtk::manage(new Gtk::Frame(
        M("TP_COLORAPP_LABEL_VIEWING"))); // "Editing viewing conditions" ???
    ...
    adaplum = Gtk::manage(new Adjuster(M("TP_COLORAPP_ABSOLUTELUMINANCE"), MINLA0,
        MAXLA0, 0.01, 16.)); //, NULL, NULL, &wbSlider2la, &wbla2Slider));
    ...
    degreeout =
        Gtk::manage(new Adjuster(M("TP_COLORAPP_CIECAT_DEGREEOUT"), 0., 100., 1., 90.));

    //   degreeout->setDelay(std::max(options.adjusterMinDelay,
    //   options.adjusterMaxDelay));
    ...
    /*
        ...
        Gtk::Image* igreenR1 = Gtk::manage (new RTImage ("circle-green-small"));
    */
    tempout = Gtk::manage(new Adjuster(M("TP_WBALANCE_TEMPERATURE"), MINTEMP0, MAXTEMP0,
        5, CENTERTEMP0, itempR1, itempL1, &wbSlider2Temp, &wbTemp2Slider));
    greenout = Gtk::manage(new Adjuster(
        M("TP_WBALANCE_GREEN"), MINGREEN0, MAXGREEN0, 0.001, 1.0, igreenR1, igreenL1));
    ...
    // I renable tempout with addautobutton to work properly (and all code disabled).
    // There are certainly some redundancies, but it doesn't matter
    tempout->show();
    greenout->show();
    ybout->show();
    ...
    gamutconn = gamut->signal_toggled().connect(
        sigc::mem_fun(*this, &ColorAppearance::gamut_toggled));
    ...
    /*
        ...
        badpixconn = badpix->signal_toggled().connect( sigc::mem_fun(*this,
       &ColorAppearance::badpix_toggled) ); pack_start (*badpix, Gtk::PACK_SHRINK);
    */
    badpixsl = Gtk::manage(new Adjuster(M("TP_COLORAPP_BADPIXSL"), 0, 2, 1, 0));

    //   badpixsl->setDelay(std::max(options.adjusterMinDelay,
    //   options.adjusterMaxDelay));
    ...
    //   surrconn = surrsource->signal_toggled().connect ( sigc::mem_fun (*this,
    //   &ColorAppearance::surrsource_toggled) );
    wbmodelconn = wbmodel->signal_changed().connect(
        sigc::mem_fun(*this, &ColorAppearance::wbmodelChanged));
    illumconn = illum->signal_changed().connect(
        sigc::mem_fun(*this, &ColorAppearance::illumChanged));
    algoconn = algo->signal_changed().connect(
        sigc::mem_fun(*this, &ColorAppearance::algoChanged));
    surroundconn = surround->signal_changed().connect(
        sigc::mem_fun(*this, &ColorAppearance::surroundChanged));
    surrsrcconn = surrsrc->signal_changed().connect(
        sigc::mem_fun(*this, &ColorAppearance::surrsrcChanged));
...
void ColorAppearance::read(const ProcParams *pp, const ParamsEdited *pedited)
{
    ...
    curveMode3Changed(); // This will set the correct sensitive state of depending
                         // Adjusters
    ...
    if (pedited) {
        ...
        degreeout->setEditedState(
            pedited->colorappearance.degreeout ? Edited : UnEdited);
        ...
        rstprotection->setEditedState(
            pedited->colorappearance.rstprotection ? Edited : UnEdited);
        tempout->setEditedState(pedited->colorappearance.tempout ? Edited : UnEdited);
        ...
        qcontrast->setEditedState(
            pedited->colorappearance.qcontrast ? Edited : UnEdited);
        ...
        degreeout->setAutoInconsistent(
            multiImage && !pedited->colorappearance.autodegreeout);
        adapscen->setAutoInconsistent(
            multiImage && !pedited->colorappearance.autoadapscen);
        ...
        tempout->setAutoInconsistent(
            multiImage && !pedited->colorappearance.autotempout);
...
void ColorAppearance::write(ProcParams *pp, ParamsEdited *pedited)
{
    ...
    if (pedited) {
        pedited->colorappearance.complexmethod =
            complexmethod->get_active_text() != M("GENERAL_UNCHANGED");
        pedited->colorappearance.modelmethod =
            modelmethod->get_active_text() != M("GENERAL_UNCHANGED");
        pedited->colorappearance.catmethod =
            catmethod->get_active_text() != M("GENERAL_UNCHANGED");
        ...
        pedited->colorappearance.surround =
            surround->get_active_text() != M("GENERAL_UNCHANGED");
        pedited->colorappearance.surrsrc =
            surrsrc->get_active_text() != M("GENERAL_UNCHANGED");
        pedited->colorappearance.wbmodel =
            wbmodel->get_active_text() != M("GENERAL_UNCHANGED");
        pedited->colorappearance.illum =
            illum->get_active_text() != M("GENERAL_UNCHANGED");
        pedited->colorappearance.algo =
        ...
        pedited->colorappearance.curveMode =
            toneCurveMode->get_active_row_number() != 2;
        pedited->colorappearance.curveMode2 =
            toneCurveMode2->get_active_row_number() != 2;
        pedited->colorappearance.curveMode3 =
            toneCurveMode3->get_active_row_number() != 3;
...
void ColorAppearance::convertParamToNormal()
{
    ...
    if (catmethod->get_active_row_number() == 1 ||
        catmethod->get_active_row_number() == 2) {
        wbmodel->set_active(2);
        illumChanged();
    }
...
void ColorAppearance::curveMode1Changed()
{
    if (listener) {
        Glib::signal_idle().connect(
            sigc::mem_fun(*this, &ColorAppearance::curveMode1Changed_));
    }
}
...
void ColorAppearance::curveMode2Changed()
{
    if (listener) {
        Glib::signal_idle().connect(
            sigc::mem_fun(*this, &ColorAppearance::curveMode2Changed_));
    }
}
...
void ColorAppearance::curveMode3Changed()
{
    ...
    if (listener) {
        Glib::signal_idle().connect(
            sigc::mem_fun(*this, &ColorAppearance::curveMode3Changed_));
    }
}
    ...
    const ProcParams *defParams, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        degree->setDefaultEditedState(
            pedited->colorappearance.degree ? Edited : UnEdited);
        degreeout->setDefaultEditedState(
            pedited->colorappearance.degreeout ? Edited : UnEdited);
        adapscen->setDefaultEditedState(
            pedited->colorappearance.adapscen ? Edited : UnEdited);
        ybscen->setDefaultEditedState(
            pedited->colorappearance.ybscen ? Edited : UnEdited);
        adaplum->setDefaultEditedState(
            pedited->colorappearance.adaplum ? Edited : UnEdited);
        badpixsl->setDefaultEditedState(
            pedited->colorappearance.badpixsl ? Edited : UnEdited);
        jlight->setDefaultEditedState(
            pedited->colorappearance.jlight ? Edited : UnEdited);
        qbright->setDefaultEditedState(
            pedited->colorappearance.qbright ? Edited : UnEdited);
        chroma->setDefaultEditedState(
            pedited->colorappearance.chroma ? Edited : UnEdited);
        schroma->setDefaultEditedState(
            pedited->colorappearance.schroma ? Edited : UnEdited);
        mchroma->setDefaultEditedState(
            pedited->colorappearance.mchroma ? Edited : UnEdited);
        rstprotection->setDefaultEditedState(
            pedited->colorappearance.rstprotection ? Edited : UnEdited);
        contrast->setDefaultEditedState(
            pedited->colorappearance.contrast ? Edited : UnEdited);
        qcontrast->setDefaultEditedState(
            pedited->colorappearance.qcontrast ? Edited : UnEdited);
        colorh->setDefaultEditedState(
            pedited->colorappearance.colorh ? Edited : UnEdited);
        tempout->setDefaultEditedState(
            pedited->colorappearance.tempout ? Edited : UnEdited);
        greenout->setDefaultEditedState(
            pedited->colorappearance.greenout ? Edited : UnEdited);
        ybout->setDefaultEditedState(
            pedited->colorappearance.ybout ? Edited : UnEdited);
        tempsc->setDefaultEditedState(
            pedited->colorappearance.tempsc ? Edited : UnEdited);
        greensc->setDefaultEditedState(
            pedited->colorappearance.greensc ? Edited : UnEdited);
...
{
    idle_register.add([this, ccam, ccamout]() -> bool {
        disableListener();
...
void ColorAppearance::adapCamChanged(double cadap)
{
    ...
    idle_register.add([this, cadap]() -> bool {
        disableListener();
...
{ // reactivate this function
    ...
    idle_register.add([this, temp, tin, autotemp]() -> bool
...
void ColorAppearance::ybCamChanged(int ybsc)
{
    ...
    idle_register.add([this, ybsc]() -> bool {
        disableListener();
...
void ColorAppearance::colorForValue(double valX, double valY,
...
{
    if (wbmodel->get_active_row_number() == 0 ||
        wbmodel->get_active_row_number() == 1) {
        illum->hide();
        illumHBox->hide();
        tempsc->hide();
        greensc->hide();
...
void ColorAppearance::algoChanged()
{
    ...
    if (algo->get_active_row_number() == 0) {
        ...
        curveEditorG2->show();
        curveEditorG3->show();
    } else if (algo->get_active_row_number() >=
               3) { // ">=3" because everything has to be visible with the "(unchanged)"
                    // option too
        contrast->show();
        rstprotection->show();
        qcontrast->show();
...
void ColorAppearance::updateCurveBackgroundHistogram(const LUTu &histToneCurve,
...
void ColorAppearance::setAdjusterBehavior(bool degreeadd, bool adapscenadd,
    bool adaplumadd, bool badpixsladd, bool jlightadd, bool chromaadd, bool contrastadd,
    bool rstprotectionadd, bool qbrightadd, bool qcontrastadd, bool schromaadd,
    bool mchromaadd, bool colorhadd, bool degreeoutadd, bool tempoutadd)
{
