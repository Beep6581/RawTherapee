...
/*
 * TODO: Maybe we could just send back the history string instead of the individual
 * values?
 */
...
public:
    ...
    // to be used by listener that has created a ThresholdAdjuster with with single
    // threshold and precision > 0
    virtual void adjusterChanged(
        ThresholdAdjuster *a, double newBottom, double newTop) = 0;
    // to be used by listener that has created a ThresholdAdjuster with with double
    // threshold and precision > 0
    virtual void adjusterChanged(ThresholdAdjuster *a, double newBottomLeft,
        double newTopLeft, double newBottomRight, double newTopRight) = 0;
    // to be used by listener that has created a ThresholdAdjuster with with single
    // threshold and precision == 0
    virtual void adjusterChanged(ThresholdAdjuster *a, int newBottom, int newTop) = 0;
    // to be used by listener that has created a ThresholdAdjuster with with double
    // threshold and precision == 0
    virtual void adjusterChanged(ThresholdAdjuster *a, int newBottomLeft,
        int newTopLeft, int newBottomRight, int newTopRight) = 0;
    virtual void adjusterChanged2(ThresholdAdjuster *a, int newBottomL, int newTopL,
        int newBottomR, int newTopR) = 0;
};
...
public:
    // Custom Threshold widget with 2 cursors (Top and Bottom) and a custom curve
    // provided by the instanciator, each cursor having his own range, default value and
    // precision
    ThresholdAdjuster(Glib::ustring label, double minValueBottom, double maxValueBottom,
    ...
    ThresholdAdjuster(Glib::ustring label, double minValue, double maxValue,
        double defBottom, double defTop, unsigned int precision, bool startAtOne,
        bool editedCheckBox = false);
    // Simple Threshold widget with 4 linked sliders by pairs (0->1->0 or 1->0->1)
    ThresholdAdjuster(Glib::ustring label, double minValue, double maxValue,
        double defBottomLeft, double defTopLeft, double defBottomRight,
        double defTopRight, unsigned int precision, bool startAtOne,
        bool editedCheckBox = false);
    ...
    void setDefault(double defBottomLeft, double defTopLeft, double defBottomRight,
        double defTopRight);
