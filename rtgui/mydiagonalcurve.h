...
public:
    ...
    std::vector<double> x,
        y; // in case of parametric curves the curve parameters are stored in vector x.
           // In other cases these vectors store the coordinates of the bullets.
};
...
protected:
    ...
    // Get the cursor position and unclamped position from the curve given an X value ;
    // BEWARE: can be time consuming, use with care
    void getCursorPositionFromCurve(float x);
    void getCursorPositionFromCurve(int x);
    // Get the cursor position and unclamped value depending on cursor's position in the
    // graph
