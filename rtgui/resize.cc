...
Resize::Resize() :
    FoldableToolPanel(this, TOOL_NAME, M("TP_RESIZE_LABEL"), false, true), maxw(100000),
    maxh(100000)
{
    ...
    wbox->pack_start(
        *Gtk::manage(new Gtk::Label(M("TP_RESIZE_W"))), Gtk::PACK_SHRINK, 0);
    wbox->pack_start(*w);
    hbox->set_spacing(3);
    hbox->pack_start(
        *Gtk::manage(new Gtk::Label(M("TP_RESIZE_H"))), Gtk::PACK_SHRINK, 0);
    hbox->pack_start(*h);
    lebox->set_spacing(3);
    lebox->pack_start(
        *Gtk::manage(new Gtk::Label(M("TP_RESIZE_LE"))), Gtk::PACK_SHRINK, 0);
    lebox->pack_start(*le);
    sebox->set_spacing(3);
    sebox->pack_start(
        *Gtk::manage(new Gtk::Label(M("TP_RESIZE_SE"))), Gtk::PACK_SHRINK, 0);
    sebox->pack_start(*se);
...
void Resize::adjusterChanged(Adjuster *a, double newval)
{
    ...
    if (listener && (getEnabled() || batchMode)) {
        listener->panelChanged(
            EvResizeScale, Glib::ustring::format(std::setw(5), std::fixed,
                               std::setprecision(2), scale->getValue()));
    }
}
...
void Resize::methodChanged()
{
    ...
    // Post-resize Sharpening assumes the image is in Lab space, and currently Lanczos
    // is the only method which uses that space, and Lanczos is on row 0.
    if (method->get_active_row_number() == 0) {
...
void Resize::setDimensions()
{
    idle_register.add([this]() -> bool {
        ...
        switch (spec->get_active_row_number()) {
            case 0: {
                // Scale mode
                w->set_value(static_cast<double>(static_cast<int>(
                    static_cast<double>(refw) * scale->getValue() + 0.5)));
                h->set_value(static_cast<double>(static_cast<int>(
                    static_cast<double>(refh) * scale->getValue() + 0.5)));
                break;
            }
            ...
            case 4: {
                ...
                if (refw > refh) {
                    const double tmp_scale =
                        le->get_value() / static_cast<double>(refw);
                    scale->setValue(tmp_scale);
                    se->set_value(static_cast<double>(
                        static_cast<int>(static_cast<double>(refh) * tmp_scale + 0.5)));
                } else {
                    const double tmp_scale =
                        le->get_value() / static_cast<double>(refh);
                    scale->setValue(tmp_scale);
                    se->set_value(static_cast<double>(
                        static_cast<int>(static_cast<double>(refw) * tmp_scale + 0.5)));
                }
            ...
            case 5: {
                ...
                if (refw > refh) {
                    const double tmp_scale =
                        se->get_value() / static_cast<double>(refh);
                    scale->setValue(tmp_scale);
                    le->set_value(static_cast<double>(
                        static_cast<int>(static_cast<double>(refw) * tmp_scale + 0.5)));
                } else {
                    const double tmp_scale =
                        se->get_value() / static_cast<double>(refw);
                    scale->setValue(tmp_scale);
                    le->set_value(static_cast<double>(
                        static_cast<int>(static_cast<double>(refh) * tmp_scale + 0.5)));
                }
...
void Resize::entryWChanged()
{
    ...
    if (!batchMode) {
        if (spec->get_active_row_number() == 3) {
            ...
            scale->setValue(
                w->get_value() / (cropw && appliesTo->get_active_row_number() == 0
                                         ? (double)cropw
                                         : (double)maxw));
    ...
    if (listener) {
        if (spec->get_active_row_number() == 3) {
            ...
            if (getEnabled() || batchMode) {
                listener->panelChanged(
                    EvResizeWidth, Glib::ustring::format(w->get_value_as_int()));
            }
        }
    }
}
...
void Resize::entryHChanged()
{
    ...
    if (!batchMode && listener) {
        if (spec->get_active_row_number() == 3) {
            ...
            scale->setValue(
                h->get_value() / (croph && appliesTo->get_active_row_number() == 0
                                         ? (double)croph
                                         : (double)maxh));
    ...
    if (listener) {
        if (spec->get_active_row_number() == 3) {
            ...
            if (getEnabled() || batchMode) {
                listener->panelChanged(
                    EvResizeHeight, Glib::ustring::format(h->get_value_as_int()));
            }
        }
    }
}
...
void Resize::entryLEChanged()
{
    ...
    if (!batchMode && listener) {
        ...
        if (refw > refh) {
            ...
            scale->setValue(
                le->get_value() / (cropw && appliesTo->get_active_row_number() == 0
                                          ? (double)cropw
                                          : (double)maxw));
        } else {
            se->set_value((double)(getComputedWidth(le->get_value())));
            scale->setValue(
                le->get_value() / (croph && appliesTo->get_active_row_number() == 0
                                          ? (double)croph
                                          : (double)maxh));
        }
    ...
    if (listener) {
        if (getEnabled() || batchMode) {
            listener->panelChanged(
                EvResizeLongedge, Glib::ustring::format(le->get_value_as_int()));
        }
    }
}
...
void Resize::entrySEChanged()
{
    ...
    if (!batchMode && listener) {
        ...
        if (refw > refh) {
            ...
            scale->setValue(
                se->get_value() / (croph && appliesTo->get_active_row_number() == 0
                                          ? (double)croph
                                          : (double)maxh));
        } else {
            le->set_value((double)(getComputedHeight(se->get_value())));
            scale->setValue(
                se->get_value() / (cropw && appliesTo->get_active_row_number() == 0
                                          ? (double)cropw
                                          : (double)maxw));
        }
    ...
    if (listener) {
        if (getEnabled() || batchMode) {
            listener->panelChanged(
                EvResizeShortedge, Glib::ustring::format(se->get_value_as_int()));
        }
    }
}
...
void Resize::notifyBBox()
{
    if (listener && (getEnabled() || batchMode)) {
        listener->panelChanged(
            EvResizeBoundingBox, Glib::ustring::compose("(%1x%2)", (int)w->get_value(),
                                     (int)h->get_value()));
    }
}
...
void Resize::setAdjusterBehavior(bool scaleadd) { scale->setAddMode(scaleadd); }
