...
bool EditWindow::isMultiDisplayEnabled() {
    ...
    if (screen) {
        return options.multiDisplayMode > 0 &&
...
void EditWindow::restoreWindow() {
    if (isClosed) {
        ...
        if (isMultiDisplayEnabled()) {
            if (options.meowMonitor >=
                0) { // Use display from last session if available
                meowMonitor = std::max(
                ...
                for (int id = 0; id < monitor_nb; id++) {
                    if (display->get_monitor_at_window(wnd) ==
...
void EditWindow::remEditorPanel(EditorPanel *ep) {
    ...
    if (mainNB->get_n_pages() > 0) {
        EditorPanel *ep1 = static_cast<EditorPanel *>(
...
void EditWindow::toFront() {
    ...
    // opening 2nd, 3rd... editor, which is annoying It will also deiconify the window
    // To avoid unexpected behavior while window is being updated, present() function is
...
bool EditWindow::keyPressed(GdkEventKey *event) {
    ...
    // Shortcut F11: Go full-screen
    if (event->keyval == GDK_KEY_F11) {
        toggleFullscreen();
        return true;
    } else {
        if (mainNB->get_n_pages() >
            0) { // pass the handling for the editor panels, if there are any
...
void EditWindow::updateExternalEditorWidget(
