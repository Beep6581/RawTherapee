...
#define RADIUS                                                                         \
    3.5 /** radius of the control points ; must be x.5 to target the center of a pixel \
         */
#define CBAR_WIDTH 10 /** inner width of the colored bar (border excluded) */
#define CBAR_MARGIN                                                                    \
    1 /** spacing between the colored bar and the graph's bullet when set at 0,0 */
...
protected:
    float pipetteR, pipetteG, pipetteB; /// RGB values from the PipetteDataProvider ; if
                                        /// a channel is set to -1.0f, it is not used
    float pipetteVal; /// Effective pipette value, i.e. where to create the point; if a
                      /// point already exist near this value, it'll be used

    CurveListener *listener;
    ColoredBar *leftBar;
    ColoredBar *bottomBar;
    CursorShape cursor_type;
    double graphX, graphY, graphW,
        graphH; /// position and dimensions of the inner graphic area, excluding the
                /// graph's border and the surrounding space for the points or for the
                /// colored bar
    ...
    bool buttonPressed;
    /**
     * snapToElmt, which will be used for the Y axis only,  must be interpreted like
     * this: -100     : no element (default) -3       : maximum value -2       :
     * identity value -1       : minimum value [0;1000[ : control point that it's
     * snapped to
     * >=1000   : moved control point which snaps to the line made by its previous and
     * next point
     */
    int snapToElmt;
    bool snapTo;
...
public:
    ...
    virtual void reset(
        const std::vector<double> &resetCurve, double identityValue = 0.5) = 0;

    virtual void pipetteMouseOver(
        CurveEditor *ce, EditDataProvider *provider, int modifierKey) = 0;
