...
public:
    ...
    virtual void developRequested(
        const std::vector<FileBrowserEntry *> &tbe, bool fastmode) = 0;
    ...
    virtual void deleteRequested(const std::vector<FileBrowserEntry *> &tbe,
        bool inclBatchProcessed, bool onlySelected) = 0;
    virtual void copyMoveRequested(
        const std::vector<FileBrowserEntry *> &tbe, bool moveRequested) = 0;
    virtual void selectionChanged(const std::vector<Thumbnail *> &tbe) = 0;
    virtual void clearFromCacheRequested(
        const std::vector<FileBrowserEntry *> &tbe, bool leavenotrace) = 0;
...
protected:
    ...
    std::map<Glib::ustring, const ExtProgAction *>
        mMenuExtProgs; // key is menuitem label
...
public:
    ...
    FileBrowserEntry *delEntry(const Glib::ustring
