...
void Geometry::setMOChannelColor(
    Cairo::RefPtr<Cairo::Context> &cr, ObjectMOBuffer *objectBuffer, unsigned short id)
{
    switch (objectBuffer->getObjectMode()) {
        ...
        default:
            /* In OM_65535 mode, FORMAT_RGB16_565 format is used:
                - Red is represented with 5 bits (mask = 0xF800, left shift = 11) from 0
               to 31
                - Green is represented with 6 bits (mask = 0x7E0, left shift = 5) from 0
               to 63
    ...
    ObjectMOBuffer *objectBuffer, EditCoordSystem &coordSystem)
{
    if (flags & F_VISIBLE) {
        if (state != INSENSITIVE) {
            ...
            cr->set_source_rgba(
                color.getR(), color.getG(), color.getB(), INNERGEOM_OPACITY);
        }

        cr->set_line_width(innerLineWidth);
        cr->set_line_cap(
            flags & F_DASHED ? Cairo::LINE_CAP_BUTT : Cairo::LINE_CAP_ROUND);
        ...
        if (filled) {
            cr->arc(
                center_.x + 0.5, center_.y + 0.5, radius_, 0., 2. * rtengine::RT_PI);
...
void Circle::drawToMOChannel(Cairo::RefPtr<Cairo::Context> &cr, unsigned short id,
    ObjectMOBuffer *objectBuffer, EditCoordSystem &coordSystem)
{
    if (flags & F_HOVERABLE) {
        ...
        if (datum == IMAGE) {
            coordSystem.imageCoordToCropCanvas(
                center.x, center.y, center_.x, center_.y);
        } else if (datum == CLICKED_POINT) {
            center_ += objectBuffer->getDataProvider()->posScreen;
        } else if (datum == CURSOR) {
            center_ += objectBuffer->getDataProvider()->posScreen +
...
void Line::drawToMOChannel(Cairo::RefPtr<Cairo::Context> &cr, unsigned short id,
    ObjectMOBuffer *objectBuffer, EditCoordSystem &coordSystem)
{
    if (flags & F_HOVERABLE) {
        cr->set_line_width(getMouseOverLineWidth());
    ...
    ObjectMOBuffer *objectBuffer, EditCoordSystem &coordSystem)
{
    ...
    if ((flags & F_VISIBLE) && state != INSENSITIVE && points.size() > 1 &&
        ...
        for (unsigned int i = 0; i < points.size(); ++i) {
            ...
            if (datum == IMAGE) {
                coordSystem.imageCoordToScreen(
                    points.at(i).x, points.at(i).y, currPos.x, currPos.y);
            } else if (datum == CLICKED_POINT) {
...
void Polyline::drawToMOChannel(Cairo::RefPtr<Cairo::Context> &cr, unsigned short id,
    ObjectMOBuffer *objectBuffer, EditCoordSystem &coordSystem)
{
    ...
    unsigned short id, ObjectMOBuffer *objectBuffer, EditCoordSystem &coordSystem)
{
    if (flags & F_HOVERABLE) {
        ...
        if (datum == IMAGE) {
            coordSystem.imageCoordToCropCanvas(
                bottomRight.x, bottomRight.y, br.x, br.y);
        } else if (datum == CLICKED_POINT) {
            br = bottomRight + objectBuffer->getDataProvider()->posScreen;
        } else if (datum == CURSOR) {
            br = bottomRight + objectBuffer->getDataProvider()->posScreen +
    ...
    ObjectMOBuffer *objectBuffer, EditCoordSystem &coordSystem)
{
    if ((flags & F_VISIBLE) && state != INSENSITIVE) {
        ...
        rtengine::Coord center_ = center;
        double radYT_ = radiusInImageSpace
                            ? coordSystem.scaleValueToCanvas(double(radYT))
                            : double(radYT);
        double radY_ = radiusInImageSpace ? coordSystem.scaleValueToCanvas(double(radY))
                                          : double(radY);
        double radXL_ = radiusInImageSpace
                            ? coordSystem.scaleValueToCanvas(double(radXL))
                            : double(radXL);
        ...
        if (radYT_ > 0 && radY_ > 0 && radXL_ > 0 && radX_ > 0) {
            ...
            // shall be twisted with a scale of radX for x-axis, radY for y-axis Center
            // of coordinates (x, y) in previous coordinates system becomes (X, Y) =
            // (radX * x, radY * y) in new one To go back to previous location, center
            // shall be translated to tx = -X * (1 - 1 / radX) in x-axis (x = tx + X)
    ...
    ObjectMOBuffer *objectBuffer, EditCoordSystem &coordSystem)
{
    if (flags & F_VISIBLE) {
        if (state != INSENSITIVE) {
            ...
            cr->set_source_rgba(
                color.getR(), color.getG(), color.getB(), opacity / 100.);
        }
        ...
        if (filled && state != INSENSITIVE) {
            if (radYT_ > 0 && radY_ > 0 && radXL_ > 0 && radX_ > 0) {
                ...
                // Center of coordinates (x, y) in previous coordinates system becomes
                // (X, Y) = (radX * x, radY * y) in new one To go back to previous
                // location, center shall be translated to tx = -X * (1 - 1 / radX) in
                // x-axis (x = tx + X) and ty = -Y * (1 - 1 / radY) in y-axis (y = ty +
                // Y)
                cr->save();
                ...
                cr->scale(radX_, radY_);
                cr->translate(
                    -center_.x * (1 - 1 / radX_), -center_.y * (1 - 1 / radY_));
                cr->arc(center_.x, center_.y, 1.0, 0.0, rtengine::RT_PI_2);
                ...
                cr->scale(radXL_, radY_);
                cr->translate(
                    -center_.x * (1 - 1 / radXL_), -center_.y * (1 - 1 / radY_));
                cr->arc(center_.x, center_.y, 1.0, rtengine::RT_PI_2, rtengine::RT_PI);
                cr->scale(radXL_, radY_);
                ...
                cr->scale(radXL_, radYT_);
                cr->translate(
                    -center_.x * (1 - 1 / radXL_), -center_.y * (1 - 1 / radYT_));
                cr->arc(
                    center_.x, center_.y, 1.0, rtengine::RT_PI, 3. * rtengine::RT_PI_2);
                ...
                cr->scale(radX_, radYT_);
                cr->translate(
                    -center_.x * (1 - 1 / radX_), -center_.y * (1 - 1 / radYT_));
                cr->arc(center_.x, center_.y, 1.0, 3. * rtengine::RT_PI_2,
                    2. * rtengine::RT_PI);
...
void Ellipse::drawToMOChannel(Cairo::RefPtr<Cairo::Context> &cr, unsigned short id,
    ObjectMOBuffer *objectBuffer, EditCoordSystem &coordSystem)
{
    if (flags & F_HOVERABLE) {
        cr->set_line_width(getMouseOverLineWidth());
...
OPIcon::OPIcon(const std::shared_ptr<RTSurface> &normal,
    ...
    const std::shared_ptr<RTSurface> &insensitive, DrivenPoint drivenPoint) :
...
OPIcon::OPIcon(Glib::ustring normalImage, Glib::ustring activeImage,
    Glib::ustring prelightImage, Glib::ustring draggedImage,
    Glib::ustring insensitiveImage, DrivenPoint drivenPoint) : drivenPoint(drivenPoint)
{
    ...
    if (!prelightImage.empty()) {
        prelightImg = std::shared_ptr<RTSurface>(
            new RTSurface(prelightImage, Gtk::ICON_SIZE_MENU));
    }
    ...
    if (!draggedImage.empty()) {
        draggedImg = std::shared_ptr<RTSurface>(
            new RTSurface(draggedImage, Gtk::ICON_SIZE_MENU));
    }
...
const std::shared_ptr<RTSurface> OPIcon::getNormalImg() { return normalImg; }
const std::shared_ptr<RTSurface> OPIcon::getPrelightImg() { return prelightImg; }
const std::shared_ptr<RTSurface> OPIcon::getActiveImg() { return activeImg; }
const std::shared_ptr<RTSurface> OPIcon::getDraggedImg() { return draggedImg; }
const std::shared_ptr<RTSurface> OPIcon::getInsensitiveImg() { return insensitiveImg; }
...
void OPIcon::drawMOImage(std::shared_ptr<RTSurface> &img,
    Cairo::RefPtr<Cairo::Context> &cr, unsigned short id, ObjectMOBuffer *objectBuffer,
    EditCoordSystem &coordSystem)
{
