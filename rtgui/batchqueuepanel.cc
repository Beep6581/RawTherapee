...
BatchQueuePanel::BatchQueuePanel(FileCatalog *aFileCatalog) : parent(nullptr)
{
    ...
    templateHelpButton->set_tooltip_markup(
        M("QUEUE_LOCATION_TEMPLATE_HELP_BUTTON_TOOLTIP"));
    ...
    destinationPreviewLabel->set_selectable(
        true); // so users can copy the path to the clipboard
    ...
    destinationPreviewScrolledWindow->set_policy(
    ...
    setExpandAlignProperties(
        saveFormatPanel, true, false, Gtk::ALIGN_FILL, Gtk::ALIGN_CENTER);
    ...
    useFolder->signal_toggled().connect(
        sigc::mem_fun(*this, &BatchQueuePanel::saveOptions));
    templateHelpButton->signal_toggled().connect(
        sigc::mem_fun(*this, &BatchQueuePanel::templateHelpButtonToggled));
    ...
    scrolledTemplateHelpWindow->set_visible(
        false); // initially hidden, templateHelpButton shows it
    ...
    zoomBox->pack_start(*Gtk::manage(new Gtk::Separator(Gtk::ORIENTATION_VERTICAL)),
        Gtk::PACK_SHRINK, 4);
    Gtk::Label *zoomLabel = Gtk::manage(
        new Gtk::Label(Glib::ustring("<b>") + M("FILEBROWSER_THUMBSIZE") + ":</b>"));
    ...
    zoomInButton->set_image(
        *Gtk::manage(new RTImage("magnifier-plus", Gtk::ICON_SIZE_LARGE_TOOLBAR)));
    ...
    zoomOutButton->set_image(
        *Gtk::manage(new RTImage("magnifier-minus", Gtk::ICON_SIZE_LARGE_TOOLBAR)));
...
// it is expected to have a non null forceOrientation value on Preferences update only.
// In this case, qsize is ignored and computed automatically
void BatchQueuePanel::updateTab(int qsize, int forceOrientation)
{
    ...
    if ((forceOrientation == 0 && options.mainNBVertical) || (forceOrientation == 2)) {
        ...
        if (!qsize) {
            grid->attach_next_to(
                *Gtk::manage(new RTImage("gears", Gtk::ICON_SIZE_LARGE_TOOLBAR)),
                Gtk::POS_TOP, 1, 1);
            l = Gtk::manage(new Gtk::Label(Glib::ustring(" ") + M("MAIN_FRAME_QUEUE")));
        } else if (qStartStop->get_active()) {
            grid->attach_next_to(
                *Gtk::manage(new RTImage("gears-play", Gtk::ICON_SIZE_LARGE_TOOLBAR)),
                Gtk::POS_TOP, 1, 1);
            l = Gtk::manage(new Gtk::Label(Glib::ustring(" ") + M("MAIN_FRAME_QUEUE") +
                                           " [" + Glib::ustring::format(qsize) + "]"));
        } else {
            grid->attach_next_to(
                *Gtk::manage(new RTImage("gears-pause", Gtk::ICON_SIZE_LARGE_TOOLBAR)),
                Gtk::POS_TOP, 1, 1);
            l = Gtk::manage(new Gtk::Label(Glib::ustring(" ") + M("MAIN_FRAME_QUEUE") +
                                           " [" + Glib::ustring::format(qsize) + "]"));
        }
        ...
        if (!qsize) {
            grid->attach_next_to(
                *Gtk::manage(new RTImage("gears", Gtk::ICON_SIZE_LARGE_TOOLBAR)),
                Gtk::POS_RIGHT, 1, 1);
            grid->attach_next_to(*Gtk::manage(new Gtk::Label(M("MAIN_FRAME_QUEUE"))),
                Gtk::POS_RIGHT, 1, 1);
        } else if (qStartStop->get_active()) {
            grid->attach_next_to(
                *Gtk::manage(new RTImage("gears-play", Gtk::ICON_SIZE_LARGE_TOOLBAR)),
            ...
            grid->attach_next_to(
                *Gtk::manage(new Gtk::Label(
                    M("MAIN_FRAME_QUEUE") + " [" + Glib::ustring::format(qsize) + "]")),
                Gtk::POS_RIGHT, 1, 1);
        } else {
            grid->attach_next_to(
                *Gtk::manage(new RTImage("gears-pause", Gtk::ICON_SIZE_LARGE_TOOLBAR)),
...
void BatchQueuePanel::queueSizeChanged(int qsize, bool queueRunning, bool queueError,
    const Glib::ustring &queueErrorMessage)
{
    ...
    if (queueError) {
        Gtk::MessageDialog msgd(
            queueErrorMessage, true, Gtk::MESSAGE_ERROR, Gtk::BUTTONS_OK, true);
        msgd.run();
    }
}
...
void BatchQueuePanel::populateTemplateHelpBuffer(Glib::RefPtr<Gtk::TextBuffer> buffer)
{
    ...
    const auto insertTopicHeading = [&pos, buffer](const Glib::ustring &text) {
        pos = buffer->insert_markup(
            pos, Glib::ustring::format("\n\n<u><b>", text, "</b></u>\n"));
    };
    ...
    pos = buffer->insert_markup(
        pos, Glib::ustring::format("<big><b>", mainTitle, "</b></big>\n"));
...
#ifdef _WIN32
    pos = buffer->insert_markup(
        pos, M("QUEUE_LOCATION_TEMPLATE_HELP_PATHS_INTRO_WINDOWS"));
    pos = buffer->insert(pos, "\n");
#endif
    pos = buffer->insert(pos, "\n");
    pos = buffer->insert_markup(pos, M("QUEUE_LOCATION_TEMPLATE_HELP_PATHS_BODY_1"));
#ifdef _WIN32
    const auto exampleFilePath =
        M("QUEUE_LOCATION_TEMPLATE_HELP_PATHS_EXAMPLE_WINDOWS");
#else
    ...
    // Examples are generated from exampleFilePath using the actual template processing
    // function
    const Options savedOptions =
        options; // to be restored after generating example results
    options.saveUsePathTemplate = true;
    // Since this code only ever runs once (the first time the help text is presented),
    // no attempt is made to be efficient. Use a brute-force method to discover the
    // number of elements in exampleFilePath.
    ...
    for (int n = 9; n >= 0; n--) {
        ...
        if (!result.empty()) {
            // The 'd' specifier returns an empty string if N exceeds the number of path
            // elements, so the largest N that does not return an empty string is the
            // number of elements in exampleFilePath.
            pathElementCount = n;
            break;
        }
    }
    // Function inserts examples for a particular specifier, with every valid N value
    // for the number of elements in the path.
    const auto insertPathExamples = [&buffer, &pos, pathElementCount, exampleFilePath](
                                        char letter, int offset1, int mult1,
                                        int offset2, int mult2) {
        for (int n = 0; n < pathElementCount; n++) {
            ...
            pos = buffer->insert_markup(
                pos, Glib::ustring::format("\n  <tt><b>", path1, "</b> = <b>", path2,
                         "</b> = <i>", result1, "</i></tt>"));
            if (result1 != result2) {
                // If this error appears, it indicates a coding error in either
                // BatchQueue::calcAutoFileNameBase or
                // BatchQueuePanel::populateTemplateHelpBuffer.
                pos = buffer->insert_markup(
                    pos, Glib::ustring::format(" ",
                             M("QUEUE_LOCATION_TEMPLATE_HELP_RESULT_MISMATCH"), " ",
                             result2));
            }
        }
    };
    ...
    insertPathExamples(
        'd', pathElementCount, -1, -1, -1); //   <b>%d4</b> = <b>%d-1</b> = <i>home</i>
    insertPathExamples('p', 1, 1, -pathElementCount,
        1); //   <b>%p1</b> = <b>%p-4</b> = <i>/home/tom/photos/2010-10-31/</i>
    insertPathExamples('P', 1, 1, -pathElementCount,
        1); //   <b>%P1</b> = <b>%P-4</b> = <i>2010-10-31/</i>
    {
        const Glib::ustring fspecifier("%f");
        options.savePathTemplate = fspecifier;
        const auto result = BatchQueue::calcAutoFileNameBase(exampleFilePath);
        pos =
            buffer->insert_markup(pos, Glib::ustring::format("\n  <tt><b>", fspecifier,
                                           "</b> = <i>", result, "</i></tt>"));
    }
    ...
    const Glib::ustring dateTimeFormatExamples[] = {
        "%Y-%m-%d", "%Y%m%d_%H%M%S", "%y/%b/%-d/"};
    const auto timezone = Glib::DateTime::create_now_local().get_timezone();
    const auto timeForExamples =
        Glib::DateTime::create_from_iso8601("2001-02-03T04:05:06.123456", timezone);
...
void BatchQueuePanel::pathFolderButtonPressed()
{
    ...
    Gtk::FileChooserDialog fc(getToplevelWindow(this), M("QUEUE_LOCATION_FOLDER"),
