...
struct LocaleToLang
    : private std::map<std::pair<Glib::ustring, Glib::ustring>, Glib::ustring> {
    static const std::pair<Glib::ustring, Glib::ustring> key(
        const Glib::ustring &major, const Glib::ustring &minor = Glib::ustring())
    {
...
void setGtkLanguage(const Glib::ustring &language)
{
    if (language != "default") { // nothing to change when using default
        ...
        lang = lang + ".UTF-8"; // According to Apple documentation, UTF-8 is a built-in
                                // encoding on all platforms on which macOS runs
...
TranslationMetadata::TranslationMetadata(
    std::map<std::string, std::string> &&metadata) : metadata(std::move(metadata))
{
}

std::string TranslationMetadata::get(
...
void MultiLangMgr::load(
    const Glib::ustring &language, const std::vector<Glib::ustring> &fnames)
{
    ...
    for (const auto &fname : fnames) {
        ...
        while (std::getline(file, entry)) {
            ...
            if (std::getline(line, key, ';') &&
                translations.find(key) == translations.end() &&
                ...
                hint =
                    translations.emplace_hint(hint, std::move(key), std::move(value));
            }
        }
    }
}
...
const TranslationMetadata *MultiLangMgr::getMetadata(const Glib::ustring &fname) const
{
    ...
    if (!file.is_open()) {
        if (rtengine::settings->verbose) {
            std::cerr << "Unable to open language file " << fname << " to get metadata."
                      << std::endl;
        }
        return nullptr;
    }
    ...
    const auto read_key_value = [&raw_metadata](const std::string &meta_line) {
        ...
        const auto definition_start =
            meta_line.find_first_not_of(space_chars, first_space + 1);
        if (definition_start == std::string::npos ||
            meta_line[definition_start] != var_symbol) {
            return;
        }
        const auto separator_pos =
            meta_line.find(key_value_separator, definition_start + 1);
        if (separator_pos == std::string::npos) {
            return;
        }
        std::string key = meta_line.substr(
            definition_start + 1, separator_pos - definition_start - 1);
        std::string value = meta_line.substr(separator_pos + 1);
        if (rtengine::settings->verbose) {
            std::cout << "Found metadata key " << key << " with value " << value
                      << std::endl;
        }
    ...
    for (std::string line;
        ...
        (line.empty() || line.front() == comment_symbol ||
...
Glib::ustring MultiLangMgr::getOSUserLanguage()
{
    ...
    const int countryLen =
        GetLocaleInfo(localeID, LOCALE_SISO3166CTRYNAME, &localeName[langLen], 9);
    if (countryLen <= 0) {
        return langName;
    }
    ...
    CFStringRef langCodeStr =
        (CFStringRef)CFLocaleGetValue(cfLocale, kCFLocaleLanguageCode);
    ...
    CFStringRef countryCodeStr =
        (CFStringRef)CFLocaleGetValue(cfLocale, kCFLocaleCountryCode);
    ...
    if (countryCodeStr != NULL) {
        ...
        CFStringGetCString(
            countryCodeStr, countryCodeBuffer, countryCodeStrLength, strEncoding);
    ...
    locale = locale + ".UTF-8"; // According to Apple documentation, UTF-8 is a built-in
                                // encoding on all platforms on which macOS runs
