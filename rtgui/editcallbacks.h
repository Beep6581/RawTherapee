...
public:
private:
    EditUniqueID ID; /// this will be used in improcfun to locate the data that has to
                     /// be stored in the buffer; it must be unique in RT
...
protected:
    ...
    std::vector<Geometry *>
        mouseOverGeometry; /// mouseOver geometry, drawn in a hidden buffer
    enum class Action {
        NONE,     ///
        DRAGGING, /// set action to this value in the buttonPressed event to start
                  /// dragging and ask for drag event
        PICKING   /// set action to this value in the buttonPressed event whenever the
                /// user is picking something through a single click. In this case, the
                /// pickX events will be called INSTEAD of buttonXReleased !
    };
...
public:
    ...
    virtual void
    switchOffEditMode(); /// Occurs when the user want to stop the editing mode
    EditUniqueID getEditID() const;
    EditType getEditingType() const;
    BufferType getPipetteBufferType() const;
    bool isDragging() const; /// Returns true if something is being dragged and drag
                             /// events has to be sent (object mode only)
    ...
    This method is also triggered when the cursor is moving over the image in ET_PIPETTE
    mode
    ...
    /** @brief Triggered when mouse button 1 is pressed, together with the CTRL modifier
    key if the subscriber is of type ET_PIPETTE Once the key is pressed, RT will enter
    in drag1 mode on subsequent mouse movements
    @param modifierKey Gtk's event modifier key (GDK_CONTROL_MASK | GDK_SHIFT_MASK |
    ...)
    @return true if the preview has to be redrawn, false otherwise */
    ...
    /** @brief Triggered when the user is releasing mouse button 1 while in
    action==ES_ACTION_PICKING mode No modifier key is provided, since having a different
    modifier key than on button press will set picked to false.
    @param picked True if the cursor is still above the the same object than on button
    pressed and with the same modifier keys. If false, the user moved the cursor away or
    the modifier key is different, so the element is considered as NOT selected.
    @return true if the preview has to be redrawn, false otherwise */
    virtual bool pick1(bool picked);

    /** @brief Triggered when the user is releasing mouse button 2 while in
    action==ES_ACTION_PICKING mode
    @param picked True if the cursor is still above the the same object than on button
    pressed and with the same modifier keys. If false, the user moved the cursor away or
    the modifier key is different, so the element is considered as NOT selected.
    @return true if the preview has to be redrawn, false otherwise */
    ...
    /** @brief Triggered when the user is releasing mouse button 3 while in
    action==ES_ACTION_PICKING mode
    @param picked True if the cursor is still above the the same object than on button
    pressed and with the same modifier keys. If false, the user moved the cursor away or
    the modifier key is different, so the element is considered as NOT selected.
    ...
    /** @brief Get the geometry to be drawn in the "mouse over" channel, hidden from the
     * user */
 ...
 *
 * It is admitted that only one Subscriber can ask data at a time. If the Subscriber is
 * of type ET_PIPETTE, it will have to trigger the usual event so that the image will be
 * reprocessed to compute the buffer of the current subscriber.
 */
class EditDataProvider
{
...
private:
    ...
    //    int object;            /// ET_OBJECTS mode: Object detected under the cursor,
    //    0 otherwise; ET_PIPETTE mode: 1 if above the image, 0 otherwise
    float pipetteVal1; /// Current pipette values
    float pipetteVal2; /// Current pipette values; if bufferType==BT_SINGLEPLANE_FLOAT,
                       /// will be set to 0
    float pipetteVal3; /// Current pipette values; if bufferType==BT_SINGLEPLANE_FLOAT,
                       /// will be set to 0

public:
    int object; /// ET_OBJECTS mode: Object detected under the cursor, 0 otherwise;
                /// ET_PIPETTE mode: 1 if above the image, 0 otherwise

    rtengine::Coord
        posScreen; /// Location of the mouse button press, in preview image space
    rtengine::Coord
        posImage; /// Location of the mouse button press, in the full image space
    rtengine::Coord deltaScreen;     /// Delta relative to posScreen
    rtengine::Coord deltaImage;      /// Delta relative to posImage
    rtengine::Coord deltaPrevScreen; /// Delta relative to the previous mouse location,
                                     /// in preview image space
    rtengine::Coord deltaPrevImage;  /// Delta relative to the previous mouse location,
    ...
    virtual void
    unsubscribe(); /// Occurs when the subscriber has been switched off first
    virtual void
    switchOffEditMode(); /// Occurs when the user want to stop the editing mode
    int getObject() const;
