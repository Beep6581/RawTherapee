...
Distortion::Distortion() : FoldableToolPanel(this, TOOL_NAME, M("TP_DISTORTION_LABEL"))
{
    ...
    EvDistortionDefishFocalLength =
        mapper->newEvent(TRANSFORM, "HISTORY_MSG_DISTORTION_DEFISH_FOCAL");
    EvDistortionDefishFocalLengthVoid =
        mapper->newEvent(M_VOID, "HISTORY_MSG_DISTORTION_DEFISH_FOCAL");
    ...
    autoDistor->set_image(
        *Gtk::manage(new RTImage("distortion-auto-small", Gtk::ICON_SIZE_BUTTON)));
    ...
    idConn = autoDistor->signal_pressed().connect(
    ...
    distor = Gtk::manage(
        new Adjuster(M("TP_DISTORTION_AMOUNT"), -0.5, 0.5, 0.001, 0, idistL, idistR));
    ...
    focal_length = Gtk::manage(new Adjuster(M("TP_DISTORTION_FOCAL_LENGTH"), 0.5, 25,
        0.01, DistortionParams::DEFAULT_FOCAL_LENGTH));
...
void Distortion::read(const ProcParams *pp, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        ...
        focal_length->setEditedState(
            pedited->distortion.focal_length != DistortionParams::DEFAULT_FOCAL_LENGTH
                ? Edited
                : UnEdited);
    }
...
void Distortion::adjusterChanged(Adjuster *a, double newval)
{
    if (listener) {
        if (a == focal_length) {
            listener->panelChanged(*event_distortion_defish_focal_length,
                Glib::ustring::format(
                    std::setw(4), std::fixed, std::setprecision(2), a->getValue()));
        } else if (a == distor) {
            listener->panelChanged(
                EvDISTAmount, Glib::ustring::format(std::setw(4), std::fixed,
...
{
    if (listener) {
        listener->panelChanged(EvDistortionDefish,
            defish->get_active() ? M("GENERAL_ENABLED") : M("GENERAL_DISABLED"));
        focal_length->setEnabled(defish->get_active());
    }
}
