...
FlatField::FlatField() : FoldableToolPanel(this, TOOL_NAME, M("TP_FLATFIELD_LABEL"))
{
    ...
    flatFieldFile = Gtk::manage(new MyFileChooserButton(
        M("TP_FLATFIELD_LABEL"), Gtk::FILE_CHOOSER_ACTION_OPEN));
    ...
    const rtengine::procparams::ProcParams *pp, const ParamsEdited *pedited)
{
    ...
    if (multiImage ||
        pp->raw.ff_BlurType == procparams::RAWParams::getFlatFieldBlurTypeString(
    ...
    if (pedited) {
        ...
        flatFieldBlurRadius->setEditedState(
            pedited->raw.ff_BlurRadius ? Edited : UnEdited);
        flatFieldClipControl->setEditedState(
            pedited->raw.ff_clipControl ? Edited : UnEdited);
        flatFieldClipControl->setAutoInconsistent(
            multiImage && !pedited->raw.ff_AutoClipControl);
    ...
    if (pp->raw.ff_AutoSelect && ffp && !batchMode) {
        ...
        if (img) {
            ffInfo->set_text(Glib::ustring::compose("%1: f/%2",
        ...
        if (a == flatFieldClipControl) {
            if (flatFieldClipControl->getAutoInconsistent()) {
                listener->panelChanged(
                    EvFlatFieldAutoClipControl, M("GENERAL_UNCHANGED"));
            } else if (flatFieldClipControl->getAutoValue()) {
                listener->panelChanged(
                    EvFlatFieldAutoClipControl, M("GENERAL_ENABLED"));
            } else {
                listener->panelChanged(
                    EvFlatFieldAutoClipControl, M("GENERAL_DISABLED"));
            }
        }
    }
}
    ...
    const rtengine::procparams::ProcParams *defParams, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        flatFieldBlurRadius->setDefaultEditedState(
            pedited->raw.ff_BlurRadius ? Edited : UnEdited);
...
void FlatField::flatFieldFileChanged()
{
    ...
    if (listener) {
        listener->panelChanged(
            EvFlatFieldFile, Glib::path_get_basename(flatFieldFile->get_filename()));
    }
}
...
void FlatField::flatFieldBlurTypeChanged()
{
    ...
    if (listener && curSelection >= 0) {
        listener->panelChanged(
            EvFlatFieldBlurType, flatFieldBlurType->get_active_text());
    }
}
...
void FlatField::checkBoxToggled(CheckBox *c, CheckValue newval)
{
    if (listener && c == flatFieldFromMetaData) {
        listener->panelChanged(EvFlatFieldFromMetaData,
            flatFieldFromMetaData->getLastActive() ? M("GENERAL_ENABLED")
                                                   : M("GENERAL_DISABLED"));
    }
}
