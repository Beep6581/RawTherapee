...
RAWCACorr::RAWCACorr() : FoldableToolPanel(this, TOOL_NAME, M("TP_RAWCACORR_LABEL"))
{
    ...
    EvPreProcessCAAutoiterations =
        m->newEvent(DARKFRAME, "HISTORY_MSG_RAWCACORR_AUTOIT");
    EvPreProcessCAColourshift =
        m->newEvent(DARKFRAME, "HISTORY_MSG_RAWCACORR_COLORSHIFT");
    EvPreProcessCAColourshiftHistory =
        m->newEvent(M_VOID, "HISTORY_MSG_RAWCACORR_COLORSHIFT");
    ...
    caAutoiterations->setDelay(
        std::max(options.adjusterMinDelay, options.adjusterMaxDelay));

    caRed = Gtk::manage(
        new Adjuster(M("TP_RAWCACORR_CARED"), -4.0, 4.0, 0.1, 0, icaredL, icaredR));
    ...
    caRed->show();
    caBlue = Gtk::manage(
        new Adjuster(M("TP_RAWCACORR_CABLUE"), -8.0, 8.0, 0.1, 0, icablueL, icablueR));
    ...
    caAvoidcolourshift =
        Gtk::manage(new CheckBox(M("TP_RAWCACORR_AVOIDCOLORSHIFT"), multiImage));
    ...
    const rtengine::procparams::ProcParams *pp, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        ...
        caAutoiterations->setEditedState(
            pedited->raw.caautoiterations ? Edited : UnEdited);
...
void RAWCACorr::checkBoxToggled(CheckBox *c, CheckValue newval)
{
    if (c == caAutocorrect) {
        ...
        if (listener) {
            listener->panelChanged(EvPreProcessAutoCA, caAutocorrect->getLastActive()
                                                           ? M("GENERAL_ENABLED")
                                                           : M("GENERAL_DISABLED"));
        }
    ...
    const rtengine::procparams::ProcParams *defParams, const ParamsEdited *pedited)
{
    ...
    if (pedited) {
        caAutoiterations->setDefaultEditedState(
            pedited->raw.caautoiterations ? Edited : UnEdited);
