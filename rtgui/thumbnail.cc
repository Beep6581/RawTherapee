...
bool CPBDump(const Glib::ustring &commFName, const Glib::ustring &imageFName,
    ...
    const CacheImageData *cfs, bool flagMode)
{
    ...
    const std::unique_ptr<FILE, int (*)(FILE *)> f(
        g_fopen(commFName.c_str(), "wt"), &std::fclose);
...
Thumbnail::Thumbnail(CacheManager *cm, const Glib::ustring &fname, CacheImageData *cf) :
    ...
    pparams(new ProcParams), pparamsValid(false), imageLoading(false), lastImg(nullptr),
    lastW(0), lastH(0), lastScale(0), initial_(false)
{

    loadProcParams();

    // should be safe to use the unprotected version of loadThumbnail, since we are in
    // the constructor
    _loadThumbnail();
    ...
    if (cfs.rankOld >= 0) {
        ...
        // file
        createProcParamsForUpdate(false,
...
Thumbnail::Thumbnail(CacheManager *cm, const Glib::ustring &fname,
    const std::string &md5, const std::string &xmpSidecarMd5) :
...
void Thumbnail::_generateThumbnailImage()
{
    ...
    tpp = rtengine::Thumbnail::loadFromImage(
        fname, tw, th, -1, pparams->wb.equal, pparams->wb.observer);
    ...
    if (!tpp) {
        ...
        if (initial_ && options.internalThumbIfUntouched) {
            quick = true;
            tpp = rtengine::Thumbnail::loadQuickFromRaw(
                fname, sensorType, tw, th, 1, TRUE);
        }

        if (!tpp) {
            quick = false;
            tpp = rtengine::Thumbnail::loadFromRaw(fname, sensorType, tw, th, 1,
                pparams->wb.equal, pparams->wb.observer, TRUE, &(pparams->raw));
        }
...
bool Thumbnail::isSupported() const { return cfs.supported; }
 ...
 *  @param flaggingMode True if the ProcParams will be created because the file browser
 * is being flagging an image (rank, to trash, color labels). This parameter is passed
 * to the CPB.
 *
...
rtengine::procparams::ProcParams *Thumbnail::createProcParamsForUpdate(
    bool returnParams, bool force, bool flaggingMode)
{
    ...
    const bool run_cpb = !options.CPBPath.empty() && !defaultPparamsPath.empty() &&
                         cfs && cfs->exifValid && create;
        ...
        CPBDump(tmpFileName, fname, outFName,
            defaultPparamsPath == DEFPROFILE_INTERNAL
                ...
                : Glib::build_filename(defaultPparamsPath,
                      Glib::path_get_basename(defProf) + paramFileExtension),
            cfs, flaggingMode);
...
void Thumbnail::loadProcParams()
{
    ...
    if (options.paramsLoadLocation == PLL_Input) {
        ...
        if (!pparamsValid) {
            pparamsValid =
                !pparams->load(getCacheFileName("profiles", paramFileExtension));
        }
    } else {
...

    {
        ...
        if (pparamsValid) {
            ...
            if (cfs.format == FT_Raw && options.internalThumbIfUntouched &&
                ...
                // full thumbnail will be generated unnecessarily, but currently there
                // is no way to pre-check if the format supports quick thumbs.
                initial_ = true;
                _generateThumbnailImage();
                initial_ = false;
            }
...
bool Thumbnail::hasProcParams() const { return pparamsValid; }

void Thumbnail::setProcParams(const ProcParams &pp, ParamsEdited *pe, int whoChangedIt,
...
bool Thumbnail::isRecentlySaved() const { return cfs.recentlySaved; }
...
bool Thumbnail::isEnqueued() const { return enqueueNumber > 0; }

bool Thumbnail::isPixelShift() const { return cfs.isPixelShift; }
bool Thumbnail::isHDR() const { return cfs.isHDR; }
...
void Thumbnail::getFinalSize(
    const rtengine::procparams::ProcParams &pparams, int &w, int &h)
{
    MyMutex::MyLock lock(mutex);

    // WARNING: When downscaled, the ratio have loosed a lot of precision, so we can't
    // get back the exact initial dimensions
...
rtengine::IImage8 *Thumbnail::processThumbImage(
    const rtengine::procparams::ProcParams &pparams, int h, double &scale)
{
    ...
    if (cfs.thumbImgType == CacheImageData::QUICK_THUMBNAIL) {
        ...
        // image = tpp->processImage (pparams, h, rtengine::TI_Bilinear,
        // cfs.getCamera(), cfs.focalLen, cfs.focalLen35mm, cfs.focusDist, cfs.shutter,
        // cfs.fnumber, cfs.iso, cfs.expcomp, scale );
        image = tpp->processImage(pparams,
            static_cast<rtengine::eSensorType>(cfs.sensortype), h,
...
void Thumbnail::generateExifDateTimeStrings()
{
    if (cfs.timeValid) {
        ...
        dateTime = Glib::DateTime::create_local(
            cfs.year, cfs.month, cfs.day, cfs.hour, cfs.min, cfs.sec);
    }
    ...
    exifString = Glib::ustring::compose("f/%1 %2s %3%4 %5mm",
        ...
        M("QINFO_ISO"), cfs.iso,
        Glib::ustring::format(
            std::setw(3), std::fixed, std::setprecision(2), cfs.focalLen));

    if (options.fbShowExpComp && cfs.expcomp != "0.00" &&
        !cfs.expcomp
            .empty()) { // don't show exposure compensation if it is 0.00EV;old cache
                        // files do not have ExpComp, so value will not be displayed.
        exifString = Glib::ustring::compose("%1 %2EV", exifString,
            cfs.expcomp); // append exposure compensation to exifString
    }
}

const Glib::ustring &Thumbnail::getExifString() const { return exifString; }

const Glib::ustring &Thumbnail::getDateTimeString() const { return dateTimeString; }

const Glib::DateTime &Thumbnail::getDateTime() const { return dateTime; }
    ...
    rtengine::StandardObserver observer, double tempBias)
{
    if (cfs.redAWBMul != -1.0) {
        rtengine::ColorTemp ct(
            cfs.redAWBMul, cfs.greenAWBMul, cfs.blueAWBMul, equal, observer);
        temp = ct.getTemp();
        green = ct.getGreen();
    } else {
        temp = green = -1.0;
...
ThFileType Thumbnail::getType() const { return (ThFileType)cfs.format; }
...
int Thumbnail::infoFromImage(const Glib::ustring &fname, CacheImageData &cfs)
{
    ...
    if (idata->hasExif()) {
        ...
        cfs.expcomp = idata->expcompToString(
            idata->getExpComp(), false); // do not mask Zero expcomp
...
void Thumbnail::_loadThumbnail(bool firstTrial)
{
    ...
    if (!initial_) {
        tw = tpp->getImageWidth(getProcParamsU(), th,
 ...
 *  - updatePParams==true (default)        : write the procparams file (sidecar or
 * cache, depending on the options)
 *  - updateCacheImageData==true (default) : write the CacheImageData values in the
 * cache folder, i.e. some General, DateTime, ExifInfo, File info and ExtraRawInfo,
 */
void Thumbnail::updateCache(bool updatePParams, bool updateCacheImageData)
{
    ...
    if (updatePParams && pparamsValid) {
        pparams->save(options.saveParamsFile ? fname + paramFileExtension : "",
...
Glib::ustring Thumbnail::getCacheFileName(
...
int Thumbnail::getRank() const { return properties.rank; }

void Thumbnail::setRank(int rank) { properties.rank = rank; }

int Thumbnail::getColorLabel() const { return properties.color; }

void Thumbnail::setColorLabel(int colorlabel) { properties.color = colorlabel; }

bool Thumbnail::getTrashed() const { return properties.trashed; }

void Thumbnail::setTrashed(bool trashed) { properties.trashed = trashed; }
...
bool Thumbnail::openDefaultViewer(int destination)
{
    ...
    if (destination == 1) {
        openFName = Glib::ustring::compose("%1.%2",
            BatchQueue::calcAutoFileNameBase(fname), options.saveFormatBatch.format);
        ...
        if (Glib::file_test(openFName, Glib::FILE_TEST_EXISTS)) {
            ...
            g_free(org);
                       ...
                       Glib::path_get_dirname(openFName), Glib::FILE_TEST_EXISTS)) {
...
void Thumbnail::loadProperties()
{
    ...
    if (pparamsValid) {
        if (options.thumbnailRankColorMode ==
...
const CacheImageData *Thumbnail::getCacheImageData() const { return &cfs; }

std::string Thumbnail::getMD5() const { return cfs.md5; }
...
bool Thumbnail::isPParamsValid() const { return pparamsValid; }
