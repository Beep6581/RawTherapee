...
void CursorManager::init(Glib::RefPtr<Gdk::Window> mainWindow)
{
    ...
    auto createCursor = [this](const Glib::ustring &name,
                            const Gdk::CursorType &fb_cursor, const double offX = 0.,
                            const double offY = 0.) -> Glib::RefPtr<Gdk::Cursor> {
        // Notes:
        ...
        auto cursor = Gdk::Cursor::create(this->display, cursor_surf.get(),
            std::min(std::max(cursor_surf.getWidth() / 2 * (1. + offX), 0.),
                static_cast<double>(cursor_surf.getWidth())),
            std::min(std::max(cursor_surf.getHeight() / 2 * (1. + offY), 0.),
                static_cast<double>(cursor_surf.getHeight())));
...
void CursorManager::setCursor(Glib::RefPtr<Gdk::Window> window, CursorShape shape)
{
    switch (shape) {
        ...
        case CursorShape::CSArrow:
            window->set_cursor(); // set_cursor without any arguments to select system
                                  // default
            break;
...
void CursorManager::setCursorOfMainWindow(
    Glib::RefPtr<Gdk::Window> window, CursorShape shape)
{
    if (window->get_display() == mainWindowCursorManager.display) {
